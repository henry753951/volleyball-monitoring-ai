...
assert (
    "正式 registry 版本為 `3.0.0`" in spec
    and "Canonical Rally command、ACK 與 snapshot 都使用 `3.0.0`" in spec
)
...
assert (ROOT / "docs/SYSTEM_SPEC_V3_2.md").read_text(encoding="utf-8") == (
    ROOT / "docs/MASTER_IMPLEMENTATION_SPEC.md"
...
for factory in [
    "createClipWorker",
    "createPlaybackPackagerWorker",
    "createAnalysisIngestWorker",
...
for role_file in ["playback-packager.ts", "analysis-ingest.ts", "outbox-publisher.ts"]:
    ...
    assert "TODO" not in source and "createPollingLifecycle" in source, (
        f"worker role remains a scaffold: {role_file}"
    )
for binding in [
    "service: 'Z'",
    "contact: 'X'",
...
for forbidden_terminal_flow in ["kind: 'terminal'", "type: 'terminal'", "kind === 'terminal'"]:
    assert forbidden_terminal_flow not in annotation_page, (
...
for required_runtime_literal in (
    "WORKER_HEALTH_PORT",
    "health/ready",
    "DEV_DATA_ROOT",
    "MINIO_DATA_HOST_PATH",
    ...
    assert required_runtime_literal in compose_source, (
        f"Compose missing runtime health/bind contract: {required_runtime_literal}"
    )
...
for path in active_annotation_sources:
    ...
    for forbidden in ("MARK_TERMINAL", "SET_SCORE", "AWAITING_SCORE", "X 結束"):
        assert forbidden not in text, (
            f"active old annotation flow in {path.relative_to(ROOT)}: {forbidden}"
        )
