...
class SoakHelpersTest(unittest.TestCase):
    def test_stats_units_and_header(self):
        ...
        self.assertEqual(
            parse_stats(text), [{"cpu_pct": 2.5, "mem_mib": 128}, {"cpu_pct": 1.0, "mem_mib": 1024}]
        )
        self.assertAlmostEqual(parse_memory("512KiB"), 0.5)

    def test_manifest_strips_quotes_and_whitespace(self):
        self.assertEqual(
            parse_manifest('#EXTM3U\n #EXTINF:2,\n  "init.mp4" \n media.mp4\n'),
            ["init.mp4", "media.mp4"],
        )
        self.assertEqual(
            parse_manifest('#EXT-X-MAP:URI="init.mp4"\n#EXTINF:2,\nmedia.mp4\n'),
            ["init.mp4", "media.mp4"],
        )
    ...
    def test_restart_delta_and_config(self):
        ...
        args = argparse.Namespace(
            duration_seconds=1, interval_seconds=1, memory_cap_mib=1, growth_cap_mib=1
        )
        validate_config(args, "00000000-0000-4000-8000-00000000d003")
        with self.assertRaises(ValueError):
            validate_config(
                argparse.Namespace(
                    duration_seconds=0, interval_seconds=1, memory_cap_mib=1, growth_cap_mib=1
                ),
                "bad",
            )
    ...
    def test_stopped_service_is_health_failure(self):
        self.assertEqual(
            service_health_failures({"worker": {"state": "exited", "health": "none"}}), 1
        )
        self.assertEqual(
            service_health_failures({"worker": {"state": "running", "health": "none"}}), 0
        )
    ...
    def test_anchor_and_boundary_validation(self):
        validate_anchor(
            {
                ...
                "resolved_player_media_time_us": "3",
                "source_pts": "-4",
            },
            "s",
            1,
        ...
        self.assertTrue(
            expected_error(409, b'{"code":"WINDOW_BOUNDARY"}', {409}, {"WINDOW_BOUNDARY"})
        )
        self.assertFalse(
            expected_error(200, b'{"code":"WINDOW_BOUNDARY"}', {409}, {"WINDOW_BOUNDARY"})
        )
