# Annotation, playback, and ReID rearchitecture confirmation plan
...
failures, the external `volleyball-analysis-engine` branch audit, and the proposed migration plan.
...
[`ADR 0037`](./adr/0037-versioned-reid-evidence-and-provider-work.md), which supersedes ADR 0035 for
active ReID writes and records the new identity model, every-frame pose evidence, and capability-gated
provider cutover. Sections labeled proposed remain target behavior until their implementation phase is
verified; approval does not make unimplemented behavior current system behavior.
...
- [`ADR 0035`](./adr/0035-fixed-roster-nested-part-adaptation.md), which supersedes ADR 0033
...
- 964 main-repository tests passed; six worker integration cases remained environment-gated and
...
- the local PostgreSQL instance reports 49 migrations up to date, pgvector 0.8.6, and the expected
...
- headed browser verification confirmed that held frame navigation advances before key release and
  ...
  the identity panel exposes separately named apply/rematch/re-extract actions, and the browser
...
- **Person-group assignment**: groups TIDs by the current versioned person-cluster proposal and
  batch-writes effective assignments. Legacy L1–R6 data is labeled “舊關聯”; it is not presented as a
...
## Current ReID data flow

### Worker and contract

The external analysis engine emits optional `AnalysisData.extensions.fixed_roster_reid` v2. Each
canonical tracklet can contain TID aliases, field side, median court position, quality, co-visibility
cannot-links, and four frozen descriptors:

- DINOv2: 384 dimensions
- Sports OSNet: 512 dimensions
- KPR: 4096 dimensions
- KPR Prompt: 4096 dimensions

The worker is otherwise stateless across jobs.

### Callback and central association

The current callback transaction:

1. Persists the immutable AnalysisRun, tracks, raw result, frame chunks, and artifacts.
2. Loads earlier active canonical ReID observations from the same match/model namespace.
3. Groups history by team and fixed S1-S6 slot.
4. Selects a Nested Part Adaptation candidate from earlier clips only, scores current tracklets, and
   solves the six-slot/cannot-link assignment.
5. Stores one `ReidFeatureObservation` per Local alias with descriptor bytes and selected recipe.
6. Applies any effective slot/player binding to Local assignments unless a manual assignment exists.

All six steps, including ReID fitting and assignment, currently run inside the AnalysisRun callback
database transaction. A ReID ingest error can therefore prevent the callback transaction from
committing otherwise valid analysis data.

### Current storage

- Each descriptor arrives as a numeric array inside
  `AnalysisData.extensions.fixed_roster_reid`. The callback encodes every value as a 32-bit
  little-endian float and Prisma stores the resulting `Bytes` field as PostgreSQL `bytea` on
  `ReidFeatureObservation` (`dinoDescriptor`, `osnetDescriptor`, `kprDescriptor`,
  `kprPromptDescriptor`, and the selected `prototype`).
- For fitting/scoring, the server reads those byte arrays and decodes them back into JavaScript
  number arrays. There is no vector index or vector service in the current path.
- The current local image is `postgres:17-alpine`; no pgvector extension or vector column/index is
  present.
- `ReidIdentity` is unique by match/team/slot index.
- `ReidPlayerBinding` is append-only and effective from a set/rally position.
- `TrackIdentityAssignment` is one current effective row per AnalysisRun/TID.
- `ReidCorrectionEvent` records the three assignment correction modes, but clearing a Local
  assignment currently deletes the effective row without adding a ReID correction event.
- Dataset export includes raw fixed-roster payloads and persisted identity/binding/correction views.

### How current vectors reach a later clip

The current system does not send an existing feature bank to the worker. Reuse happens automatically
inside Central when the **later clip's analysis callback** is being committed:

```mermaid
flowchart LR
    Engine["Analysis worker emits four descriptor arrays"]
    Callback["Later clip AnalysisRun callback transaction"]
    Current["Encode current arrays as Float32LE bytea"]
    History["SELECT earlier active canonical observations"]
    Decode["Decode historical bytea to JS arrays"]
    Fit["Fit/score Nested Part Adaptation in server memory"]
    Slot["Solve fixed S1-S6 assignment"]
    Persist["Persist observations and effective assignment"]

    Engine --> Callback
    Callback --> Current
    Callback --> History
    History --> Decode --> Fit --> Slot --> Persist
    Current --> Persist
```

The history query is restricted by the match/team/model namespace and canonical earlier set/rally
position. It then groups prior labels by fixed S1-S6 slot. There is currently no immutable bank
snapshot saying exactly which observations were eligible, no independent `REID_ASSOCIATION` retry,
and no confirmed/rejected/quarantined membership layer. Consequently, an automatic mistake can enter
the later in-memory fit, and a ReID exception can abort the otherwise valid AnalysisRun callback.

### Proposed vector source of truth and later-clip transfer

The redesign separates large immutable vector evidence from relational decisions:
...
When evidence for a later clip is ready, the transfer is:
...
GraphQL, and the worker does not receive database credentials. A human repair creates a new bank
snapshot revision. Later jobs consume that new snapshot, while old association runs remain exactly
reproducible from their original evidence and snapshot hashes. Manual assignments remain the effective
authority even if a later automatic run disagrees.

## External engine branch audit: VLM + pose + appearance ReID
...
## Confirmed ReID and identity risks
...
## Proposed target architecture for confirmation
...
- the current S1-S6 fixed identity rows are removed from the active model after migration. They may
  remain only in immutable legacy artifacts/audit exports.
...
“Cutover” here means that, after rollout and shadow verification, **new production writes use only the
new job architecture**. It does not mean deleting the database, losing old results, or sending a new
request shape to an old worker. During deployment, Central may temporarily understand both legacy and
new schemas, but it routes each job only to a worker that explicitly advertises support for that
`(work_kind, schema_version)`. This permits a safe staged rollout without keeping two active ReID write
paths forever.
...
8. Rename/migrate `AiJob` to the generic durable provider job record, retaining IDs, receipts, and
   historical analysis lineage. Do not delete completed legacy jobs.

The rollout order is: introduce compatible Central/schema readers, deploy capability-advertising new
workers, run shadow jobs with no active projection writes, enable new ReID writes, stop the legacy
fixed-slot ingest path, verify reconciliation/rollback export, and only then remove obsolete runtime
code. Historical AnalysisData, completed jobs, vectors, and human decisions remain viewable/exportable
under their original versions.
...
## Proposed dynamic player preview
...
## Proposed pose-first hitter association
...
### Phase 7: shadow evaluation, legacy migration, and capability-gated cutover

Migration policy:

- preserve immutable legacy AnalysisData, raw descriptors, completed jobs, dataset exports, and all
  manual Local assignments;
- import legacy automatic S1-S6 observations as `LEGACY_UNVERIFIED` evidence only, never as confirmed
  person truth;
- translate eligible manual Local assignments into confirmed membership/projection revisions with
  provenance after team/roster validation;
- do not import a slot binding across a player change as one person cluster;
- retain a read-only legacy adapter during shadow comparison, then remove current fixed-roster ingest,
  server-side in-callback fitting, and misleading UI actions in one cutover;
- avoid indefinite dual-write. New production writes target only the new model after cutover; legacy
  data remains exportable/read-only; and
- perform no destructive row/artifact deletion until counts, checksums, rollback export, and product
...
8. Accept a staged, capability-gated worker/Central cutover. In plain language: old workers receive
   only old job shapes, new workers advertise and receive the new job kinds, shadow verification runs
   before activation, and after cutover all new ReID writes use only the new architecture. Old
   AnalysisData/jobs/manual decisions stay readable and exportable; no legacy rows are destructively
   erased, and the two write paths do not run forever.
...
- ReID ingest/correction: `server/src/services/fixed-roster-reid.ts`
- Existing binding projection: `server/src/services/reid-automatic-assignment.ts`
- Identity GraphQL domain service: `server/src/services/coach-analytics.ts`
- Coupled callback activation: `server/src/routes/ai-callback.ts`
- Current persistence: `packages/db/prisma/schema.prisma`
- Provider control plane: `server/src/realtime/ai-provider-ws.ts`,
  `packages/contracts/ai/provider-realtime.schema.json`, and
  `sdk/src/volleyball_monitoring_ai/worker.py`
- Current analysis job contract/capabilities: `packages/contracts/ai/job.schema.json` and
  `packages/contracts/ai/capabilities.schema.json`
- Current engine pipeline/hitter association: `H:/Repos/volleyball-analysis-engine/src/volleyball_analysis_engine/pipeline.py`
  and `association.py`
- Current engine crop/descriptor/pose prompt path:
  `H:/Repos/volleyball-analysis-engine/src/volleyball_analysis_engine/reid_features.py`,
  `nested_reid.py`, and `inference.py`
