...
`SYSTEM_SPEC_V3_2` defines the annotation meanings and original default bindings of `Z`, `Space`, `X`, `<`, `>`, `?` and `Enter`, with arrow keys as the default frame/player controls. The product owner subsequently approved user-editable physical bindings. Command meaning remains the operator-safety contract; a remap changes only which physical key invokes it.
...
2. The seven annotation commands and media-control commands have the V3.2 keys as defaults, but users may replace those physical bindings. Remapping never changes command meaning; terminal still targets the existing previous key point and creates no timestamp.
...
- Restore All Defaults atomically restores `Z`, `Space`, `X`, `<`, `>`, `?`, `Enter` and the default arrow-key media controls.
- `X` still terminalizes the existing previous key point and creates no new timestamp.
