...
模型；只實作中央系統、影音流程、前端、外部 AI 介面與可由 GitHub 安裝的 Python
SDK。外部 AI Worker 由獨立 repository 透過既有 WebSocket contract 連入。}
\end{quote}
...
時不可另開新 rally \\
\texttt{X\ 擊球} & \texttt{X} & 在目前已呈現影片 frame 建立一般
\texttt{contact} key point & 播放器正在 seek、cursor stale 或位於 gap
時不可建立 \\
播放／暫停 & \texttt{Space} & 切換目前影片播放狀態 & 只控制播放器，不建立 key point \\
...
\item
  Control deck、設定選單與快捷鍵說明必須以 TanStack Hotkeys \texttt{formatForDisplay} 顯示目前綁定，讓 macOS 使用符號、Windows/Linux 使用平台慣用標籤；不得自行拼接鍵帽文字。底部固定顯示Z、X、\texttt{\textless{}}、\texttt{\textgreater{}}、\texttt{?}與按鍵設定；Enter保留為鍵盤command，不顯示大型提交按鈕。
\end{itemize}
...
\item
  ...
  mask，表示尚未提交且仍可修正。
\item
  \texttt{SUBMITTED}且AI尚未完成：黃色半透明 mask，表示 immutable submission
  已建立並正在裁切／排隊／處理。
\item
...
\item
  ...
  \end{itemize}
\item
  Mask 代表完整 clip：預設從 service key point 前 5 秒到 terminal key
  point 後 5 秒，並在 source 邊界或 DVR gap 依 server clip policy
  截短。Key point 本身仍只保存實際 service/contact/terminal
  時間，mask 的 padding 不建立事件或得分 frame。
\item
...
教練／裁判端採多頁資訊架構，iPad
使用精簡底部導覽；頂部工具列提供首頁、場次比分、live 與連線狀態。
...
每一個Z／X／\texttt{CLOSE\_\allowbreak{}RALLY}／move／delete／reopen／Enter
command都具有strict payload schema；close必須帶target ID與strict outcome；Space僅在本地控制播放器。 \\
...
\item
  只有教練／裁判顯示面板要求 installable iPad landscape PWA；標註端保留五個 annotation touch action 與按鍵設定，但不以 iPad PWA 作主要版面或驗收裝置。
\item
...
\tightlist
\item
  \texttt{worker-media}：YouTube／MP4 source、OME monitor、DVR index、sample
  index 與 FFmpeg。
\item
  \texttt{worker-workflow}：clip、playback cleanup、analysis convergence 與
  outbox publish。
\end{itemize}

每個 composition 內仍以獨立 loop、concurrency、metrics、backoff 與
graceful shutdown 隔離責任。它們共享 \texttt{packages/db} 與
\texttt{packages/contracts}，但每個 loop 只能 claim 自己的 job
type。FFmpeg subprocess 必須有 timeout、cancel、stderr capture、temporary
directory quota 與 idempotent output key。

外部 GPU AI Worker 透過 \texttt{/api/v1/ai/providers/ws} 主動連入
Server；Server 直接以 least-busy 規則分配 durable AI job。中央 worker
image 不包含 AI dispatcher，也不執行假 provider。現有 GraphQL、REST、Annotation
WebSocket 與 AI Worker WebSocket 介面保持不變。

  日常開發以 \texttt{bun run dev:infra} 只啟動 PostgreSQL、Redis、MinIO 與
  OvenMediaEngine；\texttt{bun run storage:bootstrap} 在本機 \texttt{ensure}
  模式只建立缺少的 bucket，在正式 \texttt{validate} 模式只驗證且不得要求 create-bucket
  權限。\texttt{bun run dev} 在 host 監督 Server、Nuxt、\texttt{worker-media} 與
  \texttt{worker-workflow}，只把 container DNS endpoint 映射成 loopback，不改
  \texttt{/graphql}、\texttt{/api}、\texttt{/ws}、\texttt{/ome} 或既有 contract。
  Compose 不保留一次性 MinIO init container。\texttt{bun run dev:https}
  才額外啟動 Traefik，將相同路徑轉送至 host process。

  兩個合併 worker 各自提供 internal readiness document。每個內部 loop 必須報告
  heartbeat、last success/error、active work、failed work 與可取得的 backlog。Media
  indexer 或 clip loop 停止時為 \texttt{unhealthy}；playback cleanup 等非核心
  maintenance loop 異常時為 \texttt{degraded}，不得因此重啟其他健康 loop。

目前 deterministic DVR profile 已是可直接組成 bounded HLS 的 immutable
fMP4。Server 在已授權 request 內建立 manifest/mapping；
\texttt{playback-packager} 非同步清除已過期的 ephemeral
\texttt{PlaybackWindow}，不複製完整 DVR、也不為每次 seek 重轉碼。AI
completed callback 必須在公開 REST boundary 完成
schema/checksum/passthrough/idempotent receipt 與 normalized transaction
後才 ACK；\texttt{analysis-ingest} 只負責 restart 後 active immutable
submission 的 terminal projection convergence，不得重啟
\texttt{SUPERSEDED} run 或重算/改寫 provider \texttt{court\_pos}。
\texttt{outbox-publisher} 以 PostgreSQL CAS claim
\texttt{OutboxEvent}，用 event ID/dedupe key idempotently 寫入 durable
pg-boss queue，成功後才標記 \texttt{PUBLISHED}。
...
\tightlist
\item
  OvenMediaEngine：接收來源、live LL-HLS/WebRTC、fMP4 recording 與基礎
  playback。
\item
...
Media & OvenMediaEngine + FFmpeg/ffprobe &
...
\texttt{worker-media} & YouTube／MP4 source、OME monitor、DVR index、sample
index、FFmpeg & local spool 只是暫存；工作狀態在 PostgreSQL \\
\texttt{worker-workflow} & clip、playback cleanup、analysis convergence、outbox
publish & 否 \\
\texttt{ovenmediaengine} & ingest/live/record & recording spool 與 DVR 暫存 \\
...
\texttt{traefik} & HTTPS／WSS／same-origin routing & 否 \\
\end{longtable}

Baseline 不建立 AI 推論容器。外部 GPU AI Worker 透過 outbound WebSocket 主動連入
Server，由中央端以 least-busy 規則分配 immutable Job；AI Worker 不屬於中央系統固定容器。
...
window邊界時，server 可在同一個 playback window
內丟棄瀏覽器已緩衝的舊 prefix 並附加連續的新 tail；重疊 suffix
必須完全相同。此時
\texttt{window\_\allowbreak{}capture\_\allowbreak{}start\_\allowbreak{}us}
可以向前推進，但
\texttt{presentation\_\allowbreak{}origin\_\allowbreak{}capture\_\allowbreak{}us}、window
ID、manifest URL 與 browser media-time mapping
必須固定。使用者在歷史回放時，live錄製與 live-edge更新持續進行。
...
\item
  ...
  segment references。
\item
  \texttt{archive}：同一個 bounded playback window 使用固定 manifest
  URL；依實際 MSE buffered edge 提前滑動 ready segment
  selection、丟棄已緩衝 prefix、append 新 tail 並提升 mapping
  revision，不得切換 \texttt{<video src>}。
\item
  capture epoch／init map 改變由同一 manifest 的
  \texttt{EXT-X-DISCONTINUITY} 表達；只有真正 capture gap 才中止連續範圍並禁止播放／標記。
\item
  hls.js 持有唯一長存 MediaSource Blob 並負責 playlist reload、segment
  prefetch/retry 與 buffer eviction；已遠離的 MediaSource buffer 與 overlay
  chunk 必須釋放。
\item
...
\item
  ...
  不可設為 browser必填欄位。
\item
  \texttt{cursor\_\allowbreak{}status=seeking/stale/gap} 時，Z/X必須被 UI阻擋；Space仍只切換播放器。
\item
...
\subsubsection{X - 擊球}\label{x---ux64caux7403}
...
\item
  廣播 submission／processing狀態；UI mask轉黃。
\end{enumerate}
...
\item
  底部固定精簡 control deck：\texttt{Z}發球、\texttt{X}擊球、\texttt{\textless{}}左側得分、\texttt{\textgreater{}}右側得分、\texttt{?}未知與按鍵設定；不得顯示獨立結束或大型提交控制。\texttt{Enter}提交與\texttt{Space}播放／暫停仍由集中式鍵盤registry提供。
\item
...
v2.1}\label{10-annotation-realtime-schema-v21}

\subsubsection{10.0 協議版本}\label{100-ux5354ux8b70ux7248ux672c}

Annotation Realtime Protocol 的正式registry版本為\texttt{2.1.0}。Canonical
Rally command／ACK仍使用\texttt{2.0.0}：這是先前移除v1.1獨立terminal與後續score兩階段command的breaking
boundary，改由一個
\texttt{CLOSE\_\allowbreak{}RALLY} atomically terminalize目前server-confirmed最後key
point並保存rally-level outcome。任何v1.x annotation message都不得被2.x
consumer靜默當成相同語意。

\texttt{2.1.0}只新增ephemeral
\texttt{soft\_\allowbreak{}lock\_\allowbreak{}intent}。它沒有\texttt{command\_\allowbreak{}id}、\texttt{rally\_\allowbreak{}id}、revision或authoritative
media anchor，不進PostgreSQL、不建立operation/receipt，也不得阻擋另一位operator送出MOVE；Redis短TTL提示失效、client
release或disconnect後即清除，canonical concurrency仍完全由Rally revision/CAS決定。
...
\begin{Highlighting}[]
\FunctionTok{\{}
  \DataTypeTok{"schema\_\allowbreak{}version"}\FunctionTok{:} \StringTok{"2.1.0"}\FunctionTok{,}
  \DataTypeTok{"type"}\FunctionTok{:} \StringTok{"soft\_\allowbreak{}lock\_\allowbreak{}intent"}\FunctionTok{,}
  \DataTypeTok{"room\_\allowbreak{}id"}\FunctionTok{:} \StringTok{"match:...:capture:..."}\FunctionTok{,}
  \DataTypeTok{"editing\_\allowbreak{}key\_\allowbreak{}point\_\allowbreak{}id"}\FunctionTok{:} \StringTok{"0190..."}
\FunctionTok{\}}
\end{Highlighting}
\end{Shaded}

\texttt{editing\_\allowbreak{}key\_\allowbreak{}point\_\allowbreak{}id:
null}表示主動釋放。Server只採用已驗證connection的user/device identity，並透過既有
\texttt{presence\_\allowbreak{}snapshot.members{[}{]}.editing\_\allowbreak{}key\_\allowbreak{}point\_\allowbreak{}id}廣播提示；client不得自報display
name或其他member identity。
...
\item
AiWorkerAccessToken／AiProviderInstance／AiJob／AiCallbackReceipt；中央系統只維護單一 \texttt{volleyball-analysis-engine} 作業佇列，不以 integration／pool 分區。
\item
...
\item
  Z/X/CLOSE\_RALLY(left/right/unknown)/reopen/move/delete/Enter；Space只控制播放／暫停。
\item
...
Exit：兩個獨立 PC browser session 同時標註；鍵盤、滑鼠與五個annotation touch controls加按鍵設定可用；refresh/reconnect後一致；close不建timestamp／score
frame；unknown可提交。Coach iPad PWA 在 Phase 5 另行驗收，不作為 annotation editor 的主要裝置。

\subsection{Phase 4 --- Clip／External AI
Worker／SDK}\label{phase-4--clipexternal-ai-workersdk}
...
Schema、validation、外部 WS worker adapter與 fixtures，不實作 AI
...
\begin{quote}
建立 TypeScript
server/worker、Prisma、PostgreSQL、MinIO、Redis、OvenMediaEngine與Compose
...
\NormalTok{{-} OvenMediaEngine + FFmpeg 完整 DVR、windowed lazy playback與 clip服務}
...
\NormalTok{{-} 外部 WS AI Worker integration fixtures}
...
\item
  OvenMediaEngine: \url{https://github.com/OvenMediaLabs/OvenMediaEngine}
\item
  OvenMediaEngine LL-HLS: \url{https://docs.ovenmediaengine.com/0.17.2/streaming/low-latency-hls}
\item
