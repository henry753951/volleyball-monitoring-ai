...
下列鍵盤行為與畫面按鈕是產品合約，不得由實作者自行改成其他語意。觸控按鈕與鍵盤必須呼叫同一個
command path。
...
\item
  ...
  時，\texttt{\textless{}}、\texttt{\textgreater{}}、\texttt{?}
  代表得分選擇。
\item
  方向鍵 \texttt{←}／\texttt{→} 永遠只代表前後一個 canonical
  frame或播放器移動，不作為得分快捷鍵。
\item
  其他狀態下方向鍵只控制播放器，不可暗中改資料。
\item
...
\item
  底部固定 control
  deck：\texttt{Z\ 發球}、\texttt{Space\ 擊球}、\texttt{X\ 結束}、\texttt{\textless{}\ 左側得分}、\texttt{\textgreater{}\ 右側得分}、\texttt{?\ 未知}、\texttt{Enter\ 提交}，並依
  state啟停。
\item
