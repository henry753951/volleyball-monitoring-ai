...
\texttt{worker-clip} & index、bounded playback-window lifecycle、clip、FFmpeg & local
spool 只是暫存 \\
\texttt{worker-ai-dispatcher} / \texttt{worker-analysis-ingest} /
\texttt{worker-outbox-publisher} &
submit／retry、callback terminal convergence、durable domain-event publish & 否 \\
...
\texttt{tracking-replay-provider}；它驗證 immutable Job 與 canonical clip，並重播已版本化的外部 tracking/analysis 輸出，不宣稱執行即時模型推論。
