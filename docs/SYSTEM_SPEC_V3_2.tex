...
時不可建立 \\
\texttt{X\ 結束} & \texttt{X} & 不建立新時間點；把使用者按 X
前所指向的既有最後 key point 標為 terminal & Command 必須帶
\texttt{target\_\allowbreak{}key\_\allowbreak{}point\_\allowbreak{}id}；若多人協作後它已不是最後一點，回傳
revision conflict \\
\texttt{\textless{}\ 左側得分} & \texttt{\textless{}} & X 後將得分解析為
\texttt{resolved/left} & 只有「等待得分」模式才代表得分；編輯 key point
時方向鍵改為逐幀 \\
\texttt{\textgreater{}\ 右側得分} & \texttt{\textgreater{}} & X
後將得分解析為 \texttt{resolved/right} & 同上 \\
\texttt{?\ 未知} & \texttt{?} & 使用者明確表示暫時無法判定得分方，設為
\texttt{unknown} & 不是 \texttt{pending}；可以提交
AI，但不納入勝負統計 \\
...
\item
  Control deck、設定選單與快捷鍵說明必須以 TanStack Hotkeys \texttt{formatForDisplay} 顯示目前綁定，讓 macOS 使用符號、Windows/Linux 使用平台慣用標籤；不得自行拼接鍵帽文字。七個觸控動作不可被移除，且不受鍵盤自訂影響。
\end{itemize}
...
\item
  \texttt{AWAITING\_\allowbreak{}SCORE}
  時，左側得分、右側得分、未知等 command 依目前綁定觸發；預設為
...
\tightlist
\item
  \texttt{OPEN}、\texttt{AWAITING\_\allowbreak{}SCORE}、\texttt{READY}：灰色半透明
  mask，表示尚未提交且仍可修正。
\item
...
不可提交 \\
High & X 建立另一個 rally end timestamp &
與使用者定義衝突，也會多一個虛假事件 & X 只標記前一個 key point 為
terminal，X 按下時間只可做 audit，不進 event timeline \\
High & 沒有 reopen／改 terminal 的流程 & X 誤按後只能刪整個 rally &
Draft 提供
\texttt{REOPEN\_\allowbreak{}RALLY}、\texttt{SET\_\allowbreak{}TERMINAL\_\allowbreak{}KEY\_\allowbreak{}POINT}；submitted
必須建立 correction draft \\
...
每一個Z／Space／X／score／move／delete／reopen／Enter
command都具有strict payload schema；X必須帶target ID。 \\
...
\item
  Vant 4 用於 iPad/mobile interaction；Tailwind 4 建立 design
...
PWA & \texttt{@vite-pwa/nuxt} & standalone manifest、service
...
\texttt{/graphql}、\texttt{/api/v1}、\texttt{/ws}、\texttt{/hls}，避免把\texttt{localhost}編入iPad
bundle。MinIO credentials、AI provider bearer與callback
token不可進前端。

\subsection{4.7 iPad PWA
...
\begin{Highlighting}[]
\NormalTok{OPEN}
\NormalTok{  └─ X(target\_\allowbreak{}key\_\allowbreak{}point\_\allowbreak{}id) → AWAITING\_\allowbreak{}SCORE}
\NormalTok{AWAITING\_\allowbreak{}SCORE}
\NormalTok{  ├─ \textless{} left / \textgreater{} right → READY(resolved)}
\NormalTok{  ├─ ? → READY(unknown)}
\NormalTok{  └─ reopen → OPEN}
\NormalTok{READY}
\NormalTok{  ├─ 改得分 → READY}
\NormalTok{  ├─ reopen → OPEN}
...
\subsubsection{X - 結束}\label{x---ux7d50ux675f}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Client從目前server-confirmed snapshot取得最後一個有效 key point
  ID，凍結為
  \texttt{target\_\allowbreak{}key\_\allowbreak{}point\_\allowbreak{}id}；一般流程不要求使用者先手動選取marker。
\item
  X按下時的 wall clock、player currentTime都不新增到事件序列。
\item
  Server驗證 target在 \texttt{base\_\allowbreak{}revision}仍是最後一個未刪除 key
  point。
\item
  成功後把該點 \texttt{is\_\allowbreak{}terminal=true}，狀態進入
  \texttt{AWAITING\_\allowbreak{}SCORE}，mask終點就是該點。
\item
  若另一協作者已新增點，回
  \texttt{REVISION\_\allowbreak{}CONFLICT}；前端刷新後讓使用者再決定，不自動把新點terminal。
\end{enumerate}

\subsubsection{\texorpdfstring{\texttt{\textless{}}、\texttt{\textgreater{}}、\texttt{?}
- 得分}{\textless、\textgreater、? - 得分}}\label{---ux5f97ux5206}

\begin{itemize}
\tightlist
\item
  UI必須顯示三個按鈕：\texttt{\textless{}\ 左側得分}、\texttt{\textgreater{}\ 右側得分}、\texttt{?\ 未知}。硬體鍵盤使用實際
  \texttt{\textless{}}、\texttt{\textgreater{}}、\texttt{?}（通常為
  Shift+逗號、Shift+句點、Shift+斜線）；方向鍵保留給逐幀。
\item
  \texttt{left/right} 指當下球場畫面左右側，不是永久 team identity。
\item
  Server依 submission的 side-assignment snapshot解析 team ID。
\item
  \texttt{pending} 與 \texttt{unknown}
  不同：pending代表尚未選；unknown代表使用者已明確選 \texttt{?}。
\end{itemize}
...
\item
  底部固定 control deck：發球、擊球、結束、左側得分、右側得分、未知、提交七個觸控動作，顯示目前快捷鍵（預設
  \texttt{Z}、\texttt{Space}、\texttt{X}、\texttt{\textless{}}、\texttt{\textgreater{}}、\texttt{?}、\texttt{Enter}）並依 state啟停。
\item
...
v1.1}\label{10-annotation-realtime-schema-v11}

\subsubsection{10.0 協議版本}\label{100-ux5354ux8b70ux7248ux672c}

Annotation Realtime Protocol 的正式版本為 \texttt{1.1.0}。這次升版反映
command identity 改為 connection-bound、\texttt{MARK\_\allowbreak{}TERMINAL} 必須指定
\texttt{target\_\allowbreak{}key\_\allowbreak{}point\_\allowbreak{}id}、以及 ACK/effect/snapshot message 改為
strict typed variants。任何 \texttt{1.0.0} annotation message 都不得被
1.1 consumer 靜默當成相同語意。
...
\FunctionTok{\{}
  ...
  \DataTypeTok{"kind"}\FunctionTok{:} \StringTok{"MARK\_\allowbreak{}TERMINAL"}\FunctionTok{,}
  \DataTypeTok{"payload"}\FunctionTok{:} \FunctionTok{\{}
    \DataTypeTok{"target\_\allowbreak{}key\_\allowbreak{}point\_\allowbreak{}id"}\FunctionTok{:} \StringTok{"0190..."}
  \FunctionTok{\}}
\FunctionTok{\}}
\end{Highlighting}
\end{Shaded}
...
Space \\
\texttt{MARK\_\allowbreak{}TERMINAL} & \texttt{target\_\allowbreak{}key\_\allowbreak{}point\_\allowbreak{}id} &
X；不帶新時間 \\
\texttt{SET\_\allowbreak{}SCORE} & \texttt{score\_\allowbreak{}resolution},
\texttt{scoring\_\allowbreak{}court\_\allowbreak{}side} &
\texttt{\textless{}}／\texttt{\textgreater{}}／\texttt{?}；方向鍵不計分 \\
...
\texttt{SET\_\allowbreak{}TERMINAL\_\allowbreak{}KEY\_\allowbreak{}POINT} & \texttt{target\_\allowbreak{}key\_\allowbreak{}point\_\allowbreak{}id} &
修正terminal \\
\texttt{REOPEN\_\allowbreak{}RALLY} & none & 清除terminal、回OPEN \\
...
\FunctionTok{\{}
  ...
  \DataTypeTok{"room\_\allowbreak{}id"}\FunctionTok{:} \StringTok{"match:...:capture:..."}\FunctionTok{,}
  \DataTypeTok{"rally\_\allowbreak{}id"}\FunctionTok{:} \StringTok{"0190..."}\FunctionTok{,}
  \DataTypeTok{"operation\_\allowbreak{}kind"}\FunctionTok{:} \StringTok{"MARK\_\allowbreak{}TERMINAL"}\FunctionTok{,}
  ...
  \DataTypeTok{"effects"}\FunctionTok{:} \FunctionTok{\{}
    ...
    \DataTypeTok{"annotation\_\allowbreak{}status"}\FunctionTok{:} \StringTok{"awaiting\_\allowbreak{}score"}\FunctionTok{,}
    \DataTypeTok{"score\_\allowbreak{}resolution"}\FunctionTok{:} \StringTok{"pending"}\FunctionTok{,}
    \DataTypeTok{"scoring\_\allowbreak{}court\_\allowbreak{}side"}\FunctionTok{:} \KeywordTok{null}
  \FunctionTok{\},}
...
同一 \texttt{command\_\allowbreak{}id} 重送必須回相同結果。Server先寫 DB
transaction/outbox，再廣播；WebSocket訊息不是唯一 durable record。
...
\FunctionTok{\{}
  ...
  \DataTypeTok{"command\_\allowbreak{}id"}\FunctionTok{:} \StringTok{"0190..."}\FunctionTok{,}
  \DataTypeTok{"code"}\FunctionTok{:} \StringTok{"REVISION\_\allowbreak{}CONFLICT"}\FunctionTok{,}
...
\texttt{MARK\_\allowbreak{}TERMINAL\_\allowbreak{}TARGET\_\allowbreak{}NOT\_\allowbreak{}LAST}、\texttt{CURSOR\_\allowbreak{}STALE}、\texttt{CURSOR\_\allowbreak{}IN\_\allowbreak{}GAP}、\texttt{RALLY\_\allowbreak{}NOT\_\allowbreak{}OPEN}、\texttt{SCORE\_\allowbreak{}PENDING}等均用明確
domain code。Reconnect時若 server
revision大於client最後revision且delta不完整，必須 GraphQL refetch
snapshot。
...
Exit：兩個 iPad同時標；refresh/reconnect後一致；X不建
timestamp；unknown可提交。
...
\begin{quote}
建立 Nuxt 4多頁 shell、底部導覽、頂部狀態列、annotation/coach/history
route與 fixture-driven元件。只參考舊 MVP UI libraries/framework，不沿用
domain flow/schema/statistics。
\end{quote}
...
\NormalTok{{-} X只將上一個既有key point標terminal，不建立時間點。}
