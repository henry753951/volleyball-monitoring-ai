...
Open limitations: concrete Prisma/pg-boss ingest persistence, server MinIO/sample-index adapters, production cookie identity, resolve-cursor/frame-step, real PWA player/timeline integration, deterministic Docker discontinuity/restart smoke and the two-hour bounded-memory soak remain active. No Rally, KeyPoint, AnnotationOperation, RallySubmission, ClipJob or AiJob write path was added.
