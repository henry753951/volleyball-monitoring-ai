...
| Full-session DVR | OpenAPI 1.2.0, strict media/error schemas, TS/Python SDK, playback-window migration and internal sample-index v1 active | GraphQL timeline + exact bigint sample/epoch/ffprobe/artifact kernels + injected-port create/get/HLS HTTP verified; concrete ingest/server adapters and resolve/step active | canonical REST adapter, bigint timeline/gap helpers and bounded 3-window cache verified; real player pending | 6 contracts + 2 DB + 55 media + 34 server + 63 worker + 43 Web + 12 SDK; required feature CI green | persistence, authority kernels and bounded HTTP verified | concrete Prisma/MinIO/pg-boss + production auth/sample resolver + resolve/step + PWA player + Docker discontinuity/restart smoke + 2h arbitrary lazy seek/bounded memory |
