...
| Full-session DVR | OpenAPI 1.2.0, strict media/error schemas, TS parsers and Python SDK; playback-window DB active | exact bigint sample resolver kernel verified; persisted ingest/REST active | canonical REST adapter, bigint timeline/gap helpers and bounded 3-window cache verified; real player active | 6 contracts + 12 SDK + 17 worker + 43 Web; required CI green | authority kernel verified | Docker discontinuity/restart smoke + real authenticated HLS + 2h arbitrary lazy seek/bounded memory |
