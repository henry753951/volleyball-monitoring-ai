...
function analysisData(): AnalysisData {
  return {
    ...
    playerFlags: [1, 0],
    playerConfidences: [200, 255],
    actionTaxonomyId: '',
    actionTaxonomyVersion: '',
    actionLabels: [],
    actionLabelIds: [65_535, 65_535],
    actionConfidences: [255, 255],
    ballFramePositions: [
      { x: 10, y: 20 },
      { x: 0, y: 0 },
      { x: 30, y: 40 },
    ...
    ballFlags: [1, 0, 1],
    ballConfidences: [220, 0, 210],
    courtKeypointFrameOffsets: [0, 2, 2, 3],
    courtKeypointIds: [0, 11, 35],
    courtKeypointPositions: [
      { x: 100, y: 200 },
      { x: 300, y: 400 },
      { x: 500, y: 600 },
    ],
    courtKeypointConfidences: [230, 210, 190],
    domainJson: '{}',
    inputClipSha256: 'a'.repeat(64),
    producerName: 'test',
    producerBuildId: 'test-build',
    producerSdkVersion: 'test-sdk',
...
describe('AnalysisData FlatBuffers boundary', () => {
  it('splits by frame and round-trips strict VFC1 columns without clamping court coordinates', () => {
    ...
    expect(chunks[0]).toMatchObject({
      ...
      frameOffsets: [0, 1, 1],
      trackIds: [7],
      courtPositions: [{ x: -0.2, y: 1.3 }],
      courtKeypointFrameOffsets: [0, 2, 2],
      courtKeypointIds: [0, 11],
    })
