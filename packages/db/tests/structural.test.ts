...
const migration = readFileSync(
  resolve(root, 'prisma/migrations/20260807120000_playback_windows/migration.sql'),
  'utf8',
)
const annotationMigration = readFileSync(
  resolve(root, 'prisma/migrations/20260807180000_annotation_command_receipts/migration.sql'),
  'utf8',
)
...
describe('playback window persistence contract', () => {
  it('declares enum, models, relations and exact scalar choices', () => {
    ...
    expect(schema).toMatch(
      /model PlaybackWindow \{[\s\S]*mappingVersion\s+Int[\s\S]*timelineVersion\s+BigInt[\s\S]*captureStartUs\s+BigInt[\s\S]*targetPlayerMediaTimeUs\s+BigInt/,
    )
    expect(schema).toMatch(
  ...
  it('contains every named migration check constraint', () => {
    for (const name of [
      'DvrSegment_capture_range_check',
      'DvrSegment_nonnegative_check',
      'PlaybackWindow_capture_range_check',
      'PlaybackWindow_origin_check',
      'PlaybackWindow_target_check',
      'PlaybackWindow_mapping_check',
      'PlaybackWindow_target_within_mapping_check',
      'PlaybackWindowSegment_sequence_check',
    ])
...
describe('durable annotation receipt contract', () => {
  it('keeps server sequence independent from rally revision and retains exact request/response JSON', () => {
    expect(schema).toMatch(
      /model AnnotationCommandReceipt \{[\s\S]*serverSequence\s+BigInt\s+@id @default\(autoincrement\(\)\)[\s\S]*commandId\s+String\s+@unique[\s\S]*rallyId\s+String\s+@db\.Uuid[\s\S]*requestHash\s+String[\s\S]*requestJson\s+Json[\s\S]*accepted\s+Boolean[\s\S]*responseJson\s+Json/,
    )
    expect(schema).toMatch(
      /model AnnotationOperation \{[\s\S]*receiptServerSequence\s+BigInt\?\s+@unique[\s\S]*receipt\s+AnnotationCommandReceipt\?/,
    )
