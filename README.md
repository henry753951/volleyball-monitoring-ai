...
`Z` marks only the rally segment boundary: the first press starts the segment and the second press ends it. Neither boundary claims a serve, contact, landing or scoring event. `X` adds an optional contact marker; AI-detected contacts can be reviewed and corrected later. The outcome defaults to unknown and may be changed independently before or after submission through the correction workflow. `Space` plays/pauses; `<`, `>` and `?` select left, right or unknown outcome; `Enter` submits. The comma, period and slash physical keys trigger `<`, `>` and `?` without Shift; `Ctrl+←/→` moves five frames. Touch controls expose the same annotation actions; physical bindings remain configurable and Restore Defaults returns to these keys.
