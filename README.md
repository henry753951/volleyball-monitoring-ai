...
`Z` starts a rally at service; while that rally is open, pressing `Z` again creates the authoritative terminal contact at the current cursor and marks the outcome unknown so `Enter` can submit immediately. `X` remains an optional manual contact/correction marker. `Space` plays/pauses; `<`, `>` and `?` select left, right or unknown outcome, including after the second `Z`; `Enter` submits. The comma, period and slash physical keys trigger `<`, `>` and `?` without Shift; `Ctrl+←/→` moves five frames. Touch controls expose the same annotation actions; physical bindings remain configurable and Restore Defaults returns to these keys.
