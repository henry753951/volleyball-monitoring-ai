...
class AIJobRequest(StrictModel):
    ...
    def validate_key_points(self) -> AIJobRequest:
        ...
        if (
            ...
            or boundary_times != sorted(boundary_times)
            or boundary_pts != sorted(boundary_pts)
        ):
            raise ValueError("rally boundaries must be monotonic in clip frame/time/PTS")
        if boundary_frames[-1] >= int(self.clip.video.total_frames) or boundary_times[-1] > int(
            self.clip.video.duration_us
        ):
...
class FixedRosterTracklet(StrictModel):
    ...
    def validate_track_evidence(self) -> FixedRosterTracklet:
        ...
        if self.canonical_track_id not in self.track_ids or len(self.track_ids) != len(
            set(self.track_ids)
        ):
            raise ValueError("ReID aliases must uniquely contain the canonical TID")
        if len(self.cannot_link_canonical_track_ids) != len(
            set(self.cannot_link_canonical_track_ids)
        ):
        ...
        for tracklet in self.tracklets:
            for linked in tracklet.cannot_link_canonical_track_ids:
                other = next(
                    (
                        candidate
                        for candidate in self.tracklets
                        if candidate.canonical_track_id == linked
                    ),
                    None,
                )
                if (
                    other is None
                    or tracklet.canonical_track_id not in other.cannot_link_canonical_track_ids
                ):
...
class ContactEvent(StrictModel):
    ...
    association_state: Literal[
        "resolved_single", "resolved_multiple", "ambiguous", "unresolved", "no_player"
    ]
    actors: list[Actor]
...
class AnalysisDomainData(StrictModel):
    ...
    def validate_counts_and_refs(self) -> AnalysisDomainData:
        ...
        if [event.sequence_index for event in self.contact_events] != list(
            range(len(self.contact_events))
        ):
            raise ValueError("contact event sequence_index must be contiguous from zero")
        if [segment.sequence_index for segment in self.path_segments] != list(
            range(len(self.path_segments))
        ):
        ...
        for event in self.contact_events:
            ...
            if any(
                position.track_id is not None and position.track_id not in track_ids
                for position in event.representative_court_positions
            ):
                raise ValueError("representative position references unknown track")
        unresolved = sum(
            event.association_state in {"ambiguous", "unresolved"} for event in self.contact_events
        )
        multiple = sum(
            event.association_state == "resolved_multiple" for event in self.contact_events
        )
        if unresolved != self.summary.unresolved_event_count:
            raise ValueError("summary unresolved_event_count mismatch")
        if (
            self.summary.multiple_event_count is not None
            and multiple != self.summary.multiple_event_count
        ):
        ...
        for index, segment in enumerate(self.path_segments):
            ...
            if (
                segment.start_key_point_id != start.key_point_id
                or segment.end_key_point_id != end.key_point_id
            ):
                raise ValueError("path segment must reference adjacent contact events")
            if (
                segment.start_court_positions != start.representative_court_positions
                or segment.end_court_positions != end.representative_court_positions
            ):
                raise ValueError(
                    "path segment A/B positions must equal referenced contact event positions"
                )
...
class ProviderCapabilities(StrictModel):
    ...
    supported_analysis_modules: list[Literal["court", "tracking", "reid", "contacts"]] = Field(
        min_length=1
    )
