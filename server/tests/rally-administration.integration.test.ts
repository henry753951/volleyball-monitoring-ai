...
describe('rally administration', () => {
  it('updates display placement without changing immutable submission data', async () => {
    const result = await updateRallyDisplayPlacement(
      { id: ids.actor, role: UserRole.OPERATOR },
      { ordinal: 3, rallyId: ids.rally, setNumber: 2 },
      { database: db },
    )
    expect(result).toMatchObject({ displayOrdinal: 3, displaySetNumber: 2 })
