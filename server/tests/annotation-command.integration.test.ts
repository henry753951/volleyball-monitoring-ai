...
function contactCommand(commandId: string, rallyId: string, baseRevision = '1') {
  return {
    ...
    kind: 'CREATE_CONTACT_KEY_POINT' as const,
  }
}
...
function closeCommand(
  ...
  baseRevision: string,
  outcome: 'left' | 'right' | 'unknown' = 'unknown',
) {
  return (
    outcome === 'unknown'
      ? {
          ...serviceCommand(commandId, rallyId),
          base_revision: baseRevision,
          kind: 'CLOSE_RALLY' as const,
          payload: {
            target_key_point_id: targetKeyPointId,
      ...
      : {
          ...
          payload: {
            target_key_point_id: targetKeyPointId,
            score_resolution: 'resolved' as const,
            scoring_court_side: outcome,
          },
        }
...
beforeAll(async () => {
  ...
  await db.deviceSession.createMany({
    data: [
  ...
  await db.matchSet.create({
    data: { id: ids.plannedSet, matchId: ids.match, setNumber: 2, status: 'PLANNED' },
  })
  ...
  await db.captureSession.create({
    data: {
      health: 'HEALTHY',
      id: ids.foreignCapture,
      ingestPath: '/phase3a-foreign',
      matchId: ids.match,
      sourceKind: 'fixture',
      status: 'LIVE',
    },
  ...
  await db.captureEpoch.create({
    data: {
      captureFrameOrigin: 0n,
      captureSessionId: ids.foreignCapture,
      captureTimeOriginUs: 0n,
      id: ids.foreignEpoch,
      sequenceIndex: 0,
      sourcePtsOrigin: 0n,
  ...
  await db.dvrSegment.createMany({
    data: [
      {
        captureEndUs: anchorTime + 100n,
        captureEpochId: ids.epoch,
        captureStartUs: anchorTime - 100n,
        discontinuitySequence: 0,
        dvrProgramId: ids.program,
        durationUs: 200n,
        firstFrameIndex: anchorFrame - 2n,
        frameCount: 5n,
        id: ids.segment,
        readyAt: new Date(),
        sampleIndexAssetId: ids.sampleAsset,
        sequenceNumber: 0n,
      },
      {
        captureEndUs: anchorTime + 100n,
        ...
        captureStartUs: anchorTime - 100n,
        discontinuitySequence: 0,
        dvrProgramId: ids.newerProgram,
        durationUs: 200n,
        firstFrameIndex: anchorFrame - 2n,
        frameCount: 5n,
        id: ids.foreignProgramSegment,
        readyAt: new Date(),
        sampleIndexAssetId: ids.sampleAsset,
        sequenceNumber: 0n,
      },
      {
        captureEndUs: anchorTime + 100n,
        captureEpochId: ids.foreignEpoch,
        captureStartUs: anchorTime - 100n,
        discontinuitySequence: 0,
        dvrProgramId: ids.program,
        durationUs: 200n,
  ...
  await db.playbackWindow.create({
    data: {
      captureEndUs: anchorTime + 1_000n,
      captureSessionId: ids.capture,
      captureStartUs: anchorTime - 2_000n,
      ...
      mappingVersion: 1,
      mode: 'ARCHIVE',
      presentationOriginCaptureUs: anchorTime - 1_234n,
      targetPlayerMediaTimeUs: 1_234n,
      timelineVersion: 1n,
      segments: {
        create: [
...
describe('durable service annotation command', () => {
  ...
  it('uses Z as start/end boundaries, allows zero contacts and submits with a pending score', async () => {
    ...
    const boundaryService = createAnnotationCommandService({
      ...
      resolveCursor: async cursor =>
        cursor.player_media_time_us === '1234' ? startAnchor : endAnchor,
    })
    ...
    await expect(
      db.rally.findUniqueOrThrow({ where: { id: laterRallyId } }),
    ).resolves.toMatchObject({ displayOrdinal: 1 })
    await expect(
      boundaryService.apply(
    ...
    const submit = parseAnnotationCommand({
      ...submitCommand(randomUUID(), rallyId, '2'),
      schema_version: '3.0.0',
    })
    ...
    const persisted = await db.rally.findUniqueOrThrow({
      ...
      include: {
        boundaries: true,
        keyPoints: true,
        activeSubmission: { include: { boundaries: true, keyPoints: true, clipJobs: true } },
      },
    })
    ...
    expect(persisted.activeSubmission?.boundaries.map(boundary => boundary.kind).sort()).toEqual([
      'END',
      'START',
    ])
  ...
  it('automatically tombstones an X outside the Z boundary interval and keeps clip coverage on the boundaries', async () => {
    ...
    const contact = parseAnnotationCommand({
      ...contactCommand(randomUUID(), rallyId, '1'),
      schema_version: '3.0.0',
      payload: {
        playback_cursor: {
          ...contactCommand(randomUUID(), rallyId, '1').payload.playback_cursor,
          player_media_time_us: '1184',
        },
      },
    })
    await expect(boundaryService.apply(contact, identity)).resolves.toMatchObject({
      schema_version: '3.0.0',
      type: 'command_ack',
      result_revision: '2',
    })
    await boundaryService.apply(
      boundaryCommand(randomUUID(), rallyId, 'END_RALLY', '2', '1284'),
      identity,
    )
    await boundaryService.apply(
      parseAnnotationCommand({
        ...submitCommand(randomUUID(), rallyId, '3'),
        schema_version: '3.0.0',
      }),
      identity,
    )
  ...
  it('keeps X, move and delete editable after END while preserving READY', async () => {
    ...
    const contact = await boundaryService.apply(
      parseAnnotationCommand({
        ...contactCommand(randomUUID(), rallyId, '2'),
        schema_version: '3.0.0',
        payload: {
          playback_cursor: {
    ...
    expect(contact).toMatchObject({
      type: 'command_ack',
      result_revision: '3',
      effects: { annotation_status: 'ready' },
    })
    ...
    const move = parseAnnotationCommand({
      ...
      payload: {
        ...
        playback_cursor: serviceCommand(randomUUID(), rallyId).payload.playback_cursor,
      },
    })
    await expect(boundaryService.apply(move, identity)).resolves.toMatchObject({
      type: 'command_ack',
      result_revision: '4',
      effects: { annotation_status: 'ready' },
    })
    ...
    await expect(boundaryService.apply(remove, identity)).resolves.toMatchObject({
      type: 'command_ack',
      result_revision: '5',
      effects: { annotation_status: 'ready' },
    })
  ...
  it('rejects a new draft when its START overlaps the same device READY draft', async () => {
    ...
    await boundaryService.apply(
      boundaryCommand(randomUUID(), firstRallyId, 'START_RALLY', '0', '1234'),
      identity,
    )
    await boundaryService.apply(
      boundaryCommand(randomUUID(), firstRallyId, 'END_RALLY', '1', '1284'),
      identity,
    )
  ...
  it('rejects END when its padded range overlaps another editable draft', async () => {
    ...
    const otherOrdinal =
      ((await db.rally.aggregate({ _max: { ordinal: true }, where: { setId: ids.set } }))._max
        .ordinal ?? 0) + 1
    await db.rally.create({
      data: {
        id: otherRallyId,
      ...
      data: [
        {
          ...
          sourcePts: BigInt(anchor.source_pts) + 1n,
          timingPrecision: 'FRAME_EXACT',
          updatedByUserId: ids.operator,
        },
        {
          ...
          originalPlaybackCursor: {},
          rallyId: otherRallyId,
          sourcePts: BigInt(anchor.source_pts) + 2n,
          timingPrecision: 'FRAME_EXACT',
          updatedByUserId: ids.operator,
        },
      ],
    ...
    await expect(
      boundaryService.apply(
        boundaryCommand(randomUUID(), currentRallyId, 'START_RALLY', '0', '1234'),
        identity,
      ),
    ).resolves.toMatchObject({
      type: 'command_ack',
    ...
    await expect(
      boundaryService.apply(
        boundaryCommand(randomUUID(), currentRallyId, 'END_RALLY', '1', '1284'),
        identity,
      ),
    ).resolves.toMatchObject({
    ...
    await expect(
      db.rally.findUniqueOrThrow({ where: { id: currentRallyId }, include: { boundaries: true } }),
    ).resolves.toMatchObject({
      annotationStatus: 'OPEN',
      annotationRevision: 1n,
      boundaries: [expect.objectContaining({ kind: 'START' })],
    })
    await expect(
      db.rally.findUniqueOrThrow({ where: { id: otherRallyId } }),
    ).resolves.toMatchObject({ annotationStatus: 'READY', annotationRevision: 1n })
  })
  ...
  it('atomically creates revision one, seq-zero service, receipt, operation and outbox only', async () => {
    ...
    const receipt = await db.annotationCommandReceipt.findUnique({
    ...
    expect(receipt?.serverSequence.toString()).toBe(
      response.type === 'command_ack' ? response.server_sequence : '',
    )
    await expect(
      db.rally.findUnique({
        include: { keyPoints: true, operations: true },
      ...
      setId: ids.set,
      keyPoints: [
        expect.objectContaining({ isTerminal: false, markerKind: 'SERVICE', sequenceIndex: 0 }),
      ],
      operations: [expect.objectContaining({ receiptServerSequence: receipt?.serverSequence })],
    })
    await expect(db.rallySubmission.count({ where: { rallyId: command.rally_id } })).resolves.toBe(
      0,
    )
    await expect(db.pointAward.count()).resolves.toBe(0)
    await expect(db.outboxEvent.count({ where: { aggregateId: command.rally_id } })).resolves.toBe(
      1,
    )
  })
  ...
  it('replays the exact stored response and rejects command-id hash mismatch without mutation', async () => {
    ...
    await expect(service.apply(mismatch, identity)).resolves.toMatchObject({
      type: 'command_rejected',
      code: 'COMMAND_ID_REUSED',
    })
    await expect(
      db.annotationCommandReceipt.count({ where: { commandId: command.command_id } }),
    ).resolves.toBe(1)
  ...
  it('serializes competing service starts and leaves one complete rally only', async () => {
    ...
    expect(
      responses.filter(
        value => value.type === 'command_rejected' && value.code === 'REVISION_CONFLICT',
      ),
    ).toHaveLength(1)
  ...
  it('durably rejects a sequential overlapping draft in the same set', async () => {
    ...
    await expect(
      db.annotationCommandReceipt.findUnique({ where: { commandId: second.command_id } }),
  ...
  it('rejects overlapping OPEN drafts across device sessions', async () => {
    ...
    const second = serviceCommand(randomUUID(), randomUUID())
    await expect(service.apply(first, identity)).resolves.toMatchObject({ type: 'command_ack' })
    await expect(service.apply(second, secondIdentity)).resolves.toMatchObject({
      type: 'command_rejected',
      code: 'RALLY_RANGE_RESERVED',
    })
    await expect(
    ...
    ).resolves.toEqual([{ id: first.rally_id, annotationStatus: 'OPEN' }])
  ...
  it('allows disjoint OPEN drafts across device sessions', async () => {
    const laterAnchor = {
      ...anchor,
      capture_frame_index: (BigInt(anchor.capture_frame_index) + 1n).toString(),
      capture_time_us: (BigInt(anchor.capture_time_us) + 50n).toString(),
    ...
    const boundaryService = createAnnotationCommandService({
      database: db,
      resolveCursor: async cursor =>
        cursor.player_media_time_us === '1284' ? laterAnchor : anchor,
    })
    const firstRallyId = randomUUID()
    const secondRallyId = randomUUID()
    ...
    await expect(
      boundaryService.apply(
        boundaryCommand(randomUUID(), secondRallyId, 'START_RALLY', '0', '1284'),
        secondIdentity,
      ),
    ).resolves.toMatchObject({ type: 'command_ack', operation_kind: 'START_RALLY' })
    await expect(
      db.rally.count({ where: { id: { in: [firstRallyId, secondRallyId] } } }),
    ).resolves.toBe(2)
  })

  it('serializes concurrent overlapping starts under the set allocation lock', async () => {
    const first = serviceCommand(randomUUID(), randomUUID())
    const second = serviceCommand(randomUUID(), randomUUID())
    const responses = await Promise.all([
      service.apply(first, identity),
      service.apply(second, secondIdentity),
    ])
    ...
    await expect(
      db.annotationCommandReceipt.count({
        where: { commandId: { in: [first.command_id, second.command_id] } },
      }),
    ).resolves.toBe(2)
  })
  ...
  it('allows a later non-overlapping rally while an earlier draft is READY', async () => {
    ...
    await expect(
      service.apply(serviceCommand(randomUUID(), readyRallyId), identity),
    ).resolves.toMatchObject({ type: 'command_ack' })
    ...
    await expect(
      service.apply(closeCommand(randomUUID(), readyRallyId, terminalId!, '2'), identity),
    ).resolves.toMatchObject({
      type: 'command_ack',
      effects: { annotation_status: 'ready' },
    })
    ...
    const laterAnchor = {
      ...
      resolved_player_media_time_us: (
        BigInt(anchor.resolved_player_media_time_us) + 10_000_000n
      ).toString(),
    }
    await Promise.all([
      db.playbackWindow.update({
        data: { captureEndUs: BigInt(laterAnchor.capture_time_us) + 1n },
        where: { id: ids.window },
      }),
      db.dvrSegment.update({
        data: { captureEndUs: BigInt(laterAnchor.capture_time_us) + 1n, frameCount: 303n },
        where: { id: ids.segment },
      }),
    ])
    const laterService = createAnnotationCommandService({
      database: db,
  ...
  it('durably rejects foreign rooms and cursor-not-ready without partial domain rows', async () => {
    ...
    await expect(service.apply(foreign, identity)).resolves.toMatchObject({
      code: 'ROOM_NOT_FOUND',
    })
    ...
    await expect(service.apply(notReady, identity)).resolves.toMatchObject({
      code: 'CURSOR_NOT_READY',
    })
    cursorFailure = null
    for (const command of [foreign, notReady]) {
      await expect(
        db.annotationCommandReceipt.findUnique({ where: { commandId: command.command_id } }),
      ).resolves.toMatchObject({ accepted: false })
  ...
  it('parses but durably rejects orphaned edit commands and stale service revisions', async () => {
    const orphanedEdit = parseAnnotationCommand({
      ...
      kind: 'MOVE_KEY_POINT',
      payload: {
        key_point_id: randomUUID(),
        playback_cursor: serviceCommand(randomUUID(), randomUUID()).payload.playback_cursor,
      },
    })
    await expect(service.apply(orphanedEdit, identity)).resolves.toMatchObject({
  ...
  it('durably rejects invalid authoritative anchor state without partial domain rows', async () => {
    ...
    const command = serviceCommand(randomUUID(), randomUUID())
    await expect(invalidAnchorService.apply(command, identity)).resolves.toMatchObject({
    ...
    await expect(db.rally.findUnique({ where: { id: command.rally_id } })).resolves.toBeNull()
    await expect(
      db.annotationOperation.findUnique({ where: { clientMutationId: command.command_id } }),
    ).resolves.toBeNull()
  })
  ...
  it('rejects nonexistent, foreign-program, foreign-epoch and removed playback mappings', async () => {
    ...
    for (const invalidAnchor of variants) {
      const invalidService = createAnnotationCommandService({
        database: db,
        resolveCursor: async () => invalidAnchor,
      })
      const command = serviceCommand(randomUUID(), randomUUID())
    ...
    await db.playbackWindowSegment.delete({
      where: {
        playbackWindowId_dvrSegmentId: { dvrSegmentId: ids.segment, playbackWindowId: ids.window },
      },
    })
    ...
    try {
      ...
      await expect(
        db.annotationOperation.findUnique({ where: { clientMutationId: removed.command_id } }),
      ).resolves.toBeNull()
    } finally {
  ...
  it('rolls back rally, key point and receipt when a late operation write fails', async () => {
    ...
    await db.annotationOperation.create({
      data: {
        baseRevision: 0n,
  ...
  it('creates contacts, preserves anchors, and replays accepted/rejected responses byte-for-byte', async () => {
    ...
    expect(JSON.stringify(replay)).toBe(
      JSON.stringify(parseAnnotationCommandResponse(stored.responseJson)),
    )
    const mismatch = structuredClone(command)
    mismatch.payload.playback_cursor.player_media_time_us = '9999'
    expect(await service.apply(mismatch, identity)).toMatchObject({
      type: 'command_rejected',
      code: 'COMMAND_ID_REUSED',
    })
  ...
  it('moves, deletes, reopens and voids only the mutable Rally draft', async () => {
    ...
    await expect(service.apply(remove, identity)).resolves.toMatchObject({
      type: 'command_ack',
      operation_kind: 'DELETE_KEY_POINT',
      result_revision: '4',
      effects: { deleted_key_point_id: keyPointId },
    })
    await expect(
      db.keyPoint.findUniqueOrThrow({ where: { id: keyPointId! } }),
    ...
    const terminalId =
      nextContact.type === 'command_ack' ? nextContact.effects.created_key_point_id : null
    await expect(
      service.apply(closeCommand(randomUUID(), rallyId, terminalId!, '5', 'left'), identity),
    ).resolves.toMatchObject({ type: 'command_ack', result_revision: '6' })
    ...
    await expect(service.apply(reopen, identity)).resolves.toMatchObject({
      type: 'command_ack',
      operation_kind: 'REOPEN_RALLY',
      result_revision: '7',
      effects: { annotation_status: 'open', score_resolution: 'pending' },
    })
    await expect(
      db.keyPoint.findUniqueOrThrow({ where: { id: terminalId! } }),
    ).resolves.toMatchObject({ isTerminal: false })

    const voidCommand = parseAnnotationCommand({
      ...serviceCommand(randomUUID(), rallyId),
      base_revision: '7',
      kind: 'VOID_RALLY',
      payload: { reason: 'fixture_cleanup' },
    })
    await expect(service.apply(voidCommand, identity)).resolves.toMatchObject({
      type: 'command_ack',
      operation_kind: 'VOID_RALLY',
      result_revision: '8',
      effects: { annotation_status: 'voided' },
    })
    await expect(db.rally.findUniqueOrThrow({ where: { id: rallyId } })).resolves.toMatchObject({
      annotationStatus: 'VOIDED',
      annotationRevision: 8n,
  ...
  it('allows clip handles to overlap when logical Rally ranges remain disjoint', async () => {
    ...
    await db.keyPoint.create({
      data: {
        ...
        markerKind: 'SERVICE',
        captureEpochId: ids.epoch,
        captureFrameIndex: BigInt(anchor.capture_frame_index) + 300n,
        captureTimeUs: otherTime,
        sourcePts: BigInt(anchor.source_pts) + 300n,
        timingPrecision: 'FRAME_EXACT',
        originalPlaybackCursor: {},
        createdByUserId: ids.operator,
        updatedByUserId: ids.operator,
  ...
  it('coalesces equal-frame contacts and rejects stale mapping/anchor state', async () => {
    ...
    expect(first.type).toBe('command_ack')
    expect(second.type).toBe('command_ack')
    await expect(
      db.keyPoint.findMany({
        where: { rallyId },
        orderBy: { sequenceIndex: 'asc' },
        select: { markerKind: true, possibleDuplicate: true },
      }),
    ).resolves.toEqual([
  ...
  it('inserts a contact in canonical middle order under the rally lock', async () => {
    ...
    const early = {
      ...anchor,
      capture_time_us: '9007199254740994',
      capture_frame_index: '9007199254740995',
      resolved_player_media_time_us: '1235',
    }
    const late = {
      ...anchor,
      capture_time_us: '9007199254741083',
      capture_frame_index: '9007199254740996',
      resolved_player_media_time_us: '1324',
    }
    const resolver = createAnnotationCommandService({
      database: db,
      resolveCursor: async cursor => (cursor.player_media_time_us === '1' ? early : late),
    })
    const first = contactCommand(randomUUID(), rallyId)
    first.payload.playback_cursor.player_media_time_us = '1'
    await resolver.apply(first, identity)
    const second = contactCommand(randomUUID(), rallyId, '2')
    second.payload.playback_cursor.player_media_time_us = '2'
    await resolver.apply(second, identity)
    const middle = {
      ...anchor,
    ...
    const middleService = createAnnotationCommandService({
      database: db,
      resolveCursor: async () => middle,
    })
    await middleService.apply(contactCommand(randomUUID(), rallyId, '3'), identity)
    await expect(
      db.keyPoint.findMany({
        where: { rallyId },
        orderBy: { sequenceIndex: 'asc' },
        select: { markerKind: true, sequenceIndex: true, captureTimeUs: true },
      }),
    ).resolves.toEqual([
      { markerKind: 'SERVICE', sequenceIndex: 0, captureTimeUs: 9007199254740993n },
      { markerKind: 'CONTACT', sequenceIndex: 1, captureTimeUs: 9007199254740994n },
      { markerKind: 'CONTACT', sequenceIndex: 2, captureTimeUs: 9007199254741043n },
      { markerKind: 'CONTACT', sequenceIndex: 3, captureTimeUs: 9007199254741083n },
    ])
  })

  it.each(['left', 'right', 'unknown'] as const)(
    'closes with %s outcome without creating forbidden rows',
    async outcome => {
      const rallyId = randomUUID()
      await service.apply(serviceCommand(randomUUID(), rallyId), identity)
      const before = await db.keyPoint.findMany({
      ...
      expect(response).toMatchObject({
        ...
        effects: {
          ...
          score_resolution: outcome === 'unknown' ? 'unknown' : 'resolved',
          scoring_court_side: outcome === 'unknown' ? null : outcome,
        },
      })
      const after = await db.keyPoint.findMany({
        where: { rallyId },
        select: {
          id: true,
          captureTimeUs: true,
      ...
      expect(after[0]).toMatchObject({
        id: before[0]!.id,
        captureTimeUs: before[0]!.captureTimeUs,
        captureFrameIndex: before[0]!.captureFrameIndex,
        sourcePts: before[0]!.sourcePts,
        captureEpochId: before[0]!.captureEpochId,
        timingPrecision: before[0]!.timingPrecision,
        isTerminal: true,
      })
      await expect(db.rallySubmission.count({ where: { rallyId } })).resolves.toBe(0)
      await expect(db.pointAward.count()).resolves.toBe(0)
      await expect(db.clipJob.count({ where: { submission: { rallyId } } })).resolves.toBe(0)
      await expect(db.aiJob.count({ where: { submission: { rallyId } } })).resolves.toBe(0)
      await expect(db.analysisRun.count({ where: { submission: { rallyId } } })).resolves.toBe(0)
      const replay = await service.apply(structuredClone(closeCommandValue), identity)
      expect(JSON.stringify(replay)).toBe(
        JSON.stringify(
          parseAnnotationCommandResponse(
            (
              await db.annotationCommandReceipt.findUniqueOrThrow({
                where: { commandId: closeCommandValue.command_id },
              })
            ).responseJson,
          ),
        ),
  ...
  it('rejects non-last, deleted and stale close targets with snapshot refetch', async () => {
    ...
    const contact = await service.apply(contactCommand(randomUUID(), rallyId), identity)
    const servicePoint = (
      await db.keyPoint.findFirstOrThrow({ where: { rallyId, sequenceIndex: 0 } })
    ).id
    await expect(
      service.apply(closeCommand(randomUUID(), rallyId, servicePoint, '2'), identity),
    ).resolves.toMatchObject({
    ...
    await expect(
      service.apply(
        closeCommand(randomUUID(), rallyId, (contact as any).effects.created_key_point_id, '1'),
        identity,
      ),
    ).resolves.toMatchObject({ code: 'REVISION_CONFLICT', snapshot_refetch_required: true })
  })
  ...
  it('rejects a deleted close target without changing the rally', async () => {
    ...
    await db.keyPoint.update({ data: { deletedAt: new Date() }, where: { id: point.id } })
    await expect(
      service.apply(closeCommand(randomUUID(), rallyId, point.id, '1'), identity),
    ).resolves.toMatchObject({
      type: 'command_rejected',
      code: 'CLOSE_RALLY_TARGET_NOT_LAST',
      snapshot_refetch_required: true,
  ...
  it('serializes contact races and close races to one accepted mutation', async () => {
    ...
    const contacts = await Promise.all([
      service.apply(contactCommand(randomUUID(), rallyId), identity),
      service.apply(contactCommand(randomUUID(), rallyId), identity),
    ])
    ...
    const closes = await Promise.all([
      service.apply(closeCommand(randomUUID(), rallyId, point.id, '2', 'left'), identity),
      service.apply(closeCommand(randomUUID(), rallyId, point.id, '2', 'right'), identity),
    ])
  ...
  it('serializes a contact-vs-close race from one base revision with a durable loser', async () => {
    ...
    const close = closeCommand(
      randomUUID(),
      rallyId,
      (await db.keyPoint.findFirstOrThrow({ where: { rallyId } })).id,
      '1',
      'unknown',
    )
    const responses = await Promise.all([
      service.apply(contact, identity),
      service.apply(close, identity),
    ])
    ...
    expect(
      await db.annotationCommandReceipt.count({
        where: { commandId: { in: [contact.command_id, close.command_id] } },
      }),
    ).toBe(2)
    expect((await db.rally.findUniqueOrThrow({ where: { id: rallyId } })).annotationRevision).toBe(
      2n,
    )
  })
  ...
  it('durably rejects close when room authorization becomes stale before its transaction', async () => {
    ...
    try {
      ...
      await expect(
        db.annotationCommandReceipt.findUnique({
          where: { commandId: command.command_id },
        }),
      ).resolves.toMatchObject({ accepted: false })
      await expect(db.rally.findUniqueOrThrow({ where: { id: rallyId } })).resolves.toMatchObject({
      ...
      await expect(
        db.keyPoint.findUniqueOrThrow({ where: { id: target.id } }),
      ).resolves.toMatchObject({
        isTerminal: false,
      })
      await expect(
        db.annotationOperation.findUnique({
  ...
  it('rejects revoked devices, anchor-before-service, and foreign playback mappings durably', async () => {
    ...
    try {
      await expect(service.apply(contactCommand(randomUUID(), rallyId), identity)).rejects.toThrow(
        'Authenticated device session is not active',
      )
    } finally {
      await db.deviceSession.update({ data: { revokedAt: null }, where: { id: ids.device } })
    }
    const variants = [
    ...
    for (const variant of variants) {
      const invalid = createAnnotationCommandService({
        database: db,
        resolveCursor: async () => variant,
      })
      const command = contactCommand(randomUUID(), rallyId)
      await expect(invalid.apply(command, identity)).resolves.toMatchObject({
        code: 'ANNOTATION_NOT_READY',
      })
      await expect(
        db.annotationCommandReceipt.findUnique({ where: { commandId: command.command_id } }),
  ...
  it('rolls back close terminal/outcome when the late operation write fails', async () => {
    ...
    await expect(db.keyPoint.findUniqueOrThrow({ where: { id: point.id } })).resolves.toMatchObject(
      { isTerminal: false },
    )
    await expect(
      db.annotationCommandReceipt.findUnique({ where: { commandId: command.command_id } }),
  ...
  it('allows an editable correction beside the current live OPEN rally', async () => {
    ...
    const submittedPoint = await db.keyPoint.findFirstOrThrow({
      where: { rallyId: submittedRallyId },
    })
    await service.apply(
      closeCommand(randomUUID(), submittedRallyId, submittedPoint.id, '1', 'unknown'),
      identity,
    )
    const submitted = await service.apply(
      submitCommand(randomUUID(), submittedRallyId, '2'),
      identity,
    )
    ...
    await expect(
      laterService.apply(serviceCommand(randomUUID(), openRallyId), identity),
    ).resolves.toMatchObject({
      type: 'command_ack',
  ...
  it('opens a correction draft and applies winner/unknown changes through one CAS score ledger', async () => {
    ...
    const firstSubmissionId =
      firstResponse.type === 'command_ack' ? firstResponse.effects.submission_id : null
    expect(firstSubmissionId).toBeTruthy()
    await expect(db.matchSet.findUniqueOrThrow({ where: { id: ids.set } })).resolves.toMatchObject({
      leftScore: 1,
      rightScore: 0,
      scoreRevision: 1,
    })
    await expect(
      db.scoreLedgerEntry.findMany({
        where: { setId: ids.set },
        orderBy: { scoreRevisionAfter: 'asc' },
      }),
    ).resolves.toMatchObject([
      {
        kind: 'POINT_AWARD',
    ...
    await expect(
      db.pointAward.findUnique({ where: { submissionId: firstSubmissionId! } }),
    ).resolves.toMatchObject({ ledgerEntryId: expect.any(String) })
    ...
    const restored = await db.keyPoint.findMany({
      where: { rallyId, deletedAt: null },
      orderBy: { sequenceIndex: 'asc' },
    })
    ...
    await service.apply(
      closeCommand(randomUUID(), rallyId, servicePoint.id, '5', 'right'),
      identity,
    )
    const secondResponse = await service.apply(submitCommand(randomUUID(), rallyId, '6'), identity)
    const secondSubmissionId =
      secondResponse.type === 'command_ack' ? secondResponse.effects.submission_id : null
    ...
    await expect(
      db.rallySubmission.findUniqueOrThrow({ where: { id: firstSubmissionId! } }),
    ).resolves.toMatchObject({ status: 'SUPERSEDED' })
    await expect(
      db.rallySubmission.findUniqueOrThrow({ where: { id: secondSubmissionId! } }),
    ).resolves.toMatchObject({ status: 'ACTIVE', supersedesSubmissionId: firstSubmissionId })
    await expect(
      db.scoreLedgerEntry.findMany({
        where: { setId: ids.set },
        orderBy: { scoreRevisionAfter: 'asc' },
      ...
      },
      {
        ...
        leftDelta: -1,
        rightDelta: 1,
        scoreRevisionBefore: 1,
        scoreRevisionAfter: 2,
        supersededSubmissionId: firstSubmissionId,
      },
    ])
    await expect(
      db.clipJob.findFirstOrThrow({ where: { submissionId: firstSubmissionId! } }),
    ).resolves.toMatchObject({ status: 'SUPERSEDED' })
    ...
    await service.apply(
      closeCommand(randomUUID(), rallyId, servicePoint.id, '9', 'unknown'),
      identity,
    )
    const thirdResponse = await service.apply(submitCommand(randomUUID(), rallyId, '10'), identity)
    const thirdSubmissionId =
      thirdResponse.type === 'command_ack' ? thirdResponse.effects.submission_id : null
    ...
    await expect(
      db.rallySubmission.findUniqueOrThrow({ where: { id: thirdSubmissionId! } }),
    ).resolves.toMatchObject({
      leftScoreAfter: null,
      leftScoreBefore: null,
      rightScoreAfter: null,
      rightScoreBefore: null,
      scoreResolutionState: 'UNKNOWN',
      scoreRevisionAfter: null,
      scoreRevisionBefore: null,
      supersedesSubmissionId: secondSubmissionId,
    })
    await expect(
      db.pointAward.findUnique({ where: { submissionId: thirdSubmissionId! } }),
    ).resolves.toBeNull()
    await expect(
      db.scoreLedgerEntry.findFirst({ where: { submissionId: thirdSubmissionId! } }),
    ).resolves.toMatchObject({
      kind: 'CORRECTION',
      leftDelta: 0,
      rightDelta: -1,
      scoreRevisionBefore: 2,
      scoreRevisionAfter: 3,
    })
  })
  ...
  it('turns a legacy multi-point submission into boundaries and clears contacts for AI regeneration', async () => {
    ...
    await Promise.all([
      ...
      db.dvrSegment.update({
        data: { captureEndUs: BigInt(laterAnchor.capture_time_us) + 1n, frameCount: 123n },
        where: { id: ids.segment },
      }),
    ])
    const laterService = createAnnotationCommandService({
    ...
    const finalPoint = await db.keyPoint.findFirstOrThrow({
      where: { rallyId },
      orderBy: { sequenceIndex: 'desc' },
    })
    await laterService.apply(
      closeCommand(randomUUID(), rallyId, finalPoint.id, '2', 'unknown'),
      identity,
    )
    ...
    await expect(
      db.rallyBoundary.findMany({ where: { rallyId }, orderBy: { kind: 'asc' } }),
    ).resolves.toMatchObject([
  ...
  it('creates an authoritative terminal at the second Z and submits a two-boundary rally', async () => {
    ...
    const endAnchor = {
      ...
      capture_frame_index: (BigInt(anchor.capture_frame_index) + 300n).toString(),
      resolved_player_media_time_us: (
        BigInt(anchor.resolved_player_media_time_us) + 5_000_000n
      ).toString(),
    }
    await Promise.all([
      db.playbackWindow.update({
        data: { captureEndUs: BigInt(endAnchor.capture_time_us) + 1n },
        where: { id: ids.window },
      }),
      db.dvrSegment.update({
        data: { captureEndUs: BigInt(endAnchor.capture_time_us) + 1n, frameCount: 303n },
        where: { id: ids.segment },
      }),
    ])
    const endService = createAnnotationCommandService({
      database: db,
      resolveCursor: async () => endAnchor,
    })
    ...
    const readyRally = await db.rally.findUniqueOrThrow({
      include: { keyPoints: { orderBy: { sequenceIndex: 'asc' } } },
      where: { id: rallyId },
    })
    expect(readyRally).toMatchObject({
      ...
      keyPoints: [
        ...
        expect.objectContaining({
          markerKind: 'CONTACT',
          isTerminal: true,
          captureTimeUs: BigInt(endAnchor.capture_time_us),
        }),
      ],
    })
    ...
    const submission = await db.rallySubmission.findFirstOrThrow({
  ...
  it('reverses one submitted rally side snapshot and moves its point to the opposite team', async () => {
    ...
    const firstSubmissionId = first.type === 'command_ack' ? first.effects.submission_id : null
    expect(firstSubmissionId).toBeTruthy()

    await expect(
      createCorrectionDraft(db, firstSubmissionId!, identity, { reverseCourtSides: true }),
    ).resolves.toMatchObject({
      annotation_status: 'open',
      revision: '4',
      supersedes_submission_id: firstSubmissionId,
    ...
    await expect(db.rally.findUniqueOrThrow({ where: { id: rallyId } })).resolves.toMatchObject({
      ...
      scoringTeamId: ids.right,
      sideAssignmentReversed: true,
    })

    const corrected = await service.apply(submitCommand(randomUUID(), rallyId, '4'), identity)
    ...
    await expect(
      ...
      leftTeamId: ids.right,
      rightTeamId: ids.left,
      scoringCourtSide: 'LEFT',
      scoringTeamId: ids.right,
      sideAssignmentReversed: true,
      supersedesSubmissionId: firstSubmissionId,
    })
    await expect(db.matchSet.findUniqueOrThrow({ where: { id: ids.set } })).resolves.toMatchObject({
      leftScore: scoreBefore.leftScore,
      rightScore: scoreBefore.rightScore + 1,
      scoreRevision: scoreBefore.scoreRevision + 2,
    })
  })
  ...
  it('cancels a correction and restores the active immutable submission', async () => {
    ...
    await createCorrectionDraft(db, submissionId!, identity)
    await service.apply(
      closeCommand(randomUUID(), rallyId, servicePoint.id, '4', 'right'),
      identity,
    )
    ...
    await expect(
      db.keyPoint.findMany({ where: { rallyId, deletedAt: null } }),
    ).resolves.toMatchObject([{ id: servicePoint.id, isTerminal: true, markerKind: 'SERVICE' }])
  })

  it('rolls back a failed submitted correction and records a compensating score entry', async () => {
    const rallyId = randomUUID()
    await db.matchSet.update({ where: { id: ids.set }, data: { leftScore: 0, rightScore: 0 } })
    await service.apply(serviceCommand(randomUUID(), rallyId), identity)
    const servicePoint = await db.keyPoint.findFirstOrThrow({ where: { rallyId } })
    await service.apply(closeCommand(randomUUID(), rallyId, servicePoint.id, '1', 'left'), identity)
    const submitted = await service.apply(submitCommand(randomUUID(), rallyId, '2'), identity)
    const sourceSubmissionId =
      submitted.type === 'command_ack' ? submitted.effects.submission_id : null
    expect(sourceSubmissionId).toBeTruthy()
    const sourceScore = await db.matchSet.findUniqueOrThrow({ where: { id: ids.set } })

    await createCorrectionDraft(db, sourceSubmissionId!, identity)
    await service.apply(
      parseAnnotationCommand({
        ...serviceCommand(randomUUID(), rallyId),
        base_revision: '4',
    ...
    const failedSubmissionId =
      corrected.type === 'command_ack' ? corrected.effects.submission_id : null
    expect(failedSubmissionId).toBeTruthy()
    const correctedScore = await db.matchSet.findUniqueOrThrow({ where: { id: ids.set } })
    await db.rally.update({ where: { id: rallyId }, data: { processingStatus: 'FAILED' } })

    await expect(cancelCorrectionDraft(db, rallyId, identity)).resolves.toMatchObject({
      active_submission_id: sourceSubmissionId,
      annotation_status: 'submitted',
      revision: '8',
    })
    await expect(db.rally.findUniqueOrThrow({ where: { id: rallyId } })).resolves.toMatchObject({
      activeSubmissionId: sourceSubmissionId,
      annotationStatus: 'SUBMITTED',
      scoringCourtSide: 'LEFT',
    })
    await expect(db.matchSet.findUniqueOrThrow({ where: { id: ids.set } })).resolves.toMatchObject({
      leftScore: sourceScore.leftScore,
      rightScore: sourceScore.rightScore,
      scoreRevision: correctedScore.scoreRevision + 1,
    })
    await expect(
      db.rallySubmission.findUniqueOrThrow({ where: { id: sourceSubmissionId! } }),
    ).resolves.toMatchObject({ status: 'ACTIVE' })
    await expect(
      db.rallySubmission.findUniqueOrThrow({ where: { id: failedSubmissionId! } }),
    ).resolves.toMatchObject({ status: 'SUPERSEDED' })
    await expect(
      db.scoreLedgerEntry.findMany({
        where: { submissionId: { in: [sourceSubmissionId!, failedSubmissionId!] } },
        orderBy: { scoreRevisionAfter: 'asc' },
      }),
    ).resolves.toMatchObject([
      { kind: 'POINT_AWARD', leftDelta: 1, rightDelta: 0 },
      { kind: 'CORRECTION', leftDelta: -1, rightDelta: 1 },
      {
        kind: 'CORRECTION_ROLLBACK',
        leftDelta: 1,
        rightDelta: -1,
        reversalOfEntryId: expect.any(String),
      },
    ])
  })
  ...
  it('reuses completed clip and analysis evidence for an exact-frame time correction', async () => {
    ...
    const timingAssetId = randomUUID()
    const analysisFrameAssetId = randomUUID()
    await db.mediaAsset.createMany({
      data: [
        ...
        {
          id: analysisFrameAssetId,
          kind: 'ANALYSIS_FRAME_CHUNK',
          bucket: 'reuse-test',
          objectKey: `${rallyId}.bin`,
    ...
    const sourceClip = await db.clipJob.findFirstOrThrow({
      where: { submissionId: sourceSubmission.id },
    })
    await db.clipJob.update({
    ...
    await db.clipKeyPointMapping.create({
      data: {
        clipJobId: sourceClip.id,
        submissionKeyPointId: sourcePoint.id,
        clipPts: 0n,
        clipTimeUs: 1n,
        clipFrameIndex: 0n,
      },
    })
    const sourceAi = await db.aiJob.create({
      data: {
        submissionId: sourceSubmission.id,
        clipJobId: sourceClip.id,
        status: 'COMPLETED',
        idempotencyKey: `reuse-source:${rallyId}`,
        requestPayload: {
          ai_job_id: 'source',
          rally_submission_id: sourceSubmission.id,
          annotation_revision: sourceSubmission.annotationRevision.toString(),
          clip: { clip_job_id: sourceClip.id },
          key_points: [{ key_point_id: sourcePoint.id }],
          outcome: { score_resolution: 'unknown', scoring_court_side: null },
        },
    ...
    const sourceAnalysis = await db.analysisRun.create({
      data: {
        aiJobId: sourceAi.id,
        submissionId: sourceSubmission.id,
        analysisId: `reuse-analysis-${rallyId}`,
        analysisVersion: 'reuse-v1',
        analysisDataSchemaVersion: '1.0.0',
    ...
    await db.contactEvent.create({
      data: {
        ...
        markerKind: sourcePoint.markerKind,
        isTerminal: true,
        associationState: 'NO_PLAYER',
        ballState: 'MISSING',
        qualityFlags: [],
      },
    })
    await db.ballPathSegment.create({
      data: {
        analysisRunId: sourceAnalysis.id,
        sequenceIndex: 0,
    ...
    await db.analysisDataManifest.create({
      data: {
        analysisRunId: sourceAnalysis.id,
        schemaVersion: '1.0.0',
        analysisDataVersion: '1.0.0',
        videoWidth: 1920,
        videoHeight: 1080,
        fpsNum: 60,
        fpsDen: 1,
    ...
    const freshClip = await db.clipJob.findFirstOrThrow({
      where: { submissionId: correctedSubmissionId! },
    })
    expect(freshClip).toMatchObject({
      status: 'COMPLETED',
      clipAssetId,
      timingManifestAssetId: timingAssetId,
    })
    ...
    await expect(
      db.clipJob.findUniqueOrThrow({ where: { id: sourceClip.id } }),
  ...
  it('creates an editable correction and submits it unchanged without reopening', async () => {
    ...
    const duplicateSubmissionId =
      duplicate.type === 'command_ack' ? duplicate.effects.submission_id : null
    expect(duplicateSubmissionId).toBeTruthy()
    expect(duplicateSubmissionId).not.toBe(submissionId)
    await expect(db.rallySubmission.count({ where: { rallyId } })).resolves.toBe(2)
    await expect(
      db.rallySubmission.findUniqueOrThrow({ where: { id: submissionId! } }),
    ).resolves.toMatchObject({ status: 'SUPERSEDED' })
    await expect(db.rally.findUniqueOrThrow({ where: { id: rallyId } })).resolves.toMatchObject({
      activeSubmissionId: duplicateSubmissionId,
      annotationStatus: 'SUBMITTED',
      annotationRevision: 5n,
      scoreResolutionState: 'UNKNOWN',
      scoringCourtSide: null,
  ...
  it('re-enters an existing OPEN correction without duplicating draft operations', async () => {
    const rallyId = randomUUID()
    await service.apply(serviceCommand(randomUUID(), rallyId), identity)
    const point = await db.keyPoint.findFirstOrThrow({ where: { rallyId } })
    await service.apply(closeCommand(randomUUID(), rallyId, point.id, '1', 'left'), identity)
    ...
    const opened = await createCorrectionDraft(db, submissionId!, identity)
    const operationCount = await db.annotationOperation.count({
      where: { rallyId, operationKind: 'CREATE_CORRECTION_DRAFT' },
    })
    await expect(createCorrectionDraft(db, submissionId!, identity)).resolves.toEqual(opened)
    await expect(
      db.annotationOperation.count({
        where: { rallyId, operationKind: 'CREATE_CORRECTION_DRAFT' },
      }),
    ).resolves.toBe(operationCount)
    await expect(db.rally.findUniqueOrThrow({ where: { id: rallyId } })).resolves.toMatchObject({
      activeSubmissionId: submissionId,
      annotationStatus: 'OPEN',
      annotationRevision: BigInt(opened.revision),
    })
  })
})
