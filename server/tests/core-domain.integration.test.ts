...
describe('Phase 1B GraphQL schema', () => {
  ...
  it('resolves a member timeline from the newest program and coalesces ready segments', async () => {
    ...
    for (let i = 0; i < 6; i++) { const id = `20000000-0000-4000-8000-${String(30 + i).padStart(12, '0')}`; assets.push(id); await db.mediaAsset.create({ data: { id, kind: i % 3 === 0 ? 'DVR_INIT' : i % 3 === 1 ? 'DVR_SEGMENT' : 'SAMPLE_INDEX', bucket: 'fixture', objectKey: id, contentType: 'video/mp4', state: 'READY', readyAt: new Date(), internalSchemaVersion: i === 4 ? null : '1.0.0' } }) }
    await db.dvrSegment.createMany({ data: [
      { id: '20000000-0000-4000-8000-000000000022', dvrProgramId: programNew, captureEpochId: epochId, sequenceNumber: 2n, discontinuitySequence: 0, captureStartUs: 9007199254741192n, captureEndUs: 9007199254741292n, frameCount: 3n, durationUs: 100n, readyAt: null, initAssetId: assets[6]!, mediaAssetId: assets[7]!, sampleIndexAssetId: assets[8]! },
      { id: '20000000-0000-4000-8000-000000000023', dvrProgramId: programNew, captureEpochId: epochId, sequenceNumber: 3n, discontinuitySequence: 0, captureStartUs: 9007199254741292n, captureEndUs: 9007199254741392n, frameCount: 3n, durationUs: 100n, readyAt: new Date(), initAssetId: assets[9]!, mediaAssetId: assets[10]!, sampleIndexAssetId: assets[11]! },
      ...
      const result = await execute('{ captureSession(id: "20000000-0000-4000-8000-000000000001") { id sourceKind sourceDurationUs timeline { availabilityComplete timelineVersion captureStartTimeUs liveEdgeCaptureTimeUs ingestFrontierCaptureTimeUs sourceEndCaptureTimeUs gapRanges { startUs endUs discontinuity } availableRanges { startUs endUs discontinuity } } } }', contextFor(operatorUser))
      expect(result.errors).toBeUndefined()
      expect(result.data).toEqual({ captureSession: {
        id: sessionId,
        ...
        timeline: {
          ...
          gapRanges: [{ startUs: '9007199254741392', endUs: '9007199254741492', discontinuity: 0 }],
          availableRanges: [{ startUs: '9007199254740992', endUs: '9007199254741192', discontinuity: 0 }, { startUs: '9007199254741492', endUs: '9007199254741592', discontinuity: 1 }],
        },
      } })
