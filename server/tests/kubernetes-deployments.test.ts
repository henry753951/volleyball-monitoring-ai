...
describe('Kubernetes deployment snapshot', () => {
  it('reports version, commit, deployed digest, resources, node, and accelerator', () => {
    ...
    const snapshot = buildDeploymentSnapshot('volleyball-monitoring', [{
      metadata: {
      ...
      metadata: { labels: { 'app.kubernetes.io/name': 'volleyball-analysis-worker' }, name: 'analysis-worker-1' },
      spec: { nodeName: 'k8s-worker-1' },
      status: { containerStatuses: [{ imageID: `ghcr.io/example/engine@${digest}`, name: 'analysis-worker', ready: true }], phase: 'Running' },
    }])
