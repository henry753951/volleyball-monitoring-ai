...
beforeAll(async () => {
  ...
  await db.user.create({
    data: { id: ids.operator, email: 'operator@cancel.local', displayName: 'Operator' },
  })
  ...
  await db.courtSideAssignment.create({
    data: {
      id: ids.correctionAssignment,
      setId: ids.correctionSet,
      effectiveFromRallyOrdinal: 1,
      leftTeamId: ids.left,
      rightTeamId: ids.right,
    },
  ...
  await db.rally.create({
    data: {
      id: ids.correctionRally,
      matchId: ids.match,
      setId: ids.correctionSet,
      dvrProgramId: ids.program,
      sideAssignmentId: ids.correctionAssignment,
      ordinal: 1,
  ...
  await db.rallySubmission.create({
    data: {
      id: ids.previousSubmission,
      rallyId: ids.correctionRally,
      annotationRevision: 1n,
      contentHash: 'd'.repeat(64),
      status: 'SUPERSEDED',
      scoreResolutionState: 'RESOLVED',
      scoringCourtSide: 'LEFT',
      scoringTeamId: ids.left,
      leftTeamId: ids.left,
      rightTeamId: ids.right,
      sideAssignmentId: ids.correctionAssignment,
      leftScoreBefore: 0,
  ...
  await db.scoreLedgerEntry.create({
    data: {
      kind: 'POINT_AWARD',
      setId: ids.correctionSet,
      submissionId: ids.previousSubmission,
      leftDelta: 1,
      rightDelta: 0,
      leftScoreBefore: 0,
      rightScoreBefore: 0,
  ...
  await db.rallySubmission.create({
    data: {
      ...
      clipPostRollUs: 3_000_000n,
      submittedByUserId: ids.operator,
      supersedesSubmissionId: ids.previousSubmission,
    },
  })
  await db.scoreLedgerEntry.create({
    data: {
      kind: 'CORRECTION',
      setId: ids.correctionSet,
      submissionId: ids.correctionSubmission,
      supersededSubmissionId: ids.previousSubmission,
      leftDelta: -1,
      rightDelta: 1,
      leftScoreBefore: 1,
      rightScoreBefore: 0,
  ...
  await db.rally.update({
    where: { id: ids.correctionRally },
    data: { activeSubmissionId: ids.correctionSubmission },
  })
  await db.clipJob.create({
    data: {
      id: ids.correctionClipJob,
      submissionId: ids.correctionSubmission,
      status: 'COMPLETED',
      idempotencyKey: 'cancel-correction-clip',
      canonicalizationProfileVersion: 'canonical-v1',
      requestedStartCaptureUs: 0n,
      requestedEndCaptureUs: 1_000_000n,
      completedAt: new Date(),
  ...
  await db.aiJob.create({
    data: {
      id: ids.correctionAiJob,
      submissionId: ids.correctionSubmission,
      clipJobId: ids.correctionClipJob,
      status: 'RUNNING',
      idempotencyKey: 'cancel-correction-ai',
      requestPayload: {},
      requestPayloadHash: 'f'.repeat(64),
      jobSchemaVersion: '1.1.0',
      callbackTokenHash: '1'.repeat(64),
      callbackTokenExpiresAt: new Date(Date.now() + 60_000),
    },
  })
}, 120_000)
...
describe('processing rally cancellation', () => {
  it('soft-deletes the rally, cancels the AI job, and writes durable abort audit events', async () => {
    const result = await cancelProcessingRally(db, ids.rally, {
    ...
    expect(
      (await db.rallySubmission.findUniqueOrThrow({ where: { id: ids.submission } })).status,
    ).toBe('SUPERSEDED')
    expect(
      (
        await db.rallySubmission.findUniqueOrThrow({
          where: { id: result.cancellation_submission_id },
        })
      ).status,
    ).toBe('CANCELLED')
    ...
    expect(
      await db.outboxEvent.findUnique({ where: { dedupeKey: `ai-abort:${ids.aiJob}` } }),
    ).not.toBeNull()
  })

  it('restores the previous completed submission when a processing correction is cancelled', async () => {
    await cancelProcessingRally(db, ids.correctionRally, {
      deviceSessionId: ids.device,
      role: UserRole.OPERATOR,
      userId: ids.operator,
    })
    ...
    expect(
      (await db.rallySubmission.findUniqueOrThrow({ where: { id: ids.previousSubmission } }))
        .status,
    ).toBe('ACTIVE')
    expect(
      (await db.rallySubmission.findUniqueOrThrow({ where: { id: ids.correctionSubmission } }))
        .status,
    ).toBe('SUPERSEDED')
    const set = await db.matchSet.findUniqueOrThrow({ where: { id: ids.correctionSet } })
    expect({ left: set.leftScore, right: set.rightScore, revision: set.scoreRevision }).toEqual({
      left: 1,
      right: 0,
      revision: 3,
    })
    const cancellationLedger = await db.scoreLedgerEntry.findFirstOrThrow({
      where: { supersededSubmissionId: ids.correctionSubmission },
    })
    expect({ left: cancellationLedger.leftDelta, right: cancellationLedger.rightDelta }).toEqual({
      left: 1,
      right: -1,
    })
    expect((await db.aiJob.findUniqueOrThrow({ where: { id: ids.correctionAiJob } })).status).toBe(
      'CANCELLED',
    )
  })
})
