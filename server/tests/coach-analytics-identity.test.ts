...
describe('coach track identity replacement', () => {
  it('uses the effective corrected contact actor in coach player totals', async () => {
                ...
                contactEvents: [
                  {
                    ...
                    actors: [{ trackId: 1, action: null, courtX: null, courtY: null }],
                  },
                ],
                segments: [],
              },
    ...
    expect(
      analytics?.players.map(player => [player.roster_entry_id, player.contact_count]),
    ).toEqual([
      ['roster-1', 0],
      ['roster-2', 1],
    ])
  ...
  it('only moves an occupied player off tracks whose frame ranges overlap', async () => {
    const tx = {
        ...
        findUnique: vi.fn().mockResolvedValue({
          ...
          analysisRun: {
            submission: {
              leftTeamId: 'team-left',
              rightTeamId: 'team-right',
              rally: { matchId: 'match-1', ordinal: 3, set: { setNumber: 2 } },
            },
          },
        }),
      },
      ...
      reidFeatureObservation: {
        findUnique: vi.fn().mockResolvedValue({
          id: 'observation-9',
          reidIdentityId: 'slot-1',
          modelNamespace: 'nested-v1',
          aliasTrackIds: [9],
          courtSide: 'LEFT',
        ...
        findMany: vi
          .fn()
          .mockResolvedValueOnce([{ trackId: 9 }, { trackId: 10 }, { trackId: 11 }])
    ...
    expect(
      tx.trackIdentityAssignment.upsert.mock.calls.map(
        call => call[0].where.analysisRunId_trackId.trackId,
      ),
    ).toEqual([9, 10, 11])
  })
  ...
  it('preserves a same-player track when its frame range does not overlap', async () => {
    const tx = {
      ...
      reidFeatureObservation: {
        ...
        findMany: vi
          .fn()
          .mockResolvedValueOnce([{ trackId: 9 }])
          .mockResolvedValueOnce([]),
      },
      reidPlayerBinding: {
        findFirst: vi.fn().mockResolvedValue(null),
      ...
      trackIdentityAssignment: {
        findMany: vi
          .fn()
          .mockResolvedValue([{ trackId: 5, track: { firstFrame: 201n, lastFrame: 300n } }]),
        deleteMany: vi.fn(),
        upsert: vi.fn().mockResolvedValue({
          id: 'assignment-1',
          analysisRunId: 'run-1',
    ...
    const result = await assignTrackIdentity(database, {
      analysisRunId: 'run-1',
      trackId: 9,
      rosterEntryId: 'roster-1',
      userId: 'user-1',
  ...
  it('reapplies one GID binding as separate local assignments while preserving manual overrides', async () => {
          ...
          tracks: [
            {
              trackId: 9,
              identityAssignments: [],
              reidObservation: { reidIdentityId: 'slot-1', matchConfidence: 0.91 },
            },
            {
              trackId: 12,
              identityAssignments: [],
              reidObservation: { reidIdentityId: 'slot-1', matchConfidence: 0.84 },
            },
            {
              trackId: 15,
              identityAssignments: [
              ...
              reidObservation: { reidIdentityId: 'slot-1', matchConfidence: 0.72 },
            },
            {
              trackId: 18,
              identityAssignments: [],
              reidObservation: { reidIdentityId: null, matchConfidence: null },
            },
          ],
        }),
      },
        ...
        findMany: vi.fn().mockResolvedValue([
          {
            ...
            rosterEntryId: 'roster-1',
            identityRevision: 7n,
            reidIdentity: { teamId: 'team-left' },
            rosterEntry: { teamId: 'team-left' },
          },
        ]),
      },
    ...
    await expect(
      applyReidAutomaticAssignments(database, {
        analysisRunId: 'run-1',
        userId: 'user-1',
        role: UserRole.COACH,
      }),
    ...
    expect(tx.trackIdentityAssignment.upsert).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('keeps a clip-only correction scoped to exactly one local track', async () => {
    ...
    await applyManualReidDecision(tx as never, {
      ...
      rosterEntryId: 'roster-1',
      userId: 'user-1',
      position: { setNumber: 1, rallyOrdinal: 2 },
      mode: 'clip_only',
      replacedTrackIds: [],
    })
  ...
  it('allows a clip-only manual assignment when a legacy track has no ReID observation', async () => {
    ...
    await expect(
      applyManualReidDecision(tx as never, {
        matchId: 'match-1',
        teamId: 'team-left',
        analysisRunId: 'run-1',
        trackId: 31,
        rosterEntryId: 'roster-1',
        userId: 'user-1',
        position: { setNumber: 1, rallyOrdinal: 3 },
        mode: 'clip_only',
        replacedTrackIds: [],
      }),
