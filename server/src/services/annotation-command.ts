...
async function acceptContact(database: PrismaClient, room: AnnotationRoom, command: ContactCommand, identity: AnnotationIdentity, hash: string, anchor: ResolvedMediaAnchor): Promise<AnnotationCommandResponse> {
  return serializable(database, async (tx) => {
    ...
    const mapping = await tx.playbackWindow.findFirst({ where: { id: anchor.playback_window_id, captureSessionId: room.captureSessionId, mappingVersion: anchor.mapping_version, segments: anchor.dvr_segment_id ? { some: { dvrSegment: { id: anchor.dvr_segment_id, captureEpochId: anchor.capture_epoch_id, dvrProgramId: rally.dvrProgramId, isGap: false, readyAt: { not: null }, sampleIndexAssetId: { not: null } } } } : undefined } })
