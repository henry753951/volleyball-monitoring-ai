...
async function acceptContact(database: PrismaClient, room: AnnotationRoom, command: ContactCommand, identity: AnnotationIdentity, hash: string, anchor: ResolvedMediaAnchor): Promise<AnnotationCommandResponse> {
  return serializable(database, async (tx) => {
    ...
    const captureTime = BigInt(anchor.capture_time_us); const captureFrame = BigInt(anchor.capture_frame_index); const playerTime = BigInt(anchor.resolved_player_media_time_us)
    const valid = anchor.capture_session_id === room.captureSessionId && !!mapping && !!segment && !!epoch && mapping.dvrProgramId === rally.dvrProgramId && segment.dvrProgramId === rally.dvrProgramId && segment.captureEpochId === anchor.capture_epoch_id && !segment.isGap && segment.readyAt !== null && segment.sampleIndexAssetId !== null && captureTime >= mapping.captureStartUs && captureTime < mapping.captureEndUs && captureTime >= segment.captureStartUs && captureTime < segment.captureEndUs && segment.firstFrameIndex !== null && segment.firstFrameIndex !== undefined && captureFrame >= segment.firstFrameIndex && captureFrame < segment.firstFrameIndex + segment.frameCount && playerTime === captureTime - mapping.presentationOriginCaptureUs && anchor.playback_window_id === command.payload.playback_cursor.playback_window_id && anchor.mapping_version === command.payload.playback_cursor.mapping_version
