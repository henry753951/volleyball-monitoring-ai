...
const actionName = (value: unknown) => typeof value === 'string'
  ? value
...
export async function getCoachMatchAnalytics(database: PrismaClient, input: { matchId: string; userId: string; role: UserRole }) {
  ...
  const events = analyzed.flatMap(entry => {
    const corrections = new Map(entry.run.contactActorCorrections.map(correction => [correction.keyPointId, correction.trackId]))
    const timeCorrections = new Map((entry.run.contactTimeCorrections ?? []).map(correction => [correction.keyPointId, correction.frameIndex]))
    const edits = new Map((entry.run.contactEdits ?? []).map(edit => [edit.contactId, edit]))
    const actionCorrections = new Map((entry.run.actionCorrections ?? []).map(correction => [`${correction.frameIndex}:${correction.trackId}`, correction.action]))
    const baseEvents = entry.run.contactEvents.filter(event => !edits.get(event.keyPointId)?.deleted).map((event) => {
      const correctedTrackId = corrections.get(event.keyPointId)
      const hasCorrection = corrections.has(event.keyPointId)
      const frameIndex = timeCorrections.get(event.keyPointId) ?? event.resolvedFrameIndex ?? event.anchorFrameIndex
      const selectedActors = !hasCorrection
        ? event.actors
        : correctedTrackId === null
          ? []
          : [event.actors.find(actor => actor.trackId === correctedTrackId) ?? { trackId: correctedTrackId, action: null, courtX: null, courtY: null }]
      const actors = selectedActors.map(actor => ({ ...actor, action: actionCorrections.get(`${frameIndex}:${actor.trackId}`) ?? actor.action }))
      return { ...event, frameIndex, actors, associationState: hasCorrection ? correctedTrackId === null ? 'NO_PLAYER' as const : 'RESOLVED_SINGLE' as const : event.associationState, runId: entry.run.id, rallyId: entry.rally.id }
    })
    const manualEvents = (entry.run.contactEdits ?? []).filter(edit => !edit.baseKeyPointId && !edit.deleted).map(edit => ({
      keyPointId: edit.contactId,
      anchorFrameIndex: edit.frameIndex,
      resolvedFrameIndex: edit.frameIndex,
      frameIndex: edit.frameIndex,
      associationState: edit.trackId === null ? 'NO_PLAYER' as const : 'RESOLVED_SINGLE' as const,
      qualityFlags: ['manual_contact'],
      representativePositions: [] as Array<{ courtX: number; courtY: number }>,
      actors: edit.trackId === null ? [] : [{ trackId: edit.trackId, action: actionCorrections.get(`${edit.frameIndex}:${edit.trackId}`) ?? null, courtX: null, courtY: null }],
      runId: entry.run.id,
      rallyId: entry.rally.id,
    }))
    return [...baseEvents, ...manualEvents].sort((left, right) => Number(left.frameIndex - right.frameIndex))
  })
  ...
  const playerHeatmaps = new Map<string, Array<{ x: number; y: number; rally_id: string; set_number: number; action: string | null }>>()
  ...
  for (const actor of actors) {
    ...
    if (!entry) continue
    const rallySet = playerRallies.get(rosterId) ?? new Set<string>(); rallySet.add(actor.rallyId); playerRallies.set(rosterId, rallySet)
    const category = actionCategory(actor.action)
    if (category) { const counts = playerActions.get(rosterId) ?? {}; counts[category] = (counts[category] ?? 0) + 1; playerActions.set(rosterId, counts) }
    if (actor.courtX !== null && actor.courtY !== null) {
      ...
      samples.push({ x: flip ? 1 - actor.courtX : actor.courtX, y: flip ? 1 - actor.courtY : actor.courtY, rally_id: actor.rallyId, set_number: entry.rally.set.setNumber, action: category })
  ...
  return {
    schema_version: '1.0.0', match: { id: match.id, title: match.title }, identity_revision: match.identityRevision.toString(),
    ...
    sets: [...new Set(rallies.map(rally => rally.set.setNumber))].map(setNumber => { const items = rallies.filter(rally => rally.set.setNumber === setNumber); return { set_number: setNumber, rally_count: items.length, resolved_count: items.filter(rally => rally.submission.scoreResolutionState === 'RESOLVED').length, unknown_count: items.filter(rally => rally.submission.scoreResolutionState !== 'RESOLVED').length, team_points: Object.fromEntries(teams.map(team => [team.id, items.filter(rally => rally.submission.scoringTeamId === team.id).length])) } }),
    rallies: rallies.map(rally => ({ id: rally.id, set_number: rally.set.setNumber, ordinal: rally.ordinal, score_resolution: rally.submission.scoreResolutionState.toLowerCase(), scoring_team_id: rally.submission.scoringTeamId, contact_count: events.filter(event => event.rallyId === rally.id).length, replay_url: `/matches/${match.id}/replay/${rally.id}` })),
    players: match.rosterEntries.map(entry => ({ roster_entry_id: entry.id, team_id: entry.teamId, jersey_number: entry.jerseyNumber, position: entry.position, name: entry.displayNameSnapshot ?? entry.player?.name ?? `#${entry.jerseyNumber}`, contact_count: playerContacts.get(entry.id) ?? 0, sample_count: playerContacts.get(entry.id) ?? 0, rally_count: playerRallies.get(entry.id)?.size ?? 0, action_counts: playerActions.get(entry.id) ?? {}, heatmap_samples: playerHeatmaps.get(entry.id) ?? [], error_count: null })),
    tracks: analyzed.flatMap(entry => entry.run.tracks.map((track) => {
      ...
      reid_model: observation ? { namespace: observation.modelNamespace, name: observation.modelName, checkpoint_sha256: observation.modelCheckpointSha256, preprocess_version: observation.modelPreprocessVersion, dimension: observation.modelDimension, distance: observation.modelDistance } : null,
...
export function frameRangesOverlap(left: { firstFrame: bigint; lastFrame: bigint }, right: { firstFrame: bigint; lastFrame: bigint }) {
  return left.firstFrame <= right.lastFrame && right.firstFrame <= left.lastFrame
}

export async function assignTrackIdentity(database: PrismaClient, input: { analysisRunId: string; trackId: number; rosterEntryId: string; identityMode?: unknown; userId: string; role: UserRole }) {
  if (input.role !== UserRole.ADMIN && input.role !== UserRole.OPERATOR && input.role !== UserRole.COACH) throw new Error('FORBIDDEN')
  let identityMode
  try { identityMode = parseReidIdentityMode(input.identityMode) }
  catch { throw new GraphQLError('身分修正模式無效', { extensions: { code: 'BAD_USER_INPUT' } }) }
  return database.$transaction(async (tx) => {
          ...
          select: {
            submission: {
              select: {
                leftTeamId: true,
                rightTeamId: true,
                rally: { select: { matchId: true, ordinal: true, set: { select: { setNumber: true } } } },
              },
            },
          },
        },
    ...
    const occupied = await tx.trackIdentityAssignment.findMany({
      where: { analysisRunId: input.analysisRunId, rosterEntryId: input.rosterEntryId, trackId: { not: input.trackId } },
      select: { trackId: true, track: { select: { firstFrame: true, lastFrame: true } } },
    })
    const replacedTrackIds = occupied.filter(item => frameRangesOverlap(track, item.track)).map(item => item.trackId)
    if (replacedTrackIds.length) await tx.trackIdentityAssignment.deleteMany({ where: { analysisRunId: input.analysisRunId, rosterEntryId: input.rosterEntryId, trackId: { in: replacedTrackIds } } })
    let decision
    try {
      decision = await applyManualReidDecision(tx, {
        ...
        userId: input.userId,
        position: { setNumber: track.analysisRun.submission.rally.set.setNumber, rallyOrdinal: track.analysisRun.submission.rally.ordinal },
        mode: identityMode,
        replacedTrackIds,
      })
    } catch (error) {
  ...
  return database.$transaction(async (tx) => {
    ...
    if (input.completed) {
      ...
      if (unassignedFeature) {
        throw new GraphQLError('仍有辨識到的球員軌跡尚未完成指派', { extensions: { code: 'REID_MANUAL_ASSIGNMENT_REQUIRED', trackId: unassignedFeature.trackId } })
      }
    }
