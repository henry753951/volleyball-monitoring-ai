...
import { AnalysisReviewStatus, JobStatus, Prisma, UserRole } from '@volleyball-monitoring/db/client'
    ...
    select: {
      ...
      activeSubmission: {
        select: {
          ...
          analysisRuns: { where: { status: JobStatus.COMPLETED, ...(publishedOnly ? { reviewStatus: AnalysisReviewStatus.APPROVED } : {}) }, orderBy: { activatedAt: 'desc' }, take: 1, select: replayAnalysisSelect },
          supersedes: { select: {
            clipJobs: { where: { status: JobStatus.COMPLETED, clipAssetId: { not: null } }, orderBy: { completedAt: 'desc' }, take: 1, select: replayClipSelect },
            analysisRuns: { where: { status: JobStatus.COMPLETED, ...(publishedOnly ? { reviewStatus: AnalysisReviewStatus.APPROVED } : {}) }, orderBy: { activatedAt: 'desc' }, take: 1, select: replayAnalysisSelect },
          } },
