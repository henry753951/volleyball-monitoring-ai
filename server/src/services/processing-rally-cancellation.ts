...
export class ProcessingCancellationError extends Error {
  constructor(public readonly code: ProcessingCancellationErrorCode, message: string) {
    super(message)
...
export async function cancelProcessingRally(
  ...
  if (!ALLOWED_ROLES.has(identity.role)) {
    throw new ProcessingCancellationError('FORBIDDEN', 'Processing cancellation requires annotation access')
  }
  return database.$transaction(async (tx) => {
    await tx.$queryRaw`SELECT pg_advisory_xact_lock(hashtextextended(${`processing-cancel:${rallyId}`}, 0))::text AS lock`
    const device = await tx.deviceSession.findFirst({ where: { id: identity.deviceSessionId, userId: identity.userId, revokedAt: null }, select: { id: true } })
    if (!device) throw new ProcessingCancellationError('UNAUTHENTICATED', 'Authenticated device session is not active')
    ...
    if (!rally || rally.voidedAt) throw new ProcessingCancellationError('NOT_FOUND', 'Processing rally was not found')
    const authorized = identity.role === UserRole.ADMIN || await tx.matchMember.findFirst({
      where: { matchId: rally.matchId, userId: identity.userId, role: { in: [...ALLOWED_ROLES] } },
      select: { userId: true },
    })
    if (!authorized) throw new ProcessingCancellationError('FORBIDDEN', 'Rally is outside the current annotation scope')
    if (!CANCELLABLE.has(rally.processingStatus) || !rally.activeSubmission) {
      throw new ProcessingCancellationError('INVALID_PROCESSING_STATE', 'Only an active processing rally can be deleted')
    }
    ...
    const currentState = await tx.rally.findUnique({ where: { id: rally.id }, select: { processingStatus: true, voidedAt: true } })
    if (!currentState || currentState.voidedAt || !CANCELLABLE.has(currentState.processingStatus)) {
      throw new ProcessingCancellationError('INVALID_PROCESSING_STATE', 'Processing completed before cancellation could be committed')
    }
    if (source.supersedesSubmissionId && (!source.supersedes || source.supersedes.status !== 'SUPERSEDED' || source.supersedes.rallyId !== rally.id)) {
    ...
    if (scoreAfter.left < 0 || scoreAfter.right < 0) {
      throw new ProcessingCancellationError('SCORE_CONFLICT', 'Score reversal would produce a negative score')
    }

    const now = new Date()
    const cancellationSubmissionId = randomUUID()
    const cancellationRevision = rally.annotationRevision + 1n
    const contentHash = createHash('sha256').update(`cancel:${source.id}:${cancellationRevision}`).digest('hex')
    ...
    if (scoreChanged) {
      ...
      if (updated.count !== 1) throw new ProcessingCancellationError('SCORE_CONFLICT', 'Set score changed while cancelling rally')
    ...
    if (restored) await tx.rallySubmission.update({ where: { id: restored.id }, data: { status: 'ACTIVE' } })
    await tx.clipJob.updateMany({ where: { submissionId: source.id, status: { in: [JobStatus.QUEUED, JobStatus.RUNNING] } }, data: { status: JobStatus.CANCELLED, leasedUntil: null, completedAt: now, errorCode: 'PROCESSING_CANCELLED', errorMessage: 'processing rally deleted by annotator' } })
    await tx.aiJob.updateMany({ where: { submissionId: source.id, status: { in: [JobStatus.QUEUED, JobStatus.RUNNING] } }, data: { status: JobStatus.CANCELLED, cancelRequestedAt: now, leasedUntil: null, completedAt: now, callbackTokenExpiresAt: now, errorCode: 'PROCESSING_CANCELLED', errorMessage: 'processing rally deleted by annotator' } })
    await tx.analysisRun.updateMany({ where: { submissionId: source.id, status: { in: [JobStatus.QUEUED, JobStatus.RUNNING] } }, data: { status: JobStatus.CANCELLED } })
    ...
    for (const aiJob of source.aiJobs) {
      await tx.outboxEvent.create({
        data: {
          ...
          payload: json({ ai_job_id: aiJob.id, rally_id: rally.id, reason: 'processing_rally_deleted' }),
        },
      })
    }
    await tx.outboxEvent.create({
