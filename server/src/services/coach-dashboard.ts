        ...
        select: {
          ...
          activeSubmission: {
            select: {
              ...
              clipJobs: { orderBy: { createdAt: 'desc' }, take: 1, select: { id: true, status: true } },
              analysisRuns: { orderBy: { createdAt: 'desc' }, take: 1, select: { id: true, status: true, analysisVersion: true, summary: true } },
            },
          },
        },
      },
    ...
    match: {
      ...
      rallies: match.rallies.flatMap(rally => rally.activeSubmission ? [{
        ...
        submission: {
          ...
          clip: rally.activeSubmission.clipJobs[0] ? { id: rally.activeSubmission.clipJobs[0].id, status: rally.activeSubmission.clipJobs[0].status.toLowerCase() } : null,
          analysis: rally.activeSubmission.analysisRuns[0] ? { id: rally.activeSubmission.analysisRuns[0].id, status: rally.activeSubmission.analysisRuns[0].status.toLowerCase(), version: rally.activeSubmission.analysisRuns[0].analysisVersion, summary: rally.activeSubmission.analysisRuns[0].summary } : null,
        },
      }] : []),
    },
  }
