...
export async function updateRallyDisplayPlacement(
  ...
  const result = await dependencies.database.$transaction(
    async tx => {
      ...
      const setExists = await tx.matchSet.count({
        where: { matchId: authorized.matchId, setNumber: input.setNumber },
      })
...
async function purgeRallyWithMedia(
  ...
  const authorized = await authorizeRally(dependencies.database, actor, rallyId)
  const transactionResult = await dependencies.database.$transaction(
    async tx => {
      await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${authorized.matchId}))`
      ...
      const obsoleteAggregateIds = [
        rally.id,
        ...submissionIds,
        ...clipJobIds,
        ...aiJobs.map(job => job.id),
        ...analysisRunIds,
      ]
      if (obsoleteAggregateIds.length)
        await tx.outboxEvent.deleteMany({ where: { aggregateId: { in: obsoleteAggregateIds } } })
  ...
  const retained = assets.length
    ? new Set(
        (
          await dependencies.database.mediaAsset.findMany({
            select: { id: true },
  ...
  if (dependencies.objectRemover) {
    for (const asset of removedAssets) {
      try {
        await dependencies.objectRemover(asset)
      } catch {
        cleanupWarnings.push(`物件儲存清理失敗：${asset.bucket}/${asset.objectKey}`)
      }
    }
