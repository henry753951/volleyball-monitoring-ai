...
function createMinioSigner() {
  ...
  if (!accessKey || !secretKey)
    throw new Error('MinIO credentials are required for AI job delivery')
  return new Client({
...
export function compatible(capabilities: AIProviderCapabilitiesPayload) {
  return (
    capabilities.supported_job_schema_versions.includes(AI_JOB_SCHEMA_VERSION) &&
    capabilities.supported_analysis_data_versions.includes(ANALYSIS_DATA_VERSION) &&
    REQUIRED_ANALYSIS_MODULES.every(module =>
      capabilities.supported_analysis_modules.includes(module),
    )
  )
}
      ...
      const updated = await tx.aiJob.updateMany({
        ...
        data: {
          ...
          errorMessage: terminal
            ? 'AI Worker stopped renewing the execution lease before every attempt completed'
            : null,
          availableAt: now,
        },
      })
...
export const aiProviderWebSocketRoutes =
  (dependencies: AiProviderWebSocketDependencies): FastifyPluginAsync =>
  async app => {
    ...
    const transportPingIntervalMs =
      dependencies.transportPingIntervalMs ?? heartbeatIntervalSeconds * 1_000
    const transportPingTimeoutMs = Math.max(transportPingIntervalMs * 3, 1_000)

    app.get('/api/v1/ai/providers/ws', { websocket: true }, (socket, request) => {
      ...
      void (async () => {
        ...
        const authenticated = await authenticateAiWorkerToken(database, presentedToken)
        if (!authenticated) {
              ...
              if (rallyIds.length > 0) {
                await tx.rally.updateMany({
                  where: {
                    ...
                    voidedAt: null,
                    processingStatus: {
                      in: [ProcessingStatus.AI_QUEUED, ProcessingStatus.AI_PROCESSING],
                    },
                  },
        ...
        const reconcile = async () => {
          ...
          try {
              ...
              const changed = await database.$transaction(async tx => {
                const updated = await tx.aiJob.updateMany({
                  ...
                  data: {
                    ...
                    errorMessage:
                      'AI Worker did not accept the job before every delivery lease expired',
                  },
                })
                if (updated.count !== 1) return false
                await tx.rally.updateMany({
            ...
            for (const event of durableAbortEvents) {
              ...
              const aiJobId =
                typeof event.payload.ai_job_id === 'string' ? event.payload.ai_job_id : null
              const deliveryId =
                typeof event.payload.delivery_id === 'string' ? event.payload.delivery_id : null
            ...
            for (const job of assigned) {
              ...
              if (job.status === JobStatus.CANCELLED && !abortSent.has(job.id)) {
                send({
                  ...
                  reason: 'processing clip deleted or cancelled',
                })
              ...
              if (job.status === JobStatus.COMPLETED && !committedSent.has(job.id)) {
                send({
                  ...
                  ai_job_id: job.id,
                  delivery_id: job.deliveryId,
                  committed_at: (job.completedAt ?? now).toISOString(),
                })
                committedSent.add(job.id)
              }
            }

            const activeDeliveries = assigned.filter(job =>
              isActiveProviderDelivery(job, now),
            ).length
            for (let slot = activeDeliveries; slot < maxConcurrency; slot += 1) {
              const compatibleSchemas = Prisma.join(supportedJobSchemaVersions)
              const nextCompatibleRows = await database.$queryRaw<
                Array<{ jobSchemaVersion: string }>
              >`
              ...
              const leastBusyInstanceId = await findLeastBusyProviderInstanceId(
                database,
                nextCompatibleJob.jobSchemaVersion,
              )
              ...
              if (!job) break
              if (
                !job.clipJob.clipAsset ||
                !job.clipJob.clipAsset.sha256 ||
                job.clipJob.clipAsset.byteLength === null ||
                !isRecord(job.requestPayload) ||
                !isRecord(job.requestPayload.clip)
              ) {
                await database.aiJob.update({
                  where: { id: job.id },
                  data: {
                    status: JobStatus.FAILED,
                    errorCode: 'AI_INPUT_NOT_READY',
                    errorMessage: 'canonical clip or request payload is unavailable',
              ...
              const downloadUrl = await presign(
                job.clipJob.clipAsset.bucket,
                job.clipJob.clipAsset.objectKey,
                expiresSeconds,
              )
              const payload = {
                ...job.requestPayload,
        ...
        const interval = setInterval(() => {
          void reconcile().catch(error =>
            request.log.error({ error }, 'AI provider reconciliation failed'),
          )
        }, tickMs)
        const latencyInterval = setInterval(() => {
          ...
          if (
            ...
            Date.now() - transportPingSentAt > transportPingTimeoutMs
          ) {
          ...
          void database.aiProviderInstance
            ...
            .catch(error =>
              request.log.warn({ error, instanceId }, 'AI provider ping persistence failed'),
            )
        ...
        socket.on('pong', () => {
          ...
          void database.aiProviderInstance
            ...
            .catch(error =>
              request.log.warn({ error, instanceId }, 'AI provider pong persistence failed'),
            )
        })
        socket.on('close', () => {
          ...
          if (instanceId)
            void database.aiProviderInstance
              .update({ where: { id: instanceId }, data: { disconnectedAt: new Date() } })
              .catch(() => undefined)
        })
        ...
        handleMessage = raw => {
          messageQueue = messageQueue
            .then(async () => {
              let message
              try {
                message = parseAIProviderClientMessage(JSON.parse(raw.toString()))
              } catch {
                send({
                  schema_version: '1.0.0',
                  type: 'protocol_error',
                  code: 'INVALID_MESSAGE',
                  message: 'message failed Provider Realtime 1.0.0 validation',
                  retryable: false,
                })
              ...
              if (message.type === 'provider_hello') {
                ...
                if (
                  ...
                  !compatible(message.capabilities)
                ) {
                  send({
                    schema_version: '1.0.0',
                    type: 'protocol_error',
                    ...
                    message:
                      'provider capabilities are incompatible with volleyball-analysis-engine',
                    retryable: false,
                  })
                ...
                supportedJobSchemaVersions =
                  message.capabilities.supported_job_schema_versions.filter(
                    version => version === AI_JOB_SCHEMA_VERSION,
                  )
                const instance = await database.aiProviderInstance.upsert({
                  ...
                  update: {
                    ...
                    providerBuildId: message.provider_build_id,
                    capabilities: json(message.capabilities),
                    maxConcurrency,
                    lastSeenAt: new Date(),
              ...
              if (!owned) {
                send({
                  schema_version: '1.0.0',
                  type: 'discard_job',
                  ai_job_id: message.ai_job_id,
                  delivery_id: message.delivery_id,
                  reason: 'delivery is no longer owned by this worker instance',
                })
                return
              }
              ...
              if (owned.status !== JobStatus.QUEUED && owned.status !== JobStatus.RUNNING) {
                send({
                  ...
                  ai_job_id: message.ai_job_id,
                  delivery_id: message.delivery_id,
                  reason: `central job is ${owned.status.toLowerCase()}`,
                })
                return
              }
              if (message.type === 'job_accepted') {
                ...
                const expectedStatus =
                  message.type === 'job_rejected' ? JobStatus.QUEUED : JobStatus.RUNNING
                const changed = await database.$transaction(async tx => {
                  const updated = await tx.aiJob.updateMany({
                    where: {
                      id: owned.id,
                    ...
                    data: {
                      ...
                      errorMessage: message.message.slice(0, 500),
        ...
        async function resumeOrStop(active: AIProviderActiveJob) {
          ...
          const job = await database.aiJob.findUnique({
            where: { id: active.ai_job_id },
            select: { status: true, providerInstanceId: true, deliveryId: true },
          })
          if (
            !job ||
            job.providerInstanceId !== instanceId ||
            job.deliveryId !== active.delivery_id
          ) {
            send({
              schema_version: '1.0.0',
              type: 'discard_job',
              ai_job_id: active.ai_job_id,
              delivery_id: active.delivery_id,
              reason: 'delivery is unknown or reassigned',
            })
            return
          }
          ...
          if (job.status !== JobStatus.RUNNING) {
            send({
              ...
              ai_job_id: active.ai_job_id,
              delivery_id: active.delivery_id,
              reason: `central job is ${job.status.toLowerCase()}`,
            })
            return
          }
          const leaseExpiresAt = new Date(Date.now() + leaseMs)
          ...
          if (resumed.count !== 1) {
            send({
              ...
              ai_job_id: active.ai_job_id,
              delivery_id: active.delivery_id,
              reason: 'central job changed while resuming',
            })
            return
          }
          send({
