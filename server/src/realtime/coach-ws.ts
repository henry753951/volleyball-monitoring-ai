...
export interface CoachMatchInvalidation {
  ...
  reason:
    ...
    | 'analysis_state_updated'
    | 'identity_mapping_updated'
    | 'rally_deleted'
    | 'rally_placement_updated'
...
function parsePing(value: unknown): PingMessage | null {
  ...
  if (
    message.type !== 'ping' ||
    typeof message.client_time_ms !== 'number' ||
    !Number.isFinite(message.client_time_ms)
  )
    return null
...
export const coachWebSocketRoutes =
  (deps: CoachWebSocketDependencies): FastifyPluginAsync =>
  async app => {
      ...
      void (async () => {
        ...
        socket.on('message', raw => {
          ...
          try {
            ...
            socket.close(1003, 'invalid coach message')
            return
          }
          const ping = parsePing(value)
          if (!ping) {
            socket.close(1003, 'unsupported coach message')
            return
          }
          socket.send(
            JSON.stringify({
