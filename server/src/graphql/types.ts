...
export const ScoreResolutionStateType = builder.enumType(ScoreResolutionState, {
  name: 'ScoreResolutionState',
})
  ...
  fields: t => ({
    ...
    name: t.string({
      resolve: async entry => {
        ...
        return (
          player?.name ?? domainError('Roster entry player is unavailable', 'INTERNAL_SERVER_ERROR')
        )
      },
    }),
...
RallyType.implement({
  fields: t => ({
    ...
    annotationStatus: t.field({
      type: AnnotationStatusType,
    ...
    processingStatus: t.field({
      ...
      resolve: rally => rally.processingStatus,
    }),
    scoreResolutionState: t.field({
      type: ScoreResolutionStateType,
      resolve: rally => rally.scoreResolutionState,
    }),
    scoringCourtSide: t.field({
      type: CourtSideType,
      nullable: true,
      resolve: rally => rally.scoringCourtSide,
    }),
...
async function teamsForMatch(matchId: string): Promise<Team[]> {
  const initialAssignment = await db.courtSideAssignment.findFirst({
    orderBy: [{ set: { setNumber: 'asc' } }, { effectiveFromRallyOrdinal: 'asc' }],
    where: { set: { matchId } },
  })
...
MatchType.implement({
  fields: t => ({
    ...
    captureSessions: t.field({
      type: [CaptureSessionType],
      resolve: match => listCaptureSessionsForMatch(match.id),
    }),
    rosterEntries: t.field({
      type: [MatchRosterEntryType],
      resolve: match =>
        db.matchRosterEntry.findMany({
          orderBy: [{ createdAt: 'asc' }, { id: 'asc' }],
...
export const CaptureTimelineRangeType =
  builder.objectRef<CaptureTimelineRangeView>('CaptureTimelineRange')
