  ...
  fields: t => ({
    ...
    deleteMatch: t.field({
      ...
      resolve: (_root, args, context) =>
        requestMatchDeletion(requireIdentity(context), args.matchId, {
          database: db,
        }),
    ...
    stopCapture: t.field({
      ...
      resolve: async (_root, args, context) => {
        const stopped = await operational(() =>
          stopCapture(db, requireIdentity(context), args.captureSessionId),
        )
    ...
    updateMatchClipPolicy: t.field({
      ...
      resolve: (_root, args, context) =>
        updateMatchClipPolicy(requireIdentity(context), args.input),
      type: MatchType,
    }),
