...
import {
  ...
  ingestFixedRosterReid,
  parseFixedRosterReidExtension,
} from '../services/fixed-roster-reid.js'
...
const callbackSchema = JSON.parse(
  await readFile(new URL('callback.schema.json', contractsRoot), 'utf8'),
)
...
function storage() {
  ...
  return {
    ...
    bucket: process.env.MINIO_ANALYSIS_BUCKET ?? 'analysis-artifacts',
  }
}
...
function invariantError(result: Record<string, unknown>, job: Awaited<ReturnType<typeof loadJob>>) {
  ...
  for (let index = 0; index < events.length; index += 1) {
    ...
    if (
      ...
      typeof event.key_point_id !== 'string' ||
      seenIds.has(event.key_point_id)
    )
...
function analysisDataInvariantError(
...
export const aiCallbackRoutesWithDependencies =
  (dependencies: AiCallbackRouteDependencies = {}): FastifyPluginAsync =>
  async app => {
    async function notifyCoach(matchId: string) {
      try {
        await dependencies.onAnalysisStateChanged?.(matchId)
      } catch (error) {
        app.log.warn({ error, matchId }, 'Coach analysis invalidation failed')
      }
    }

    app.post<{ Params: { aiJobId: string } }>(
      '/api/v1/ai/callback/:aiJobId',
      async (request, reply) => {
        ...
        try {
          ...
          if (request.isMultipart()) {
            ...
            for await (const part of request.parts({
              ...
              else if (part.type === 'file' && part.fieldname === 'analysis_data') {
                analysisDataPath = join(directory, 'analysis-data.fb')
                analysisDataInfo = await writeBounded(part.file, analysisDataPath, maximumBytes)
              } else if (part.type === 'file') part.file.resume()
            }
          } else {
            metadata = request.body
          }
          if (!validateCallback(metadata) || !isRecord(metadata) || metadata.ai_job_id !== aiJobId)
            return reject(
              reply,
              422,
              'INVALID_CALLBACK',
              'Callback metadata failed schema or job validation',
            )
          const kind = String(metadata.kind)
          if (kind === 'completed' && (!analysisDataPath || !analysisDataInfo))
            return reject(
              reply,
              422,
              'INVALID_CALLBACK',
              'Completed callback requires one AnalysisData part',
            )

          const payloadHash = sha256(
            `${JSON.stringify(metadata)}:${analysisDataInfo?.sha256 ?? ''}`,
          )
          const existing = await db.aiCallbackReceipt.findUnique({
            where: { callbackId: String(metadata.callback_id) },
          })
          if (existing) {
            if (existing.aiJobId !== aiJobId || existing.payloadHash !== payloadHash)
              return reject(
                reply,
                409,
          ...
          if (kind === 'processing') {
            ...
            await db.$transaction([
              db.aiCallbackReceipt.create({
                data: {
                  aiJobId,
                  callbackId: String(metadata.callback_id),
          ...
          if (
            !analysisDataPath ||
            !analysisDataInfo ||
            analysisDataInfo.sha256 !== String(metadata.analysis_data_sha256).toLowerCase() ||
            analysisDataInfo.bytes !== Number(metadata.analysis_data_bytes)
          )
            return reject(
              reply,
              422,
              'CHECKSUM_MISMATCH',
              'Completed callback AnalysisData checksum or length mismatch',
            )
          const analysisDataBytes = await readFile(analysisDataPath)
          if (
            analysisDataInfo.bytes < 16 ||
            analysisDataBytes.subarray(4, 8).toString('ascii') !== 'VAD1'
          )
            return reject(
              reply,
              415,
              'INVALID_ANALYSIS_DATA',
              'AnalysisData is not a VAD1 FlatBuffer',
            )
          let analysisData: AnalysisData
          try {
            analysisData = parseAnalysisData(analysisDataBytes)
          } catch {
            return reject(
              reply,
              415,
          ...
          let result: unknown
          try {
            result = JSON.parse(analysisData.domainJson) as unknown
          } catch {
            return reject(
              reply,
              422,
              'INVALID_ANALYSIS_DATA',
              'AnalysisData domain payload is not valid JSON',
            )
          }
          if (!validateAnalysisDataDomain(result) || !isRecord(result))
            return reject(
              reply,
              422,
              'INVALID_ANALYSIS_DATA',
              'AnalysisData domain payload failed schema validation',
            )
          const invariant = invariantError(result, job)
          if (invariant) return reject(reply, 409, 'PASSTHROUGH_MISMATCH', invariant)
          let reidFeatureBank
          try {
            reidFeatureBank = parseFixedRosterReidExtension(result)
          } catch (error) {
          ...
          if (analysisDataInvariant)
            return reject(reply, 409, 'PASSTHROUGH_MISMATCH', analysisDataInvariant)
          const analysisFrameChunkCount = Number(process.env.ANALYSIS_FRAME_CHUNK_COUNT ?? 120)
          if (
            !Number.isSafeInteger(analysisFrameChunkCount) ||
            analysisFrameChunkCount < 1 ||
            analysisFrameChunkCount > 3_600
          )
            return reject(
              reply,
              503,
              'ANALYSIS_DATA_CONFIGURATION_INVALID',
              'AnalysisData chunk configuration is invalid',
            )
          const browserChunks = chunkAnalysisData(analysisData, analysisFrameChunkCount).map(
            chunk => {
          ...
          await objectStore.client.fPutObject(
            objectStore.bucket,
            analysisDataKey,
            analysisDataPath,
            {
              'Content-Type': 'application/vnd.volleyball.analysis-data+flatbuffers;version=1',
              'x-amz-meta-sha256': analysisDataInfo.sha256,
              'x-amz-meta-byte-length': String(analysisDataInfo.bytes),
              'x-amz-meta-artifact-kind': 'analysis-data',
            },
          )
          for (const item of browserChunks) {
            ...
            await objectStore.client.putObject(
              ...
              {
                'Content-Type':
                  'application/vnd.volleyball.analysis-frame-chunk+flatbuffers;version=1',
          ...
          await db.$transaction(async tx => {
            ...
            const current = await tx.aiJob.findUnique({
              ...
              select: {
                status: true,
                submission: { select: { rally: { select: { voidedAt: true } } } },
              },
            })
            if (
              !current ||
              current.status === JobStatus.CANCELLED ||
              current.status === JobStatus.SUPERSEDED ||
              current.submission.rally.voidedAt
            )
              throw new Error('AI_JOB_NOT_ACTIVE')
            ...
            const requestAnalysisPlan = isRecord(requestPayloadRoot.analysis_plan)
              ...
              : {}
            const preserveManualCorrections =
              requestAnalysisPlan.preserve_manual_corrections !== false
            ...
            const priorRun = preserveManualCorrections
              ...
              : null
            const rawAnalysisData = await tx.mediaAsset.create({
              data: {
                kind: MediaAssetKind.ANALYSIS_DATA,
                bucket: objectStore.bucket,
                objectKey: analysisDataKey,
                contentType: 'application/vnd.volleyball.analysis-data+flatbuffers;version=1',
                byteLength: BigInt(analysisDataInfo!.bytes),
                sha256: analysisDataInfo!.sha256,
                internalSchemaVersion: analysisDataSchemaVersion,
            ...
            const analysisRun = await tx.analysisRun.create({
              data: {
                ...
                analysisDataSchemaVersion,
                inputClipSha256: String(result.input_clip_sha256),
                producerName: String(producer.name),
                producerBuildId: String(producer.build_id),
                producerSdkVersion:
                  typeof producer.sdk_version === 'string' ? producer.sdk_version : null,
                status: JobStatus.COMPLETED,
                rawAnalysisDataAssetId: rawAnalysisData.id,
            ...
            await tx.analysisDataManifest.create({
              data: {
                ...
                chunkFrameCount: analysisFrameChunkCount,
            ...
            if (tracks.length)
              await tx.analysisTrack.createMany({
                data: tracks.map(track => ({
                  ...
                  meanConfidence:
                    typeof track.mean_confidence === 'number' ? track.mean_confidence : null,
            ...
            for (const event of records(result.contact_events)) {
              ...
              const claimedSourceId =
                typeof event.source_key_point_id === 'string'
                  ? event.source_key_point_id
                  : keyPointId
              ...
              if (!mapping && !generated)
                throw new Error('AnalysisData human contact has no immutable clip mapping')
              const anchorFrameIndex = BigInt(String(event.anchor_frame_index))
              const anchorTimeUs =
                mapping?.clipTimeUs ?? (anchorFrameIndex * 1_000_000n * fpsDen) / fpsNum
              ...
              await tx.contactEvent.create({
                data: {
                  ...
                  detectionConfidence:
                    typeof event.detection_confidence === 'number'
                  ...
                  anchorFrameIndex,
                  resolvedFrameIndex:
                    event.resolved_frame_index == null
                      ? null
                  ...
                  associationState: String(
                    event.association_state,
                  ).toUpperCase() as AssociationState,
                  ballState: String(ball.state).toUpperCase() as BallObservationState,
                  ballFrameIndex:
                    ball.sample_frame_index == null
                      ? null
                      : BigInt(String(ball.sample_frame_index)),
                  ballFrameX:
                  ...
                  ballFrameY:
                    isRecord(ball.frame_pos) && typeof ball.frame_pos.y === 'number'
                      ? ball.frame_pos.y
                      : null,
                  qualityFlags: Array.isArray(event.quality_flags)
                    ? event.quality_flags.map(String)
                    : [],
                },
              })
                ...
                await tx.contactEventPosition.createMany({
                  data: positions.map((position, positionIndex) => {
                    ...
                    return {
                      ...
                      confidence:
                        typeof position.confidence === 'number' ? position.confidence : null,
                    }
                  }),
                })
            }
            for (const path of records(result.path_segments)) {
              const segment = await tx.ballPathSegment.create({
                data: {
                  ...
                  startFrameIndex:
                    path.start_frame_index === undefined
                      ? null
                      : BigInt(String(path.start_frame_index)),
                  endFrameIndex:
                    path.end_frame_index === undefined
                      ? null
                      : BigInt(String(path.end_frame_index)),
                  renderState: String(path.render_state).toUpperCase() as SegmentRenderState,
                  isTerminalSegment: Boolean(path.is_terminal_segment),
                  qualityFlags: Array.isArray(path.quality_flags)
                    ? path.quality_flags.map(String)
                    : [],
                },
              })
              const endpoints = [
            ...
            if (priorRun) {
              const validKeyPointIds = new Set(
                records(result.contact_events).map(event => String(event.key_point_id)),
              )
              ...
              const identityAssignments = priorRun.tracks
                .flatMap(track => track.identityAssignments)
                .filter(item => validTrackIds.has(item.trackId))
              if (identityAssignments.length)
                await tx.trackIdentityAssignment.createMany({
                  data: identityAssignments.map(item => ({
                    analysisRunId: analysisRun.id,
                    trackId: item.trackId,
                    ...
                    reidIdentityId: item.reidIdentityId,
                    reidBindingId: item.reidBindingId,
            ...
            await tx.aiCallbackReceipt.create({
              data: {
                aiJobId,
                callbackId: String(metadata.callback_id),
            ...
            if (job.submission.supersedesSubmissionId) {
              await Promise.all([
                tx.clipJob.updateMany({
                  where: { submissionId: job.submission.supersedesSubmissionId },
                  data: { status: JobStatus.SUPERSEDED, leasedUntil: null },
                }),
                tx.aiJob.updateMany({
                  where: { submissionId: job.submission.supersedesSubmissionId },
                  data: { status: JobStatus.SUPERSEDED, leasedUntil: null },
                ...
                tx.analysisRun.updateMany({
                  where: { submissionId: job.submission.supersedesSubmissionId },
                  data: { status: JobStatus.SUPERSEDED },
                }),
              ])
            }
          ...
          if (
            ...
            return reject(
              ...
              413,
              'PAYLOAD_TOO_LARGE',
              'Callback payload exceeds the configured limit',
            )
          if (error instanceof Error && error.message === 'AI_JOB_NOT_ACTIVE')
            return reject(reply, 409, 'JOB_NOT_ACTIVE', 'AI job was cancelled or superseded')
          if (error instanceof FixedRosterReidError) {
