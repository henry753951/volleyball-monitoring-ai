...
export function analysisMediaRoutesWithDependencies(dependencies: {
  timingManifestReader: MediaObjectReader
}): FastifyPluginAsync {
  return async app => {
    const storage = client()
    app.get<{ Params: { rallyId: string }; Querystring: { clipJobId?: string } }>(
      '/api/v1/analysis/rallies/:rallyId/clip',
      async (request, reply) => {
        if (!UUID.test(request.params.rallyId)) return reply.status(404).send({ code: 'NOT_FOUND' })
        if (request.query.clipJobId && !UUID.test(request.query.clipJobId))
        ...
        const clip = await db.clipJob.findFirst({
          where: {
            ...(request.query.clipJobId ? { id: request.query.clipJobId } : {}),
            status: JobStatus.COMPLETED,
            submission: {
              rally: {
                id: request.params.rallyId,
                voidedAt: null,
    ...
    app.get<{ Params: { previewId: string } }>(
      '/api/v1/reid/previews/:previewId',
      async (request, reply) => {
        if (!UUID.test(request.params.previewId))
          return reply.status(404).send({ code: 'NOT_FOUND' })
        const identity = await authenticateDevelopmentAnnotationRequest(request, db)
        if (!identity) return reply.status(401).send({ code: 'UNAUTHENTICATED' })
        const preview = await db.reidIdentityPreview.findFirst({
          where: {
            id: request.params.previewId,
            status: ArtifactState.READY,
            mediaAsset: { state: ArtifactState.READY, deletedAt: null },
            tracklet: {
              evidenceSet: {
                analysisRun: {
                  submission: {
        ...
        const asset = preview?.mediaAsset
        if (
          !asset?.byteLength ||
          !asset.sha256 ||
          asset.byteLength > BigInt(Number.MAX_SAFE_INTEGER)
    ...
    app.get<{ Params: { analysisRunId: string } }>(
      ...
      async (request, reply) => {
        if (!UUID.test(request.params.analysisRunId))
          return reply.status(404).send({ code: 'NOT_FOUND' })
        const identity = await authenticateDevelopmentAnnotationRequest(request, db)
        if (!identity) return reply.status(401).send({ code: 'UNAUTHENTICATED' })
        ...
        if (!run.rawAnalysisDataAsset || !run.aiJob.clipJob.timingManifest) {
          return reply.status(409).send({
            ...
            message: 'The completed analysis is missing AnalysisData or its timing manifest.',
          })
        }
        ...
        const assets = [
          ...
          ...(run.rawAnalysisDataAsset
            ? [
                {
                  name: 'analysis/analysis-data.vad1',
                  asset: run.rawAnalysisDataAsset,
                  store: true,
                },
              ]
            : []),
          ...(run.aiJob.clipJob.timingManifest
            ? [
                {
                  name: 'analysis/timing-manifest.json',
                  asset: run.aiJob.clipJob.timingManifest,
                  store: false,
                },
              ]
            : []),
          ...run.artifacts.map((entry, index) => ({
            name: `analysis/artifacts/${String(entry.kind).toLowerCase()}-${index + 1}${extname(entry.asset.objectKey)}`,
            asset: entry.asset,
            store: true,
          })),
        ].filter(
        ...
        const uniqueAssets = assets.filter(entry =>
          seenAssets.has(entry.asset.id) ? false : (seenAssets.add(entry.asset.id), true),
        )
        const analysisDataBytes = await readObject(
          storage,
          run.rawAnalysisDataAsset.bucket,
          run.rawAnalysisDataAsset.objectKey,
          512 * 1024 * 1024,
        )
        ...
        try {
          ...
          request.log.warn(
            { error, analysisRunId: run.id },
            'ML dataset timing manifest is invalid',
          )
          return reply.status(409).send({
            code: 'DATASET_TIMING_INVALID',
            message: 'The canonical frame timeline could not be verified.',
          })
        }
        ...
        const review = {
          ...
          ball_corrections: run.ballCorrections.map(item =>
            item.visible
              ? {
          ...
          action_corrections: run.actionCorrections.map(item => ({
            frame_index: item.frameIndex.toString(),
            track_id: item.trackId,
            action: item.action,
            revision: item.revision.toString(),
          })),
          player_bbox_corrections: run.playerBBoxCorrections.map(item => ({
            frame_index: item.frameIndex.toString(),
            track_id: item.trackId,
            frame_bbox: { x1: item.frameX1, y1: item.frameY1, x2: item.frameX2, y2: item.frameY2 },
            revision: item.revision.toString(),
          })),
          contact_actor_corrections: run.contactActorCorrections.map(item => ({
            key_point_id: item.keyPointId,
            track_id: item.trackId,
            revision: item.revision.toString(),
          })),
          contact_time_corrections: run.contactTimeCorrections.map(item => ({
            key_point_id: item.keyPointId,
            frame_index: item.frameIndex.toString(),
            revision: item.revision.toString(),
          })),
        }
        const identityAssignments = run.tracks.flatMap(track =>
          track.identityAssignments.map(assignment => ({
            ...
            player: {
              ...
              name:
                assignment.rosterEntry.displayNameSnapshot ??
                assignment.rosterEntry.player?.name ??
                null,
        ...
        const generated: GeneratedDatasetFile[] = [
          ...
          },
          {
            ...
            description:
              'Persisted inference request with callback delivery credentials omitted or redacted.',
          },
          {
            ...
            bytes: jsonBytes({
              ...
              video:
                isRecord(run.aiJob.requestPayload) && isRecord(run.aiJob.requestPayload.clip)
            ...
            contentType: 'application/json',
            description:
              'Canonical clip, source capture range, cut policy, video properties, and immutable asset checksum.',
          },
          {
            ...
            bytes: jsonBytes({
              ...
              analysis_run: {
                ...
                producer: {
                  ...
                  build_id: run.producerBuildId,
                  sdk_version: run.producerSdkVersion,
                },
                summary: run.summary,
          ...
          },
          {
            ...
            bytes: jsonLines(
              run.aiJob.callbackReceipts.map(receipt => ({
          ...
          },
          {
            ...
            bytes: jsonLines([
              ...
              ...review.contact_actor_corrections.map(value => ({
                kind: 'contact_actor',
                ...value,
              })),
            ...
            contentType: 'application/x-ndjson',
            description:
              'Complete sparse current human review state; values override predictions at the same key.',
          },
          {
            path: 'labels/identity-assignments.jsonl',
            bytes: jsonLines(
              identityAssignments.map(assignment => ({ schema_version: '1.0.0', ...assignment })),
            ),
            contentType: 'application/x-ndjson',
            ...
            : []),
          {
            ...
            bytes: jsonBytes({
              schema_version: '1.0.0',
              tracks: run.tracks.map(
                ({ identityAssignments: _identityAssignments, ...track }) => track,
              ),
            ...
            contentType: 'application/json',
            description:
              'Normalized relational view ingested from the raw AI result with correction links.',
          },
        ...
        const manifest = {
          ...
          integrity: {
            algorithm: 'sha256',
            scope:
              'Every entry in files; manifest.json is excluded to avoid a self-referential checksum.',
          },
          coverage: {
            all_persisted_inference_outputs: true,
            raw_authoritative_files: [
              'analysis/analysis-data.vad1',
              'analysis/timing-manifest.json',
            ],
            generated_ml_tables: generated
            ...
            excluded_transient_worker_state: [
              'gpu_tensors',
              'feature_maps',
              'per_frame_detection_embeddings',
              'process_memory',
              'worker_local_debug_previews',
              'worker_logs',
            ],
          },
          files: [
            ...uniqueAssets.map(entry => ({
              path: entry.name,
              byte_length: entry.asset.byteLength?.toString() ?? null,
              sha256: entry.asset.sha256,
              content_type: entry.asset.contentType,
              generated: false,
            })),
            ...generated.map(entry => ({
              path: entry.path,
              byte_length: String(entry.bytes.byteLength),
        ...
        archive.on('warning', error =>
          request.log.warn({ error, analysisRunId: run.id }, 'dataset zip warning'),
        )
        archive.on('error', error =>
          request.log.error({ error, analysisRunId: run.id }, 'dataset zip failed'),
        )
        ...
        for (const entry of uniqueAssets) {
          archive.append(
            entry.asset.id === run.rawAnalysisDataAssetId
              ? analysisDataBytes
              : await storage.getObject(entry.asset.bucket, entry.asset.objectKey),
            { name: entry.name, store: entry.store },
          )
        }
        const baseName = `${run.submission.rally.match.title}-${run.submission.rally.id}`
        ...
        reply
          ...
          .header(
            'Content-Disposition',
            `attachment; filename*=UTF-8''${encodeURIComponent(`${baseName}-dataset.zip`)}`,
          )
    ...
    app.get<{ Params: { analysisRunId: string } }>(
      '/api/v1/analysis-runs/:analysisRunId/analysis-data-manifest',
      async (request, reply) => {
        if (!UUID.test(request.params.analysisRunId))
          return reply.status(404).send({ code: 'NOT_FOUND' })
        const identity = await authenticateDevelopmentAnnotationRequest(request, db)
        ...
        const manifest = await db.analysisDataManifest.findFirst({
          where: {
            analysisRunId: request.params.analysisRunId,
            analysisRun: {
              status: JobStatus.COMPLETED,
              submission: {
                rally: {
          ...
          include: {
            chunks: {
              where: { asset: { state: ArtifactState.READY, deletedAt: null } },
              include: { asset: { select: { contentType: true } } },
        ...
        if (clipJob.timingManifest) {
          try {
            ...
            if (BigInt(timeline.clipTimeUs.length) !== manifest.totalFrames)
              throw new Error('AnalysisData and timing manifest frame counts differ')
            frameTiming = {
            ...
            request.log.warn(
              { error, analysisRunId: manifest.analysisRunId },
              'Exact AnalysisData frame timeline is unavailable',
            )
          }
        }
        return reply.header('Cache-Control', 'private, no-store').send({
          ...
          video: {
            ...
            fps: { num: manifest.fpsNum, den: manifest.fpsDen },
            total_frames: manifest.totalFrames.toString(),
          },
          frame_timing: frameTiming,
          chunk_frame_count: manifest.chunkFrameCount,
          chunks: manifest.chunks.map(chunk => ({
            chunk_index: chunk.chunkIndex,
            start_frame_index: chunk.startFrameIndex.toString(),
            frame_count: chunk.frameCount,
            url: `/api/v1/analysis-runs/${manifest.analysisRunId}/analysis-frame-chunks/${chunk.chunkIndex}`,
            byte_length: chunk.byteLength.toString(),
            sha256: chunk.sha256,
          })),
    ...
    app.get<{ Params: { analysisRunId: string; chunkIndex: string } }>(
      '/api/v1/analysis-runs/:analysisRunId/analysis-frame-chunks/:chunkIndex',
      async (request, reply) => {
        if (
          !UUID.test(request.params.analysisRunId) ||
          !/^(0|[1-9][0-9]*)$/.test(request.params.chunkIndex)
        )
          return reply.status(404).send({ code: 'NOT_FOUND' })
        ...
        const chunk = await db.analysisFrameChunk.findFirst({
          where: {
            ...
            asset: { state: ArtifactState.READY, deletedAt: null },
          },
        ...
        return reply
          ...
          .header('Content-Length', chunk.byteLength.toString())
          .header('ETag', `"${chunk.sha256}"`)
          .type(chunk.asset.contentType)
          .send(await storage.getObject(chunk.asset.bucket, chunk.asset.objectKey))
      },
    )
  }
}
