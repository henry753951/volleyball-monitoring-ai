...
import {
  ...
  canReadAnalysisReview,
  readAnalysisReview,
  recalculateAnalysisReview,
} from '../services/analysis-review.js'
...
function broadcast(analysisRunId: string, revision: string) {
  const message: AnalysisReviewRevisionEvent = {
  ...
  for (const socket of sockets.get(analysisRunId) ?? [])
...
export const analysisReviewRoutesWithDependencies =
  (dependencies: AnalysisReviewRouteDependencies = {}): FastifyPluginAsync =>
  async app => {
    ...
    app.get<{ Params: { analysisRunId: string }; Querystring: { after_revision?: string } }>(
      '/api/v1/analysis-runs/:analysisRunId/review',
      async (request, reply) => {
        if (
          !UUID.test(request.params.analysisRunId) ||
          (request.query.after_revision !== undefined &&
            !/^(0|[1-9][0-9]*)$/.test(request.query.after_revision))
        )
        ...
        const state = await readAnalysisReview(db, {
          analysisRunId: request.params.analysisRunId,
          ...(request.query.after_revision === undefined
            ? {}
            : { afterRevision: BigInt(request.query.after_revision) }),
          identity,
        })
        return state
          ? reply.header('Cache-Control', 'private, no-store').send(state)
          : reply.status(404).send({ code: 'NOT_FOUND' })
      },
    )

    app.patch<{ Params: { analysisRunId: string } }>(
      '/api/v1/analysis-runs/:analysisRunId/review',
      async (request, reply) => {
        ...
        if (!identity) return reply.status(401).send({ code: 'UNAUTHENTICATED' })
        try {
          const result = await applyAnalysisReviewPatch(db, {
            analysisRunId: request.params.analysisRunId,
            patch: parseAnalysisReviewPatch(request.body),
            identity,
          })
          ...
          if (error instanceof AnalysisReviewError)
            return reply
              .status(error.code === 'FORBIDDEN' ? 403 : error.code === 'NOT_FOUND' ? 404 : 409)
    ...
    app.get<{ Params: { analysisRunId: string } }>(
      '/ws/analysis-reviews/:analysisRunId',
      { websocket: true },
      (socket, request) => {
        void (async () => {
          const analysisRunId = request.params.analysisRunId
          if (!UUID.test(analysisRunId)) return socket.close(1008, 'invalid analysis run')
          const identity = await authenticateDevelopmentAnnotationRequest(request, db).catch(
            () => null,
          )
          if (!identity || !(await canReadAnalysisReview(db, analysisRunId, identity)))
            return socket.close(1008, 'analysis review unavailable')
          const peers = sockets.get(analysisRunId) ?? new Set<ReviewSocket>()
          peers.add(socket)
          sockets.set(analysisRunId, peers)
          socket.on('close', () => {
            peers.delete(socket)
            if (!peers.size) sockets.delete(analysisRunId)
          })
    ...
    for (const action of ['recalculate', 'approve'] as const) {
      app.post<{ Params: { analysisRunId: string } }>(
        `/api/v1/analysis-runs/:analysisRunId/review/${action}`,
        async (request, reply) => {
          if (!UUID.test(request.params.analysisRunId))
            return reply.status(400).send({ code: 'BAD_REQUEST' })
          const identity = await authenticateDevelopmentAnnotationRequest(request, db)
          ...
          try {
            const result =
              action === 'recalculate'
                ? await recalculateAnalysisReview(db, {
                    analysisRunId: request.params.analysisRunId,
                    identity,
                  })
                : await approveAnalysisReview(db, {
                    analysisRunId: request.params.analysisRunId,
                    identity,
                  })
            broadcast(request.params.analysisRunId, result.revision)
