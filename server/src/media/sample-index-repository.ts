...
function validateSegmentIds(segmentIds: readonly string[]): string[] {
  if (
    segmentIds.length === 0
    || segmentIds.length > MAX_SAMPLE_INDEX_SEGMENT_IDS
  ) {
    repositoryFailure(
      'INVALID_REQUEST',
      'Sample index segment request size is invalid',
    )
  }
  ...
  for (const segmentId of segmentIds) {
    if (!UUID.test(segmentId)) {
      repositoryFailure(
        'INVALID_REQUEST',
        'Sample index segment identity is invalid',
      )
    }
    ...
    if (uniqueIds.has(normalizedId)) {
      repositoryFailure(
        'INVALID_REQUEST',
        'Sample index segment identities must be unique',
      )
    }
...
function orderedRows(
  ...
  if (rowsById.size !== segmentIds.length) {
    repositoryFailure(
      'SEGMENT_NOT_FOUND',
      'A requested sample index segment was not found',
    )
  }
...
function validateSegmentRow(row: SegmentRow): void {
  if (row.isGap || row.readyAt === null) {
    repositoryFailure(
      'SEGMENT_NOT_READY',
      'A requested sample index segment is not ready',
    )
  }
  if (
...
function sampleIndexReadRequest(row: SegmentRow): MediaObjectReadRequest {
  ...
  if (
    ...
    repositoryFailure(
      'SAMPLE_INDEX_ASSET_NOT_READY',
      'A sample index asset is not ready',
    )
  }
  return {
    bucket: asset.bucket,
...
function validateIndexAgainstSegment(
  row: SegmentRow,
  index: SampleIndex,
): void {
...
function validateResolvedSegmentSet(segments: readonly IndexedSegment[]): void {
  try {
    ...
    repositoryFailure(
      'INVALID_SEGMENT_SET',
      'Sample indexes are not one contiguous resolver set',
    )
  }
}
...
export class SampleIndexRepository {
  ...
  async loadOrderedSegments(
    ...
    try {
      ...
      repositoryFailure(
        'DATABASE_READ_FAILED',
        'Sample index metadata could not be read',
      )
    }
    ...
    for (const row of ordered) {
      ...
      try {
        ...
        repositoryFailure(
          'OBJECT_READ_FAILED',
          'Sample index object could not be read',
        )
      }
