...
export interface MediaObjectLocation { bucket: string; objectKey: string }
...
function clientOptions(endpointValue: string, accessKey: string, secretKey: string): ClientOptions {
  ...
  if (!['http:', 'https:'].includes(endpoint.protocol) || !endpoint.hostname) throw new Error('MinIO endpoint is invalid')
  return {
    accessKey,
...
export function createMediaObjectRemoverFromEnv(environment: NodeJS.ProcessEnv = process.env): MediaObjectRemover | undefined {
  ...
  if (!endpoint || !accessKey || !secretKey) throw new Error('MinIO cleanup configuration is incomplete')
  const client = new Client(clientOptions(endpoint, accessKey, secretKey))
  return async ({ bucket, objectKey }) => {
    try { await client.removeObject(bucket, objectKey) }
    catch (error) {
