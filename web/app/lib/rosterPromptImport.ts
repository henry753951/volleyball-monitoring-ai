import type { Match, RosterInput } from './coreDomain'

export const ROSTER_IMPORT_SCHEMA = 'vollyai.roster-import.v1' as const
...
const PLAYER_KEYS = ['jerseyNumber', 'name']
...
export function buildRosterResearchPrompt(match: RosterPromptMatch) {
  ...
  return [
    '你是一位嚴謹的排球賽事資料研究員。請調查下列比賽兩隊的正式參賽球員名單與球衣背號。',
    '',
    `比賽：${match.title}`,
    `場地：${match.venue ?? '未提供'}`,
    ...
    '2. 不要猜測；無法可靠確認的球員不要加入。姓名保留官方慣用拼寫。',
    ...
    '',
    '每位球員格式必須是：{"jerseyNumber":"字串","name":"字串"}',
  ].join('\n')
}
...
export function parseRosterImportPaste(value: string, match: RosterPromptMatch): RosterImportParseResult {
  ...
  for (const rawTeam of parsed.teams) {
    ...
    for (const rawPlayer of rawTeam.players) {
      if (!isRecord(rawPlayer) || !hasOnlyKeys(rawPlayer, PLAYER_KEYS)) return invalid('球員資料只能包含 jerseyNumber 與 name。')
      if (typeof rawPlayer.jerseyNumber !== 'string' || typeof rawPlayer.name !== 'string') return invalid('球員背號與姓名必須是字串。')
      ...
      players.push({ jerseyNumber, name })
    }
