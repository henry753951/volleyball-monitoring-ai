...
export function createRealtimeReconnectScheduler(
  ...
  if (typeof document !== 'undefined') document.addEventListener('visibilitychange', handleAvailabilityChange)
  ...
  return {
    ...
    dispose() {
      ...
      waitingForAvailability = false
      clearTimer()
      if (typeof window !== 'undefined') window.removeEventListener('online', handleAvailabilityChange)
      if (typeof document !== 'undefined') document.removeEventListener('visibilitychange', handleAvailabilityChange)
    },
    schedule() {
