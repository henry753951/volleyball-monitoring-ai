...
export interface CoachDraft {
  ...
  boundaries?: Array<{ kind: 'start' | 'end'; capture_time_us: string; capture_frame_index: string }>
}
...
export interface ReplayGlobalIdentity { id: string; label: string; source: 'ai' | 'manual' | 'propagated'; confidence: number | null; identity_revision: string | null }
export interface CoachRallyReplay {
  ...
  analysis: { id: string; analysis_id: string; version: string; review_revision?: string; producer: { name: string; build_id: string }; summary: unknown; tracks: Array<{ track_id: number; court_side: string; first_frame_index: string; last_frame_index: string; mean_confidence: number | null; global_identity?: ReplayGlobalIdentity | null; identity: ReplayTrackIdentity | null }>; contact_events: ReplayContactEvent[]; paths: ReplayPath[] } | null
}
...
export interface CoachMatchAnalytics {
  ...
  sets: Array<{ set_number: number; rally_count: number; resolved_count: number; unknown_count: number; team_points: Record<string, number> }>
  rallies: Array<{ id: string; set_number: number; ordinal: number; score_resolution: string; scoring_team_id: string | null; contact_count: number; replay_url: string }>
  players: Array<{ roster_entry_id: string; team_id: string; jersey_number: string; position: RosterPosition; name: string; contact_count: number; sample_count: number; rally_count: number; action_counts: Record<string, number>; heatmap_samples: Array<{ x: number; y: number; rally_id: string; set_number: number; action: string | null }>; error_count: number | null }>
  tracks: Array<{
...
export function createCoachDomainClient(transport: GraphQLTransport) {
  return {
    ...
    async assignTrackIdentity(input: { analysisRunId: string; trackId: number; rosterEntryId: string; identityMode?: 'from_here' | 'clip_only' | 'split_identity' }) {
      return transport.request<{ assignTrackIdentity: { schema_version: '1.0.0' } }>('mutation AssignTrackIdentity($analysisRunId: ID!, $trackId: Int!, $rosterEntryId: ID!, $identityMode: String) { assignTrackIdentity(analysisRunId: $analysisRunId, trackId: $trackId, rosterEntryId: $rosterEntryId, identityMode: $identityMode) }', input)
    },
