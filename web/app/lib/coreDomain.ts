...
export const CORE_OPERATIONS = {
  ...
  startCapture: `mutation StartCapture($input: StartCaptureInput!) { startCapture(input: $input) { id ingestPath matchId sourceKind sourceLabel sourceDurationUs status health startedAt endedAt timeline { captureSessionId captureStartTimeUs liveEdgeCaptureTimeUs timelineVersion availabilityComplete ingestFrontierCaptureTimeUs sourceEndCaptureTimeUs availableRanges { startUs endUs discontinuity } gapRanges { startUs endUs discontinuity } } } }`,
  stopCapture: `mutation StopCapture($captureSessionId: ID!) { stopCapture(captureSessionId: $captureSessionId) { id ingestPath matchId sourceKind sourceLabel sourceDurationUs status health startedAt endedAt timeline { captureSessionId captureStartTimeUs liveEdgeCaptureTimeUs timelineVersion availabilityComplete ingestFrontierCaptureTimeUs sourceEndCaptureTimeUs availableRanges { startUs endUs discontinuity } gapRanges { startUs endUs discontinuity } } } }`,
