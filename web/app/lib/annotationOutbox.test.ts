...
import { enqueueAnnotationCommand, markAnnotationOutboxAttempted, readAnnotationOutbox, requireAnnotationOutboxConfirmation, resolveAnnotationOutboxEntry, writeAnnotationOutbox } from './annotationOutbox'

const room = 'match:00000000-0000-4000-8000-000000000001:capture:00000000-0000-4000-8000-000000000002'
const command = { schema_version: '2.0.0', command_id: '00000000-0000-4000-8000-000000000003', room_id: room, base_revision: '0', rally_id: '00000000-0000-4000-8000-000000000004', kind: 'CREATE_SERVICE_KEY_POINT', payload: { playback_cursor: { playback_window_id: 'window-1', mapping_version: 1, player_media_time_us: '0', observation_source: 'current_time_fallback', presented_frames: null, seek_generation: 0, cursor_status: 'ready' } } } as const
class MemoryStorage {
...
describe('annotation local outbox', () => {
  ...
  it('marks conflicts for confirmation and removes acknowledged entries', () => {
    ...
    expect(requireAnnotationOutboxConfirmation(entries, command.command_id, 'revision changed')[0]).toMatchObject({ status: 'needs_confirmation', reason: 'revision changed' })
  ...
  it('fails closed on malformed or cross-room storage', () => {
    const storage = new MemoryStorage()
    storage.setItem(`volleyball-monitoring-ai:annotation-outbox:1:${room}`, JSON.stringify({ version: 1, entries: [{ command: { ...command, room_id: `${room}-other` }, queued_at: new Date().toISOString(), status: 'pending', reason: null }] }))
