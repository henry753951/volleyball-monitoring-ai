...
export function findAvailableRange(
  timeUs: bigint | string,
  ranges: readonly CaptureTimelineRange[],
) {
  const t = BigInt(timeUs)
  return ranges.find(r => t >= BigInt(r.startUs) && t < BigInt(r.endUs)) ?? null
}
...
export function canSeekCaptureTime(
  timeUs: bigint | string,
  ranges: readonly CaptureTimelineRange[],
): boolean {
...
export function availableBounds(ranges: readonly CaptureTimelineRange[]) {
  ...
  return {
    startUs: ranges.reduce((a, r) => (BigInt(a.startUs) < BigInt(r.startUs) ? a : r)).startUs,
    endUs: ranges.reduce((a, r) => (BigInt(a.endUs) > BigInt(r.endUs) ? a : r)).endUs,
  }
}
