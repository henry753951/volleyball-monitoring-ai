...
describe('match media source client', () => {
  it('starts a YouTube source with same-origin credentials', async () => {
    const fetcher = vi.fn<typeof fetch>(async () => new Response(JSON.stringify({ capture_session_id: 'capture-1' }), { status: 202 }))
    await createMediaSourceClient({ fetcher }).create('match-1', {
      kind: 'youtube', label: '主場轉播', url: 'https://www.youtube.com/watch?v=NMTbgYfa-ZM',
    })
    ...
    expect(JSON.parse(String(fetcher.mock.calls[0]?.[1]?.body))).toEqual({
      match_id: 'match-1', source_label: '主場轉播', source_url: 'https://www.youtube.com/watch?v=NMTbgYfa-ZM',
    })
  })

  it('uploads MP4 through multipart without setting a content-type boundary', async () => {
    const fetcher = vi.fn<typeof fetch>(async () => new Response(JSON.stringify({ capture_session_id: 'capture-2' }), { status: 202 }))
    await createMediaSourceClient({ fetcher }).create('match-2', {
      kind: 'local_mp4', label: '完整賽事', file: new File(['video'], 'match.mp4', { type: 'video/mp4' }),
    })
  ...
  it('does not issue a request when setup is deferred', async () => {
    ...
    await expect(createMediaSourceClient({ fetcher }).create('match-3', { kind: 'later' })).resolves.toBeNull()
