  ...
  const timelineSegments = computed(() => {
    ...
    const submitted = submittedRallies.value.flatMap((rally) => {
      ...
      return [{
        ...
        status: failed ? 'failed' as const : processingCompleted && analysis?.status === 'completed' ? analysis.identity_mapping_completed ? 'mapped' as const : 'analyzed' as const : 'processing' as const,
    ...
    const drafts = annotationDrafts.value.flatMap((draft) => {
      ...
      return [{
        id: draft.id,
        label: `第 ${draft.display_set_number} 局 · 回合 ${draft.display_ordinal}`,
