...
class FakeKeyboardEvent extends Event {
   ...
   constructor(
      ...
      code: string,
      modifiers: Partial<
         Pick<FakeKeyboardEvent, "altKey" | "ctrlKey" | "metaKey" | "shiftKey">
      > = {},
      eventType = "keydown",
   ) {
   ...
   override composedPath(): EventTarget[] {
      return [this.target].filter((target): target is EventTarget =>
         Boolean(target),
      );
   }
}
...
describe("annotation TanStack runtime adapter", () => {
   ...
   it("accepts unshifted scoring punctuation and Ctrl frame acceleration", async () => {
      ...
      keyup(scope, "/", "Slash");
      expect(
         keydown(scope, "ArrowLeft", "ArrowLeft", { ctrlKey: true })
            .defaultPrevented,
      ).toBe(true);
   ...
   it("records and normalizes a shifted product key, then removes its listener on unmount", async () => {
         ...
         defineComponent({
            setup() {
               const recorder = useHotkeyRecorder({
                  onRecord: (hotkey) => {
                     const normalized = normalizeRecordedHotkey(
                        hotkey,
                        "windows",
                     );
      ...
      document.dispatchEvent(
         new FakeKeyboardEvent("<", "Comma", { shiftKey: true }),
      );
