...
function createFakeRenderer(document: FakeDocument) {
  return createRenderer<FakeNode, FakeElement>({
    ...
    createElement: tag =>
      tag === 'input' ? new FakeInputElement(tag, document) : new FakeElement(tag, document),
...
describe('annotation TanStack runtime adapter', () => {
  ...
  it('does not lose the first press after a command becomes enabled', async () => {
    ...
    expect(blocked).toEqual(['contact'])
    keyup(scope, 'x', 'KeyX')
    enabled.value = true
    await nextTick()
    expect(keydown(scope, 'x', 'KeyX').defaultPrevented).toBe(true)
    ...
    const app = renderer.createApp(
      defineComponent({
        setup() {
          ...
          useHotkeys(
            () =>
              createAnnotationHotkeyDefinitions(restoreDefaultHotkeys(), (command, event) =>
                calls.push({
                  command,
                  ctrl: event.ctrlKey,
  ...
  it('reports frame-navigation release separately from keyboard repeat', async () => {
    const pressed: HotkeyCommand[] = []
    const released: HotkeyCommand[] = []
    const renderer = createFakeRenderer(document)
    const root = new FakeElement('root', document)
    const app = renderer.createApp(
      defineComponent({
        setup() {
          ...
          useHotkeys(
            () =>
              createAnnotationHotkeyDefinitions(restoreDefaultHotkeys(), command =>
                pressed.push(command),
              ),
            {
              target: scope,
              conflictBehavior: 'error',
          ...
          useHotkeys(
            () =>
              createAnnotationHotkeyReleaseDefinitions(restoreDefaultHotkeys(), command =>
    ...
    await nextTick()
    const scope = root.children[0] as FakeElement
    keydown(scope, 'ArrowRight', 'ArrowRight')
    keydown(scope, 'ArrowRight', 'ArrowRight')
    expect(released).toEqual([])
    keyup(scope, 'ArrowRight', 'ArrowRight')

    expect(pressed).toEqual(['frame_next', 'frame_next'])
    expect(released).toEqual(['frame_next'])
    app.unmount()
  })
