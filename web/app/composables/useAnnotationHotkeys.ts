...
export type HotkeyCommandDispatcher = (
...
export function useAnnotationHotkeys() {
   const bindings = useState<HotkeyBindings>(
      "annotation-hotkeys-v5",
      restoreDefaultHotkeys,
   );
   const initialized = useState(
      "annotation-hotkeys-v5-initialized",
      () => false,
   );
   ...
   onMounted(() => {
      ...
      const legacy = current
         ? null
         : localStorage.getItem(LEGACY_ANNOTATION_HOTKEYS_STORAGE_KEY);
      ...
      if (legacy)
         localStorage.removeItem(LEGACY_ANNOTATION_HOTKEYS_STORAGE_KEY);
   ...
   watch(
      ...
      (value) => {
         if (import.meta.client && initialized.value) {
            localStorage.setItem(
               HOTKEY_PREFERENCES_STORAGE_KEY,
               serializeHotkeyPreferences(value),
            );
         }
      },
      { deep: true },
...
export function createAnnotationHotkeyDefinitions(
   ...
   return HOTKEY_COMMANDS.flatMap((command) => {
      const repeatable =
         command.action === "frame_previous" || command.action === "frame_next";
      const definition = (
...
export function createAnnotationHotkeyReleaseDefinitions(
   ...
   return HOTKEY_COMMANDS.flatMap((command) => {
      if (
         command.action !== "frame_previous" &&
         command.action !== "frame_next"
      )
         return [];
      ...
      const accelerated =
         typeof runtime === "string"
            ...
            : { ...runtime, ctrl: true };
      return [
         ...runtimes.map(releaseDefinition),
         releaseDefinition(accelerated),
      ];
   });
}
 ...
 */
export function useAnnotationHotkeyRuntime(
   options: AnnotationHotkeyRuntimeOptions,
): void {
