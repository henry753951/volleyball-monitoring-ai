...
export function useCoalescedFrameNavigation<T>(
   ...
   function settled() {
      return (
         !running.value && pendingDelta.value === 0 && heldDirections.size === 0
      );
   }
   ...
   function enqueue(
      ...
      generation += 1;
      pendingDelta.value = Math.max(
         -maxDelta,
         Math.min(maxDelta, pendingDelta.value + delta),
      );
      updateActive();
   ...
   async function flush(): Promise<void> {
      ...
      const direction: FrameNavigationDirection =
      ...
      try {
         ...
         running.value = false;
         updateActive();
         if (!stopped && pendingDelta.value !== 0 && heldDirections.size === 0)
            scheduleFlush(0);
         else notifySettled();
      }
   }
