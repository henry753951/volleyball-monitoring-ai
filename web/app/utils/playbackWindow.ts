...
export function isCaptureTimeWithinWindow(
  ...
  return (
    capture >= BigInt(descriptor.window_capture_start_us) &&
...
export function captureTimeToPlayerSeconds(
  ...
  descriptor: Pick<
    ...
    'window_capture_start_us' | 'window_capture_end_us' | 'presentation_origin_capture_us'
  >,
): number {
...
export function isPlayerMediaTimeWithinWindow(
  ...
  const player =
    typeof playerMediaTimeUs === 'bigint' ? playerMediaTimeUs : BigInt(playerMediaTimeUs)
  if (player < 0n) return false
  return isCaptureTimeWithinWindow(
    BigInt(descriptor.presentation_origin_capture_us) + player,
    descriptor,
  )
}
