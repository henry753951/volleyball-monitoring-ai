...
const chunk: AnalysisFrameChunk = {
  ...
  courtPositions: [{ x: 0.25, y: 0.75 }],
  playerFlags: [
    ANALYSIS_PLAYER_FLAG.frameBBox |
      ANALYSIS_PLAYER_FLAG.frameFootPosition |
      ANALYSIS_PLAYER_FLAG.courtPosition,
  ],
...
const event: ReplayContactEvent = {
  ...
  marker_kind: 'contact',
  is_terminal: false,
  anchor_frame_index: '12',
  resolved_frame_index: '12',
  anchor_time_us: '0',
  association_state: 'resolved',
  ball: { state: 'missing', frame_index: null, frame_pos: null },
  quality_flags: [],
  actors: [
    {
      track_id: 1,
      observation_frame_index: '12',
      association_confidence: 0.8,
      frame_bbox: { x1: 0.1, y1: 0.1, x2: 0.3, y2: 0.9 },
      frame_foot_pos: null,
      court_pos: null,
      action: null,
    },
    {
      ...
      frame_bbox: { x1: 0.65, y1: 0.1, x2: 0.85, y2: 0.9 },
      frame_foot_pos: null,
      court_pos: null,
      action: null,
    },
...
describe('volleyball overlay geometry', () => {
  it('maps the video into the actual letterboxed content rectangle', () => {
    expect(
      resolveVideoContentRect({ x: 0, y: 0, width: 1_000, height: 1_000 }, 1_920, 1_080),
    ).toEqual({
      x: 0,
      y: 218.75,
      width: 1_000,
  ...
  it('rejects annotation clicks in the letterbox and maps valid clicks to video pixels', () => {
    ...
    expect(overlayCanvasPointToVideo({ x: 500, y: 500 }, viewport, 1_920, 1_080)).toEqual({
      x: 960,
      y: 540,
    })
  })
  ...
  it('uses the last tracked ball when the contact frame is explicitly missing', () => {
    const ballChunk: AnalysisFrameChunk = {
      ...
      ballFlags: [ANALYSIS_BALL_FLAG.framePosition, 0, 0],
      ballConfidences: [240, 0, 0],
      courtKeypointFrameOffsets: [0, 0, 0, 0],
      courtKeypointIds: [],
      courtKeypointPositions: [],
      courtKeypointConfidences: [],
    }
    const position = resolveEffectiveHitPosition(
      {
        ballCorrections: { 12: { state: 'missing' } },
        chunk: ballChunk,
        videoWidth: 1_000,
        videoHeight: 500,
      },
      ...
      resolveEffectiveContactActor(
        {
          ballCorrections: {},
          chunk: null,
          contactActorCorrections: { [event.key_point_id]: null },
