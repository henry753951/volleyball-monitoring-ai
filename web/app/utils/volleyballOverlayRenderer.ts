...
export type OverlayBallOverride = { state: 'position'; position: OverlayPoint } | { state: 'missing' }
...
const TRACK_PALETTE = [
  '#7dd3fc', '#fda4af', '#fde68a', '#c4b5fd', '#86efac',
  '#fdba74', '#67e8f9', '#f9a8d4', '#93c5fd', '#bef264',
  '#f0abfc', '#a5b4fc', '#5eead4', '#fca5a5', '#d9f99d',
  '#cbd5e1', '#fed7aa', '#bae6fd', '#fbcfe8', '#bbf7d0',
] as const
...
export function resolveVideoContentRect(viewport: OverlayRect, videoWidth: number, videoHeight: number): OverlayRect {
  if (videoWidth <= 0 || videoHeight <= 0 || viewport.width <= 0 || viewport.height <= 0) return viewport
  const videoAspect = videoWidth / videoHeight
  const viewportAspect = viewport.width / viewport.height
  if (viewportAspect > videoAspect) {
    const width = viewport.height * videoAspect
    return { x: viewport.x + (viewport.width - width) / 2, y: viewport.y, width, height: viewport.height }
  }
  const height = viewport.width / videoAspect
  return { x: viewport.x, y: viewport.y + (viewport.height - height) / 2, width: viewport.width, height }
}

function quantizedPoint(position: OverlayPoint, content: OverlayRect): OverlayPoint {
  return { x: content.x + position.x / QUANTIZED_MAX * content.width, y: content.y + position.y / QUANTIZED_MAX * content.height }
}
...
function framePoint(position: OverlayPoint, content: OverlayRect, videoWidth: number, videoHeight: number): OverlayPoint {
  return { x: content.x + position.x / videoWidth * content.width, y: content.y + position.y / videoHeight * content.height }
}

function quantizedToFrame(position: OverlayPoint, videoWidth: number, videoHeight: number): OverlayPoint {
...
function collectDetections(chunk: BrowserOverlayChunk, frame: number, videoWidth: number, videoHeight: number, corrections?: Record<number, OverlayFrameBBox>): FrameDetection[] {
  ...
  for (let index = range.start; index < range.end; index += 1) {
    result.push({
      ...
      bbox: corrections?.[chunk.trackIds[index]!] ?? (() => {
...
function drawCornerBox(context: CanvasRenderingContext2D, topLeft: OverlayPoint, bottomRight: OverlayPoint, color: string) {
  const width = Math.max(1, bottomRight.x - topLeft.x)
  const height = Math.max(1, bottomRight.y - topLeft.y)
  const corner = Math.min(15, width * .22, height * .16)
  context.fillStyle = `${color}12`
  context.fillRect(topLeft.x, topLeft.y, width, height)
  context.strokeStyle = `${color}42`
  context.lineWidth = 1
  context.strokeRect(topLeft.x + .5, topLeft.y + .5, width - 1, height - 1)
  ...
  context.moveTo(bottomRight.x - corner, topLeft.y); context.lineTo(bottomRight.x, topLeft.y); context.lineTo(bottomRight.x, topLeft.y + corner)
  context.moveTo(bottomRight.x, bottomRight.y - corner); context.lineTo(bottomRight.x, bottomRight.y); context.lineTo(bottomRight.x - corner, bottomRight.y)
  context.moveTo(topLeft.x + corner, bottomRight.y); context.lineTo(topLeft.x, bottomRight.y); context.lineTo(topLeft.x, bottomRight.y - corner)
  context.stroke()
}

function drawSelectionBox(context: CanvasRenderingContext2D, topLeft: OverlayPoint, bottomRight: OverlayPoint) {
  context.save()
  context.strokeStyle = '#ffffff'
  context.lineWidth = 1.5
  context.setLineDash([5, 4])
  context.strokeRect(topLeft.x - 2, topLeft.y - 2, bottomRight.x - topLeft.x + 4, bottomRight.y - topLeft.y + 4)
  context.setLineDash([])
  for (const point of [topLeft, { x: bottomRight.x, y: topLeft.y }, bottomRight, { x: topLeft.x, y: bottomRight.y }]) {
    context.fillStyle = '#0b0d10'; context.fillRect(point.x - 3, point.y - 3, 6, 6)
    context.strokeStyle = '#ffffff'; context.strokeRect(point.x - 3, point.y - 3, 6, 6)
  }
  context.restore()
}

function drawPlayerLabel(context: CanvasRenderingContext2D, detection: FrameDetection, topLeft: OverlayPoint, bottomRight: OverlayPoint, color: string, input: VolleyballOverlayRenderInput, track?: OverlayTrackMetadata) {
  ...
  if (teamText || identityText) {
    ...
    const badgeY = below + 16 <= input.viewport.y + input.viewport.height ? below : bottomRight.y - 19
    const badgeX = Math.max(input.viewport.x + 3, Math.min((topLeft.x + bottomRight.x - badgeWidth) / 2, input.viewport.x + input.viewport.width - badgeWidth - 3))
    roundedRect(context, badgeX, badgeY, badgeWidth, 16, 4)
    context.fillStyle = 'rgba(7, 11, 16, .78)'; context.fill()
    context.strokeStyle = 'rgba(255,255,255,.12)'; context.lineWidth = 1; context.stroke()
    context.fillStyle = teamColor; context.fillRect(badgeX, badgeY, 2.5, 16)
    let cursorX = badgeX + 7
    if (teamText) {
      context.font = '800 8px Inter, ui-sans-serif, system-ui, sans-serif'
      context.fillStyle = teamColor; context.fillText(teamText, cursorX, badgeY + 11)
      cursorX += teamWidth - 3
      context.fillStyle = 'rgba(255,255,255,.22)'; context.fillRect(cursorX - 3, badgeY + 4, 1, 8)
    }
    if (identityText) {
      context.font = '750 9px Inter, ui-sans-serif, system-ui, sans-serif'
      context.fillStyle = '#f8fafc'; context.fillText(identityText, cursorX, badgeY + 11.3)
    }
  }
...
function drawCourtKeypoints(context: CanvasRenderingContext2D, input: VolleyballOverlayRenderInput, chunk: BrowserOverlayChunk, localFrame: number, content: OverlayRect) {
  ...
  for (const point of points.values()) {
    ...
    context.beginPath(); context.arc(point.x, point.y, 1.9, 0, Math.PI * 2)
    context.fillStyle = '#d7e8f4'; context.fill()
    context.globalAlpha = .36
    context.beginPath(); context.arc(point.x, point.y, 3.7, 0, Math.PI * 2)
    context.strokeStyle = '#173244'; context.lineWidth = 1; context.stroke()
  }
  context.restore()
}
...
function drawBallTrail(context: CanvasRenderingContext2D, input: VolleyballOverlayRenderInput, chunk: BrowserOverlayChunk, localFrame: number, content: OverlayRect) {
  ...
  for (let index = 1; index < points.length; index += 1) {
    const opacity = .08 + .6 * index / points.length
    context.beginPath()
...
function drawBallMarker(context: CanvasRenderingContext2D, position: OverlayPoint, corrected = false) {
  context.save()
  context.shadowColor = 'rgba(0, 0, 0, .55)'
  context.shadowBlur = 8
  context.beginPath(); context.arc(position.x, position.y, corrected ? 9 : 8, 0, Math.PI * 2)
  context.fillStyle = 'rgba(12, 16, 22, .72)'; context.fill()
  context.shadowBlur = 0
  context.beginPath(); context.arc(position.x, position.y, corrected ? 6 : 5.5, 0, Math.PI * 2)
  context.fillStyle = 'rgba(255, 211, 79, .16)'; context.fill()
  context.strokeStyle = corrected ? '#fff3b0' : BALL; context.lineWidth = 2.5; context.stroke()
  context.beginPath(); context.arc(position.x - 1.8, position.y - 2, 1.4, 0, Math.PI * 2)
  context.fillStyle = '#fff7cc'; context.fill()
  context.restore()
}
...
export function resolveEffectiveHitPosition(input: Pick<VolleyballOverlayRenderInput, 'ballCorrections' | 'chunk' | 'contactTimeCorrections' | 'videoHeight' | 'videoWidth'>, event: ReplayContactEvent) {
  ...
  if (!exactOverride) {
    ...
    if (raw) return raw
    if (event.ball.frame_pos) return { x: event.ball.frame_pos.x * input.videoWidth, y: event.ball.frame_pos.y * input.videoHeight }
  }
...
function contactPoint(bbox: OverlayFrameBBox) {
  return { x: (bbox.x1 + bbox.x2) / 2, y: bbox.y1 + (bbox.y2 - bbox.y1) * .34 }
}
...
export function resolveEventActorFromResult(event: ReplayContactEvent, position: OverlayPoint | null, videoWidth: number, videoHeight: number) {
  if (position) {
    let best: { trackId: number; distance: number } | null = null
...
function drawNextHit(context: CanvasRenderingContext2D, input: VolleyballOverlayRenderInput, content: OverlayRect) {
  ...
  context.save()
  context.strokeStyle = BALL; context.lineWidth = 3
  context.setLineDash([Math.max(5, radius * .35), Math.max(3, radius * .18)])
  context.lineDashOffset = remaining * .65
  context.beginPath(); context.arc(target.x, target.y, radius, 0, Math.PI * 2); context.stroke()
  context.setLineDash([])
  context.strokeStyle = `${color}aa`; context.lineWidth = 1.5
  context.beginPath(); context.arc(target.x, target.y, Math.max(10, radius - 6), 0, Math.PI * 2); context.stroke()
  ...
  context.fillStyle = remaining <= 1 ? BALL : 'rgba(10, 14, 20, .84)'; context.fill()
  context.fillStyle = remaining <= 1 ? '#171006' : '#fff5c2'; context.fillText(label, target.x + radius + 14, target.y + 3.5)
  context.restore()
}
...
function drawCourtRadar(context: CanvasRenderingContext2D, input: VolleyballOverlayRenderInput, detections: FrameDetection[], metadata: Map<number, OverlayTrackMetadata>) {
  ...
  context.fillStyle = 'rgba(7, 11, 16, .76)'; context.fill()
  ...
  context.strokeStyle = 'rgba(255, 255, 255, .68)'; context.lineWidth = 1
  context.strokeRect(court.x, court.y, court.width, court.height)
  context.beginPath(); context.moveTo(court.x + court.width / 2, court.y); context.lineTo(court.x + court.width / 2, court.y + court.height); context.stroke()
  context.setLineDash([3, 3]); context.strokeStyle = 'rgba(255, 255, 255, .28)'
  context.beginPath(); context.moveTo(court.x + court.width / 3, court.y); context.lineTo(court.x + court.width / 3, court.y + court.height); context.moveTo(court.x + court.width * 2 / 3, court.y); context.lineTo(court.x + court.width * 2 / 3, court.y + court.height); context.stroke(); context.setLineDash([])
  for (const player of players) {
    ...
    context.beginPath(); context.arc(px, py, 3.5, 0, Math.PI * 2); context.fillStyle = color; context.fill(); context.strokeStyle = '#0a0e14'; context.lineWidth = 1.5; context.stroke()
  }
  if (input.teamLabels) {
    context.font = '650 8px Inter, ui-sans-serif, system-ui, sans-serif'
    context.fillStyle = LEFT; context.fillText(input.teamLabels.left, court.x, y + height - 2)
    const rightWidth = context.measureText(input.teamLabels.right).width
    context.fillStyle = RIGHT; context.fillText(input.teamLabels.right, court.x + court.width - rightWidth, y + height - 2)
  }
  context.restore()
}
...
function drawEventFallback(context: CanvasRenderingContext2D, input: VolleyballOverlayRenderInput, content: OverlayRect) {
  ...
  if (input.layers.ball) {
    ...
    else if (input.ballCorrection?.state !== 'missing' && event.ball.frame_pos) drawBallMarker(context, normalizedPoint(event.ball.frame_pos, content))
  }
}
...
export function renderVolleyballOverlay(input: VolleyballOverlayRenderInput) {
  ...
  const detections = chunk ? collectDetections(chunk, input.frame, input.videoWidth, input.videoHeight, input.playerBBoxCorrections?.[input.frame]) : []
  ...
  if (!chunk || !range) drawEventFallback(context, input, content)
    ...
    for (const detection of detections) {
      ...
      if (layers.footprint && (detection.flags & OVERLAY_PLAYER_FLAG.frameFootPosition)) {
        ...
        context.beginPath(); context.ellipse(foot.x, foot.y, 9, 3.5, 0, 0, Math.PI * 2); context.fillStyle = `${color}55`; context.fill(); context.strokeStyle = color; context.lineWidth = 1; context.stroke()
      }
    }
  ...
  if (layers.court) drawCourtRadar(context, input, detections, metadata)
}

export function hitTestOverlayTrack(input: Pick<VolleyballOverlayRenderInput, 'chunk' | 'frame' | 'videoWidth' | 'videoHeight' | 'viewport' | 'playerBBoxCorrections'>, point: OverlayPoint) {
  if (!input.chunk) return null
  const content = resolveVideoContentRect(input.viewport, input.videoWidth, input.videoHeight)
  if (point.x < content.x || point.x > content.x + content.width || point.y < content.y || point.y > content.y + content.height) return null
...
export function overlayCanvasPointToVideo(point: OverlayPoint, viewport: OverlayRect, videoWidth: number, videoHeight: number) {
  const content = resolveVideoContentRect(viewport, videoWidth, videoHeight)
  if (point.x < content.x || point.x > content.x + content.width || point.y < content.y || point.y > content.y + content.height) return null
  return { x: (point.x - content.x) / content.width * videoWidth, y: (point.y - content.y) / content.height * videoHeight }
}
