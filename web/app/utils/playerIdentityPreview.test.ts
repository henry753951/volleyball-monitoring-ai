...
const track = (overrides: Record<string, unknown>) => ({
  analysis_run_id: 'run-current', track_id: 1, rally_id: 'rally-current', set_number: 1, rally_ordinal: 3,
  court_side: 'left', first_frame_index: '0', last_frame_index: '90', roster_entry_id: 'roster-1',
  identity_mapping_completed: false, ...overrides,
}) as never
...
describe('player identity preview', () => {
  it('selects the latest distinct clips and excludes the current analysis run', () => {
    const selected = selectPlayerPreviewTracks([
      ...
      track({ analysis_run_id: 'run-future', track_id: 7, rally_id: 'rally-future', rally_ordinal: 4 }),
      track({ analysis_run_id: 'other', track_id: 3, rally_id: 'other', roster_entry_id: 'roster-2' }),
    ], 'roster-1', { analysisRunId: 'run-current', trackId: 1 })
  ...
  it('samples three stable points inside a tracked frame range', () => {
    expect(previewFrameSeconds({ firstFrameIndex: '30', lastFrameIndex: '90', fps: { num: 30, den: 1 }, durationUs: '5000000' }))
      .toEqual([1.4, 2, 2.6])
  })

  it('clamps absolute frame indexes to the available clip instead of seeking past its end', () => {
    expect(previewFrameSeconds({ firstFrameIndex: '3000', lastFrameIndex: '3600', fps: { num: 60, den: 1 }, durationUs: '5000000' }))
      .toEqual([4.96])
  })
})
