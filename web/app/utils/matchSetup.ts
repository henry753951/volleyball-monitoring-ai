...
export function validateMatchSetup(input: CreateMatchSetupInput): string[] {
  ...
  if (!second!.name.trim() || !second!.shortName.trim())
    errors.push('請完整填寫參賽隊伍 2 的名稱。')
  ...
  for (const [index, roster] of input.teams.map(team => team.roster).entries()) {
    ...
    if (roster.some(row => !row.name.trim() || !row.jerseyNumber.trim()))
      errors.push(`${label} 的球員姓名與背號都必填。`)
    if (roster.some(row => row.position === 'UNSPECIFIED'))
      errors.push(`${label} 的球員位置都必須選擇。`)
  }
  return errors
}
