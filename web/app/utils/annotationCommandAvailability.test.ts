...
describe('draft annotation command availability', () => {
  ...
  it('allows X while a segment is open without using the rendered mask as a hard boundary', () => {
    expect(draftCommandAvailability({ ...openDraft, action: 'contact' })).toEqual({
      enabled: true,
      reason: '',
    })
  })
  ...
  it('keeps contact edits and outcome selection available after Z fixes the end boundary', () => {
    expect(draftCommandAvailability({ ...openDraft, action: 'contact', state: 'READY' })).toEqual({
      enabled: true,
      reason: '',
    })
    expect(
      draftCommandAvailability({ ...openDraft, action: 'close_right', state: 'READY' }),
    ).toEqual({ enabled: true, reason: '' })
  })
})
  ...
  it('blocks an end boundary when the resulting draft would overlap another segment', () => {
    expect(
      boundaryCommandAvailability({
        ...
        currentDraftCaptureTimes: ['10000000'],
        clipPreRollUs: 0n,
        clipPostRollUs: 0n,
        segments: [
          segment,
          { id: 'draft', startCaptureTimeUs: '10000000', endCaptureTimeUs: '25000000' },
        ],
      }),
  ...
  it('allows a correction draft to coexist with a new non-overlapping segment', () => {
    expect(
      boundaryCommandAvailability({
        ...
        canMark: true,
        cursorCaptureTimeUs: '40000000',
        currentRallyId: 'correction',
        currentDraftCaptureTimes: ['20000000', '30000000'],
        clipPreRollUs: 0n,
        clipPostRollUs: 0n,
        segments: [segment],
      }),
