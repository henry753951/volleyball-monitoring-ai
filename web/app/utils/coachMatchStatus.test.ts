...
function match(status: string, captureSessions: CaptureSession[] = []): Pick<Match, 'status' | 'captureSessions'> {
  return { status, captureSessions }
}

describe('coach match status', () => {
  it('only calls a healthy active capture live', () => {
    expect(coachMatchStatus(match('LIVE', [capture({ status: 'LIVE', health: 'HEALTHY' })]))).toEqual({ kind: 'live', label: '直播中' })
    expect(coachMatchStatus(match('LIVE', [capture({ status: 'LIVE', health: 'OFFLINE' })]))).toEqual({ kind: 'failed', label: '直播異常' })
  })

  it('distinguishes completed live and video sources', () => {
    expect(coachMatchStatus(match('LIVE', [capture({ sourceKind: 'youtube_live' })]))).toEqual({ kind: 'finished', label: '直播已結束' })
    expect(coachMatchStatus(match('LIVE', [capture({ sourceKind: 'youtube_vod' })]))).toEqual({ kind: 'ready', label: '影片已就緒' })
  })
  ...
  it('uses explicit lifecycle and recovery states', () => {
    ...
    expect(coachMatchStatus(match('LIVE', [capture({ status: 'FAILED' })]))).toEqual({ kind: 'failed', label: '影音失敗' })
