...
export function selectPlayerPreviewTracks(
  ...
  const current = excluded?.analysisRunId
    ? tracks.find(track => track.analysis_run_id === excluded.analysisRunId && (excluded.trackId === null || track.track_id === excluded.trackId))
  ...
  return tracks
    .filter(track =>
      ...
      && (!current || track.set_number < current.set_number || (track.set_number === current.set_number && track.rally_ordinal < current.rally_ordinal)),
    )
    .sort((left, right) =>
...
export function previewFrameSeconds(input: {
  ...
  return [0.2, 0.5, 0.8]
    ...
    .filter((value, index, values) => value >= 0 && (index === 0 || Math.abs(value - values[index - 1]!) >= 0.04))
}
