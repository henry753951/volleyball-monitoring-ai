...
import { CircleHelp, ListTree, LoaderCircle, RotateCcw, ShieldCheck, Sparkles, UserRoundCheck, UsersRound } from 'lucide-vue-next'
...
const gidGroups = computed(() => [
  { teamId: props.leftTeamId, label: props.teams.find(team => team.id === props.leftTeamId)?.name ?? '左隊' },
  { teamId: props.rightTeamId, label: props.teams.find(team => team.id === props.rightTeamId)?.name ?? '右隊' },
  ...
  rows: assignment.view.model.gidGroups
    ...
    .map(identity => ({
      ...identity,
      options: assignment.view.model.options.forTrack({ teamId: group.teamId, trackId: identity.representativeTrackId, trackIds: identity.trackIds }).filter(option => option.value),
    ...
    <template v-else>
      <div class="identity-toolbar">
        <div class="identity-view-switch" role="tablist" aria-label="球員分派顯示方式">
          <button type="button" role="tab" :aria-selected="displayMode === 'local'" :class="{ active: displayMode === 'local' }" @click="displayMode = 'local'"><ListTree :size="13" />Local 分派<b>{{ assignment.view.model.tracks.length }}</b></button>
          <button type="button" role="tab" :aria-selected="displayMode === 'gid'" :class="{ active: displayMode === 'gid' }" @click="displayMode = 'gid'"><UsersRound :size="13" />GID 分派<b>{{ assignment.view.model.gidGroups.length }}</b></button>
        </div>
        <button type="button" class="identity-auto" :disabled="assignment.view.busy || !assignment.view.model.gidGroups.length" @click="assignment.actions.applyAutomaticAssignments"><LoaderCircle v-if="assignment.state.autoAssigning" class="spin" :size="13" /><Sparkles v-else :size="13" />ReID 自動分配</button>
      </div>
      <p class="mapping-hint">{{ displayMode === 'local' ? '每個 Local ID 都保留獨立球員指派，可逐一修正辨識錯誤。' : '一個 GID 可包含多個 Local ID；在此選擇球員會批次寫回每個 Local ID。' }}</p>
      ...
      <template v-else>
        <p v-if="!gidGroups.length" class="identity-empty">此片段尚無可用的 GID，請改用 Local 分派。</p>
        <section v-for="group in gidGroups" :key="group.teamId ?? 'unknown'" class="gid-section">
          <header><span>{{ group.label }}</span><b>{{ group.rows.length }}/6</b></header>
          <label v-for="row in group.rows" :key="row.gidId" class="gid-row">
            <code><span>{{ row.gidLabel }}</span><i :class="{ active: row.active }">{{ row.active ? '畫面中' : '未出現' }}</i><small class="gid-tids"><span v-for="tid in row.tidLabels" :key="tid">{{ tid }}</span></small></code>
            <span class="identity-control">
              <span class="identity-state" :data-tone="row.status.tone"><ShieldCheck v-if="['manual', 'propagated'].includes(row.status.tone)" :size="11" /><CircleHelp v-else :size="11" />{{ row.status.label }}<small v-if="row.confidence != null">{{ Math.round(row.confidence * 100) }}%</small></span>
              <span class="identity-select">
                <UiPlayerCombobox :model-value="row.rosterEntryId ?? ''" :options="row.options" :disabled="assignment.state.savingTrackId === row.representativeTrackId" :aria-label="`依 ${row.gidLabel} 批次指派 ${row.trackIds.length} 個 Local ID`" @update:model-value="assignment.actions.requestGidAssignment({ trackId: row.representativeTrackId, trackIds: row.trackIds, rosterEntryId: $event })">
                  <template #preview="{ option }"><PlayerIdentityPreview v-if="assignment.view.model.players.byRosterEntry(option.value)" :match-id="matchId" :roster-entry-id="option.value" :player-name="assignment.view.model.players.byRosterEntry(option.value)!.name" :jersey-number="assignment.view.model.players.byRosterEntry(option.value)!.jersey_number" :tracks="assignment.state.analytics?.tracks ?? []" :analysis-run-id="analysisRunId" :track-id="row.representativeTrackId" /></template>
                </UiPlayerCombobox>
                <LoaderCircle v-if="assignment.state.savingTrackId === row.representativeTrackId" class="spin" :size="14" />
              </span>
            </span>
          </label>
        </section>
        <p v-if="assignment.view.model.ungroupedTrackCount" class="identity-required"><CircleHelp :size="13" />另有 {{ assignment.view.model.ungroupedTrackCount }} 個 Local ID 尚無 GID，請在 Local 分派中處理。</p>
      </template>
      <p v-if="assignment.state.automaticAssignmentResult" class="identity-auto-result" role="status">{{ assignment.state.automaticAssignmentResult }}</p>
...
.identity-toolbar{display:grid;gap:7px}.identity-view-switch{display:grid;grid-template-columns:1fr 1fr;padding:3px;border:1px solid #303840;border-radius:9px;background:#151a1f}.identity-view-switch button{min-height:32px!important;display:flex!important;align-items:center!important;justify-content:center!important;gap:6px!important;padding:4px 8px!important;border:0!important;border-radius:6px!important;background:transparent!important;color:#8f9aa4!important;font-size:.61rem!important}.identity-view-switch button.active{background:#28323a!important;color:#eef3f6!important;box-shadow:0 2px 8px #0005}.identity-view-switch button:focus-visible{outline:2px solid #9fc7eb;outline-offset:1px}.identity-view-switch b{min-width:18px;color:inherit;font-size:.53rem}.identity-auto{min-height:34px!important;display:flex!important;align-items:center!important;justify-content:center!important;gap:6px!important;border-color:#3d6553!important;background:#1b2c25!important;color:#aee6c8!important;font-size:.61rem!important}.identity-auto:disabled{opacity:.48}.identity-auto-result{margin:0;padding:8px 9px;border:1px solid #315344;border-radius:7px;background:#192720;color:#aee6c8;font-size:.58rem;line-height:1.45}.gid-section header b{min-width:32px}.identity-panel label.gid-row{align-items:start;padding-block:5px}.gid-tids{display:flex!important;flex-wrap:wrap;gap:3px;overflow:visible!important;text-overflow:clip!important}.gid-tids span{padding:1px 4px;border-radius:4px;background:#29323a;color:#aeb9c2;font-size:.49rem}.gid-row .identity-control{width:100%}
</style>
