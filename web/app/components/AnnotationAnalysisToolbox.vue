  ...
  <div class="toolbox-anchor">
    <AnimatePresence mode="wait">
      <Motion
        ...
        role="toolbar"
        :aria-label="mode === 'actor' ? '擊球球員指派工具' : '分析結果修改工具'"
        :initial="initial"
        :animate="animate"
        :exit="exit"
        ...
        <template v-else>
          <span class="toolbox-title"
            ><MousePointer2 :size="15" /><b>{{
              selectedTrackId === null ? '球員結果' : `Track ${selectedTrackId}`
            }}</b
            ><code v-if="selectedTrackId !== null">F{{ frameIndex }}</code></span
          >
          <span v-if="selectedTrackId === null" class="toolbox-instruction"
            >點擊畫面中的球員框</span
          >
          <label v-else class="action-select"
            >動作<select :value="selectedAction ?? ''" @change="handleAction">
              <option value="">自動</option>
              <option v-for="action in ANALYSIS_REVIEW_ACTIONS" :key="action" :value="action">
                {{ action }}
              </option>
            </select></label
          >
