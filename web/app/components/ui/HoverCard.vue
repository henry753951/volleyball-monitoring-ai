...
.ui-hover-card[data-side='left'] {
  --ui-hover-card-offset: 6px;
}
.ui-hover-card[data-side='right'] {
  --ui-hover-card-offset: -6px;
}
.ui-hover-card[data-state='open'] {
  animation: ui-hover-card-in 190ms cubic-bezier(0.16, 1, 0.3, 1) both;
}
.ui-hover-card[data-state='closed'] {
  animation: ui-hover-card-out 110ms cubic-bezier(0.4, 0, 1, 1) both;
}
.ui-hover-card__arrow {
  fill: #111114;
}
@keyframes ui-hover-card-in {
  from {
    opacity: 0;
    filter: blur(3px);
    transform: translateX(var(--ui-hover-card-offset, 0)) scale(0.97);
  }
  to {
    opacity: 1;
    filter: blur(0);
