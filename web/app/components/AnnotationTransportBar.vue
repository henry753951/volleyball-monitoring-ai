...
const clipInitial = computed(() => reducedMotion.value === 'reduce'
  ? { width: 0, opacity: 0 }
  : { width: 0, opacity: 0, filter: 'blur(7px)' })
const clipAnimate = computed(() => reducedMotion.value === 'reduce'
  ? { width: `${clipActionsWidth.value}px`, opacity: clipActionsWidth.value > 0 ? 1 : 0 }
  : { width: `${clipActionsWidth.value}px`, opacity: clipActionsWidth.value > 0 ? 1 : 0, filter: clipActionsWidth.value > 0 ? 'blur(0px)' : 'blur(7px)' })
const clipExit = computed(() => reducedMotion.value === 'reduce'
  ? { width: 0, opacity: 0 }
  : { width: 0, opacity: 0, filter: 'blur(4px)' })
const clipTransition = computed(() => reducedMotion.value === 'reduce'
  ? { duration: 0.01 }
...
<template>
  <div class="transport-bar">
    ...
    <div class="transport-context"><div><strong>{{ contextTitle }}</strong><small>{{ contextHits }} 次擊球 · {{ contextDuration }}</small></div><AnnotationProcessingBadge :label="contextState" :processing="processing" :retrying="processingRetrying" @retry="$emit('retryProcessing')" /></div>
    <AnimatePresence :initial="false">
      ...
      >
        <div ref="clipActionsContent" class="clip-actions" role="group" aria-label="片段工具">
          ...
          <UiTooltip content="永久刪除目前選取的片段"><button type="button" class="tool-button danger" :disabled="!editReady" aria-label="刪除所選片段" @click="$emit('deleteClip')"><Trash2 :size="14" />刪除所選片段</button></UiTooltip>
        </div>
      </Motion>
    </AnimatePresence>
    <div class="keypoint-actions" role="group" aria-label="擊球點工具">
      ...
      <UiTooltip content="刪除目前選取的擊球點"><button type="button" class="tool-button danger" :disabled="!pointDeleteEnabled || !editReady" aria-label="刪除所選擊球點" @click="$emit('deletePoint')"><Trash2 :size="14" />刪除擊球點</button></UiTooltip>
    </div>
