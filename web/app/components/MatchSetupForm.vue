...
const leftTeam = reactive({ name: '', shortName: '', roster: [{ name: '', jerseyNumber: '', position: 'UNSPECIFIED' }] as RosterInput[] })
const rightTeam = reactive({ name: '', shortName: '', roster: [{ name: '', jerseyNumber: '', position: 'UNSPECIFIED' }] as RosterInput[] })
...
function matchInput(): CreateMatchSetupInput {
  return {
    ...
    leftTeam: { name: leftTeam.name.trim(), shortName: leftTeam.shortName.trim(), roster: leftTeam.roster.map(row => ({ name: row.name.trim(), jerseyNumber: row.jerseyNumber.trim(), position: row.position })) },
    rightTeam: { name: rightTeam.name.trim(), shortName: rightTeam.shortName.trim(), roster: rightTeam.roster.map(row => ({ name: row.name.trim(), jerseyNumber: row.jerseyNumber.trim(), position: row.position })) },
  }
}

function validYoutubeUrl(value: string) {
  try {
    return ['youtube.com', 'www.youtube.com', 'youtu.be', 'm.youtube.com'].includes(new URL(value).hostname.toLowerCase())
  }
  catch { return false }
}
...
<template>
  <form class="match-setup" :class="{ compact: props.compact }" :aria-busy="props.pending" @submit.prevent="submit">
    <div class="setup-progress" aria-label="建立進度">
      <span :class="{ active: step === 1, done: step === 2 }"><i><Check v-if="step === 2" :size="12" /><template v-else>1</template></i><b>場次與隊伍</b></span>
      <em />
    ...
    <div v-else class="media-step">
      <header><span>影音來源</span><h2>連接這場比賽的畫面</h2><p>YouTube 影片、直播與本機 MP4 都會由伺服器建立完整 DVR 與逐幀索引。</p></header>
    ...
    <div class="match-actions">
      ...
      <UiButton v-if="step === 2" type="button" variant="ghost" :disabled="props.pending" @click="step = 1"><ArrowLeft :size="15" />返回</UiButton>
...
<style scoped>
.match-setup{display:grid;gap:14px;padding:14px;overflow:auto;color:#fafafa}.setup-progress{display:grid;grid-template-columns:auto minmax(30px,100px) auto;align-items:center;justify-content:center;gap:10px;padding-block:2px}.setup-progress span{display:flex;align-items:center;gap:7px;color:#71767c;font-size:.62rem}.setup-progress i{width:22px;height:22px;display:grid;place-items:center;border:1px solid #383d43;border-radius:50%;font-style:normal;font-variant-numeric:tabular-nums}.setup-progress span.active{color:#f4f5f5}.setup-progress span.active i,.setup-progress span.done i{border-color:#d9dcdf;background:#e7e9eb;color:#111417}.setup-progress em{height:1px;background:#30353a}.match-fields{display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;padding:14px;border-radius:10px;background:#111113}.match-fields :deep(.field){border:1px solid #27272a;background:#18181b;color:#fafafa}.match-fields :deep(.field:focus){border-color:#71717a;box-shadow:0 0 0 2px #fafafa24}.team-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.media-step{display:grid;gap:17px;min-height:390px;padding:18px;border:1px solid #292e33;border-radius:13px;background:#0f1215}.media-step header{display:grid;gap:5px}.media-step header span{color:#8c939a;font-size:.58rem;font-weight:750;letter-spacing:.12em;text-transform:uppercase}.media-step h2{margin:0;font-size:1rem;letter-spacing:-.025em}.media-step p{max-width:62ch;margin:0;color:#7e858c;font-size:.64rem;line-height:1.55}.match-errors{padding:12px;border-radius:9px;background:#2b1114;color:#fca5a5;font-size:.72rem}.match-actions{display:flex;justify-content:flex-end;gap:8px}.match-actions :deep(button){display:inline-flex;align-items:center;gap:7px}.compact{max-height:calc(100dvh - 78px)}@media(max-width:760px){.match-fields,.team-grid{grid-template-columns:1fr}.setup-progress b{display:none}}
</style>
