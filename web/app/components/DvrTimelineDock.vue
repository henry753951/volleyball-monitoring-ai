...
const emit = defineEmits<{ seek: [target: string]; select: [keyPointId: string] }>()
...
<template>
  <section class="timeline-surface" :class="{ dragging }" aria-label="Server DVR timeline" @wheel.prevent="wheel" @pointerdown="beginPan" @pointermove="movePan" @pointerup="endPan" @pointercancel="endPan">
    ...
    <div class="lane-row keypoint-lane"><span class="lane-label">KEYPOINTS</span><div class="lane-content" @click="seek"><button v-for="point in annotationPoints" v-show="isVisible(point.capture_time_us)" :key="point.key_point_id" type="button" class="keypoint-dot" :class="{ service: point.marker_kind === 'service', terminal: point.is_terminal, locked: immutable || !editable, editable: editable && !immutable, selected: selectedKeyPointId === point.key_point_id }" :style="{ left: `${position(point.capture_time_us)}%` }" :aria-pressed="selectedKeyPointId === point.key_point_id" :title="`${point.marker_kind} · frame ${point.capture_frame_index}${editable && !immutable ? ' · click to fine-tune' : ''}`" @pointerdown.stop @click.stop="selectPoint(point.key_point_id, point.capture_time_us)" /></div></div>
