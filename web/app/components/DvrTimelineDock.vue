...
<template>
  <section class="timeline-surface" :class="{ dragging }" aria-label="Server DVR timeline" @wheel.prevent="wheel" @pointerdown="beginPan" @pointermove="movePan" @pointerup="endPan" @pointercancel="endPan">
    ...
    <div class="lane-row keypoint-lane"><span class="lane-label">KEYPOINTS</span><div class="lane-content" @click="seek"><button v-for="point in annotationPoints" v-show="isVisible(point.capture_time_us)" :key="point.key_point_id" type="button" class="keypoint-dot" :class="{ service: point.marker_kind === 'service', terminal: point.is_terminal, locked: annotation?.snapshot.active_submission_id }" :style="{ left: `${position(point.capture_time_us)}%` }" :title="`${point.marker_kind} · frame ${point.capture_frame_index}`" @pointerdown.stop /></div></div>
