  ...
  <section class="timeline-surface" aria-label="影音時間軸" @wheel.prevent="wheel">
    ...
    <div class="lane-row clip-lane">
      ...
      <div class="lane-content" @click="emit('clearSelection')">
        <button v-for="segment in displaySegments" v-show="segmentVisible(segment)" :key="segment.id" data-timeline-interactive type="button" class="timeline-mask historical" :class="[segment.status, segmentDensityClass(segment), { selected: selectedSegmentId === segment.id }]" :style="{ left: `${segmentLeft(segment)}%`, top: `${maskTop()}px`, width: `${segmentWidth(segment)}%` }" :aria-label="`${segment.label} · ${segment.stateLabel || segmentStatusLabel(segment.status)}`" @click.stop="emit('selectSegment', segment.id, segment.startCaptureTimeUs)" @dblclick.stop="focusHistoricalSegment(segment)"><span>{{ segment.label }}</span><small>{{ segment.stateLabel || segmentStatusLabel(segment.status) }}</small><b v-if="segment.status === 'analyzed'">待指派</b></button>
        <button v-if="currentMaskGeometry" v-show="currentMaskGeometry.visible" data-timeline-interactive type="button" class="timeline-mask current" :class="[currentMaskTone, currentMaskGeometry.density, { selected: maskSelected }]" :style="{ left: `${currentMaskGeometry.left}%`, top: `${maskTop()}px`, width: `${currentMaskGeometry.width}%` }" :aria-label="`${currentMaskLabel} · ${currentMaskStateLabel}`" @click.stop="emit('selectMask')" @dblclick.stop="focusCurrentMask"><span>{{ currentMaskLabel }}</span><small>{{ currentMaskStateLabel }}</small></button>
...
.timeline-mask.density-compact b{opacity:0;pointer-events:none}
