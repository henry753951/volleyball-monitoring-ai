    ...
    <div class="worker-delete-dialog">
      <div class="worker-delete-dialog__warning">
        ...
        <p>
          ...
          ><span>已完成的分析結果會保留；遠端 Worker 再次啟動時仍可重新註冊。</span>
        </p>
      </div>
      <dl>
        <div>
          <dt><Cpu :size="15" />Build</dt>
          <dd>{{ worker?.providerBuildId }}</dd>
        </div>
        <div>
          <dt>最後心跳</dt>
          <dd>{{ worker ? new Date(worker.lastSeenAt).toLocaleString() : '—' }}</dd>
        </div>
      </dl>
...
.worker-delete-dialog__warning p {
  display: grid;
  gap: 4px;
  margin: 0;
}
.worker-delete-dialog__warning strong {
  color: #fee2e2;
  font-size: 0.76rem;
}
.worker-delete-dialog__warning span {
  font-size: 0.66rem;
  line-height: 1.55;
}
.worker-delete-dialog dl {
  display: grid;
...
.worker-delete-dialog dl > div {
  display: grid;
  gap: 7px;
  padding: 12px;
}
.worker-delete-dialog dl > div + div {
  border-left: 1px solid #29292e;
}
.worker-delete-dialog dt {
  display: flex;
  align-items: center;
  gap: 6px;
  color: #8d8d98;
