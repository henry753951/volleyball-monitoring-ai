      ...
      <article v-for="component in deployment.components" :key="component.component">
        ...
        <dl>
          ...
          <div>
            ...
            <dd>
              <a
                v-if="commitUrl(component.repositoryUrl, component.gitSha)"
                :href="commitUrl(component.repositoryUrl, component.gitSha)!"
                target="_blank"
                rel="noreferrer"
              >
          ...
          <div>
            <dt>Image digest</dt>
            <dd>
              <code :title="component.imageDigest ?? ''">{{
                shortDigest(component.imageDigest)
              }}</code>
            </dd>
          </div>
          <div>
            <dt>Rollout</dt>
            <dd>{{ component.readyReplicas }} / {{ component.desiredReplicas }} ready</dd>
          </div>
          <div v-if="component.nodeNames.length">
            <dt>節點</dt>
            <dd>{{ component.nodeNames.join(', ') }}</dd>
          </div>
          <div v-if="component.accelerator">
            <dt>GPU</dt>
            <dd>{{ component.accelerator }}</dd>
          </div>
          <div v-if="component.modelVersion">
            <dt>模型</dt>
            <dd>{{ component.modelVersion }}</dd>
          </div>
        </dl>
      </article>
    </div>
...
.deployment-surface header p {
  margin: 4px 0 0;
  color: #85858a;
  font-size: 0.54rem;
}
.deployment-surface header > span,
...
.deployment-surface i {
  width: 6px;
  height: 6px;
  border-radius: 50%;
  background: #6f737a;
}
.deployment-surface .ready i {
  background: #4bc28a;
}
.deployment-surface .progressing i {
  background: #d4a255;
}
.deployment-surface .degraded i {
  background: #d56f6d;
}
.deployment-list {
  display: grid;
