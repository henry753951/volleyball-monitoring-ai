...
const currentSet = computed(
  () =>
    props.match.sets.find(set => set.status.toLowerCase() === 'live') ?? props.match.sets.at(-1),
)
const leftSetWins = computed(
  () => props.match.sets.filter(set => set.winningTeamId === props.match.teams[0]?.id).length,
)
const rightSetWins = computed(
  () => props.match.sets.filter(set => set.winningTeamId === props.match.teams[1]?.id).length,
)
const currentSource = computed(
  () =>
    props.streams.find(stream => ['LIVE', 'STARTING', 'STOPPING'].includes(stream.status)) ??
    props.streams[0] ??
    null,
)
  ...
  <article class="match-workspace">
    <header class="match-workspace__header">
      <div class="match-identity">
        <span class="match-state" :class="match.status.toLowerCase()"
          ><i />{{
      ...
      <div class="match-actions">
        <NuxtLink :to="`/annotate/${match.id}`" class="annotate-action"
          ><SquarePen :size="15" />開啟標記</NuxtLink
        >
    ...
    <section class="scoreboard" aria-label="目前比分">
      <div class="score-team left">
        <strong>{{ match.teams[0]?.shortName || match.teams[0]?.name || '左隊' }}</strong
        ><span>{{ match.teams[0]?.name }}</span
        ><small class="team-set-score" :aria-label="`左隊勝局 ${leftSetWins}`">{{
          leftSetWins
        }}</small>
      </div>
      <div class="point-score">
        <div>
          <b>{{ currentSet?.leftScore ?? 0 }}</b
          ><i>:</i><b>{{ currentSet?.rightScore ?? 0 }}</b>
        </div>
        <small>第 {{ currentSet?.setNumber ?? 1 }} 局</small>
      </div>
      <div class="score-team right">
        <strong>{{ match.teams[1]?.shortName || match.teams[1]?.name || '右隊' }}</strong
        ><span>{{ match.teams[1]?.name }}</span
        ><small class="team-set-score" :aria-label="`右隊勝局 ${rightSetWins}`">{{
          rightSetWins
        }}</small>
      </div>
    </section>
    ...
    <div class="runtime-grid">
      <section class="runtime-rail media-rail">
        <div class="rail-heading">
          ...
          <div class="rail-heading-actions">
            <strong :class="{ danger: media?.failedCaptureCount }"
              >{{ media?.activeCaptureCount ?? 0 }} 運行中</strong
            ><button type="button" @click="emit('monitor')">
              詳細監控<ChevronRight :size="13" />
            </button>
          </div>
        </div>
        <div v-if="currentSource" class="source-chips">
          <span :class="currentSource.status.toLowerCase()"
            ><i />{{ sourceLabel(currentSource.sourceKind)
            }}<small>{{ statusLabel(currentSource.status) }}</small></span
          >
        </div>
        ...
        <p v-if="liveSource?.program">
          {{
            liveSource.program.fps.denominator
              ? (liveSource.program.fps.numerator / liveSource.program.fps.denominator).toFixed(2)
              : '—'
          }}
          fps · rev {{ liveSource.program.playlistRevision }}
        </p>
      </section>
      ...
      <section class="runtime-rail ai-rail">
        <div class="rail-heading">
          ...
          <div class="rail-heading-actions">
            <strong>{{ jobs.length ? `${jobs.length} 處理中` : '閒置' }}</strong
            ><NuxtLink to="/control/jobs">查看紀錄<ChevronRight :size="13" /></NuxtLink>
          </div>
        </div>
        <div v-if="latestJob" class="latest-job">
          <div>
            <strong>{{ stageLabel(latestJob.stage) }}</strong
            ><span>{{ statusLabel(latestJob.status) }}</span>
          </div>
          <p>{{ latestJob.workerInstanceKey || '尚未分配 Worker' }}</p>
          <span class="job-progress"
            ><i :style="{ width: `${Math.min(100, (latestJob.progress ?? 0) * 100)}%` }"
          /></span>
          <small
            >{{ Math.round((latestJob.progress ?? 0) * 100) }}% · 最近更新
            {{
              new Intl.DateTimeFormat('zh-TW', {
                hour: '2-digit',
                minute: '2-digit',
                second: '2-digit',
                hour12: false,
              }).format(new Date(latestJob.updatedAt))
            }}</small
          >
        </div>
        <div v-else class="rail-empty">目前沒有處理中的作業</div>
        <div v-if="jobs.length > 1" class="job-summary">
          <span v-for="job in jobs.slice(1, 4)" :key="job.id"
            ><i :class="job.status.toLowerCase()" />{{ stageLabel(job.stage)
            }}<small>{{ statusLabel(job.status) }}</small></span
          >
        </div>
      </section>
    </div>
    ...
    <footer>
      ...
      <button type="button" @click="emit('media')">
        管理場次工作區<ChevronRight :size="14" />
      </button>
    </footer>
  </article>
</template>
...
.match-menu summary::-webkit-details-marker {
  display: none;
}
.match-menu > div {
  position: absolute;
...
.scoreboard {
  ...
  min-height: 88px;
  display: grid;
  grid-template-columns: minmax(90px, 1fr) auto 12px auto minmax(90px, 1fr);
  align-items: center;
  gap: 12px;
  padding: 12px 18px;
...
.job-summary i.completed {
  background: #56bd88;
}
.job-summary i.failed {
  background: #d46f6b;
}
.job-summary small {
  color: #6c7076;
}
.match-workspace footer {
  height: 39px;
  display: flex;
  align-items: center;
  justify-content: space-between;
...
.match-workspace footer button:hover {
  color: #fff;
}
...
.scoreboard > .score-team.left {
  text-align: right;
}
.scoreboard > .score-team.right {
  text-align: left;
}
.scoreboard > .score-team strong {
  font-size: 0.7rem;
}
.scoreboard > .score-team span {
  color: #929296;
  font-size: 0.52rem;
}
.team-set-score {
...
.score-team.left .team-set-score {
  justify-self: end;
}
.score-team.right .team-set-score {
  justify-self: start;
}
.point-score {
...
.rail-heading-actions > strong {
  color: #9a9a9f;
  font-size: 0.54rem;
}
.rail-heading-actions > strong.danger {
  color: #e28a87;
}
.rail-heading-actions button,
...
.rail-heading-actions button:hover,
.rail-heading-actions a:hover {
  color: #fff;
}
.rail-heading-actions a:focus-visible {
