...
const averageLatency = computed(() => {
  const values = onlineWorkers.value.flatMap(worker =>
    worker.latencyMs === null ? [] : [worker.latencyMs],
  )
  if (!values.length) return null
  return Math.round(values.reduce((total, value) => total + value, 0) / values.length)
})
const availableSlots = computed(() =>
  onlineWorkers.value.reduce(
  ...
  <div class="worker-console">
    <section class="engine-surface" aria-labelledby="engine-title">
      ...
      <dl class="engine-stats">
        <div>
          <dt>在線 Worker</dt>
          <dd>{{ onlineWorkers.length }}</dd>
        </div>
        <div>
          <dt>可用工作槽</dt>
          <dd>{{ availableSlots }}</dd>
        </div>
        <div>
          <dt>執行中</dt>
          <dd>{{ activeJobs }}</dd>
      ...
      <div class="credentials">
        ...
        <div v-if="tokens.length" class="credential-list">
          ...
          >
            ...
            <div>
              ...
              <small
                >{{ token.tokenPrefix }}… ·
                {{ token.lastUsedAt ? `${relativeTime(token.lastUsedAt)}使用` : '尚未使用' }}</small
              >
            </div>
            <span class="credential-state"><i />{{ token.enabled ? '啟用' : '停用' }}</span>
            <button
              type="button"
              :aria-label="token.enabled ? `停用 ${token.name}` : `啟用 ${token.name}`"
              :title="token.enabled ? '停用 Token' : '啟用 Token'"
              @click="emit('toggleToken', token)"
            >
              <Check v-if="token.enabled" :size="14" /><CircleDot v-else :size="14" />
            </button>
            <button
              type="button"
              :aria-label="`輪替 ${token.name}`"
              title="輪替 Token"
              @click="emit('rotateToken', token)"
            >
              <RotateCw :size="14" />
            </button>
            <button
        ...
        <button v-else type="button" class="first-token" @click="emit('createToken')">
          <KeyRound :size="16" /><span
            ><strong>建立第一組 Token</strong><small>供 AI Worker 安全連線</small></span
          >
        </button>
      </div>
    </section>
    ...
    <section class="fleet-surface" aria-labelledby="fleet-title">
      <header class="fleet-header">
        <div>
          <h2 id="fleet-title">Worker 狀態</h2>
          <p>持續心跳與 transport ping/pong 延遲</p>
        </div>
        <span :class="onlineWorkers.length ? 'online' : 'offline'"
          ><i />{{ onlineWorkers.length }} 在線</span
        >
      </header>
      ...
      <div v-else class="fleet-list">
        <article v-for="worker in workers" :key="worker.id">
          ...
          <div class="worker-load">
            <span><i :style="{ width: `${Math.min(100, worker.utilization * 100)}%` }" /></span
            ><small>{{ worker.activeJobs }} / {{ worker.maxConcurrency }} 工作槽</small>
          </div>
          ...
          <span class="worker-state" :class="worker.status"
            ><i />{{
              worker.status === 'online'
                ? `心跳 ${relativeTime(worker.lastSeenAt)}`
                : worker.status === 'stale'
                  ? '心跳逾時'
                  : '離線'
            }}</span
          ...
          <button
            ...
            title="移除離線 Worker"
            @click="emit('deleteWorker', worker)"
          >
            <Trash2 :size="14" />
          </button>
        </article>
...
.connection-row {
  min-height: 44px;
  display: grid;
  grid-template-columns: 18px auto minmax(0, 1fr) 30px;
  align-items: center;
  gap: 10px;
  margin: 0 20px 18px;
  padding: 0 10px;
...
.credential-row .delete-token:hover {
  border-color: #563033;
  background: #351b1d;
  color: #fca5a5;
}
...
.credential-row {
  min-height: 52px;
  display: grid;
  grid-template-columns: 22px minmax(0, 1fr) auto 30px 30px 30px;
  align-items: center;
  gap: 8px;
...
.credential-row.disabled .credential-state i,
.fleet-header > span.offline i,
...
.fleet-list article {
  min-height: 82px;
  display: grid;
  grid-template-columns: 34px minmax(190px, 1.1fr) minmax(150px, 0.75fr) 120px 120px 30px;
  align-items: center;
  gap: 12px;
  padding: 8px 20px;
  border-top: 1px solid #2b2b2e;
...
.worker-name .worker-hardware {
  color: #a8b7d2;
}
.worker-name .worker-current {
  color: #91c6a8;
}
.worker-load {
  display: grid;
...
@media (max-width: 980px) {
  .fleet-list article {
    grid-template-columns: 34px minmax(0, 1fr) 130px 100px auto 30px;
  }
}
@media (max-width: 700px) {
  ...
  .engine-stats > div:nth-child(odd) {
    padding-left: 0;
  }
  .fleet-list article {
  ...
  .credential-row {
    grid-template-columns: 22px minmax(0, 1fr) 30px 30px 30px;
  }
  .credential-state {
    display: none;
  }
