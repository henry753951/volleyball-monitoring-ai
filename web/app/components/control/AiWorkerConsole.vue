...
import type { AiWorkerSnapshot, AiWorkerTokenSnapshot } from '~/lib/operationsMonitor'
    ...
    <section class="engine-surface" aria-labelledby="engine-title">
      <header class="engine-header">
        <div class="engine-identity">
          <span class="engine-mark"><Cpu :size="18" /></span>
          <div>
            <h2 id="engine-title">volleyball-analysis-engine</h2>
            <p>外部 AI Worker 以 Token 連入中央系統</p>
          </div>
        </div>
      ...
      <div class="credentials">
        ...
        <div v-if="tokens.length" class="credential-list">
          <div v-for="token in tokens" :key="token.id" class="credential-row" :class="{ disabled: !token.enabled }">
            ...
            <button type="button" :title="token.enabled ? '停用 Token' : '啟用 Token'" @click="emit('toggleToken', token)">
              <Check v-if="token.enabled" :size="14" /><CircleDot v-else :size="14" />
            </button>
            <button type="button" title="輪替 Token" @click="emit('rotateToken', token)"><RotateCw :size="14" /></button>
          </div>
        </div>
...
<style scoped>
.worker-console{display:grid;gap:20px}.engine-surface,.fleet-surface{overflow:hidden;border:0;border-radius:14px;background:#101114}.engine-header,.fleet-header{display:flex;align-items:center;justify-content:space-between;gap:20px;padding:18px 20px}.engine-identity{display:flex;align-items:center;gap:12px}.engine-mark,.worker-avatar{display:grid;place-items:center;border-radius:9px;background:#202227;color:#d7d9dc}.engine-mark{width:38px;height:38px}.engine-header h2,.fleet-header h2,.credentials-heading h3{margin:0;color:#f3f4f5}.engine-header h2{font-size:.78rem;letter-spacing:-.015em}.engine-header p,.fleet-header p,.credentials-heading p{margin:4px 0 0;color:#7f8288;font-size:.54rem}.primary-button{height:36px;display:flex;align-items:center;gap:7px;padding:0 13px;border:0;border-radius:8px;background:#f1f2f3;color:#111216;font-size:.61rem;font-weight:760}.engine-stats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));margin:0;padding:4px 20px 18px}.engine-stats>div{display:grid;gap:5px;padding-left:16px}.engine-stats>div:first-child{padding-left:0}.engine-stats dt{color:#777a80;font-size:.51rem}.engine-stats dd{margin:0;font-size:.9rem;font-weight:730;font-variant-numeric:tabular-nums}.connection-row{min-height:44px;display:grid;grid-template-columns:18px auto minmax(0,1fr) 30px;align-items:center;gap:10px;margin:0 20px 18px;padding:0 10px;border-radius:8px;background:#181a1e;color:#8e9197}.connection-row>span{font-size:.5rem}.connection-row code{overflow:hidden;color:#c4c7cb;font-size:.52rem;text-overflow:ellipsis;white-space:nowrap}.connection-row button,.credential-row button,.delete-worker{width:30px;height:30px;display:grid;place-items:center;border:0;border-radius:7px;background:transparent;color:#aeb1b6}.connection-row button:hover,.credential-row button:hover,.delete-worker:hover{background:#292c31;color:#fff}.credentials{padding:0 20px 12px}.credentials-heading{display:flex;align-items:flex-end;justify-content:space-between;padding:0 0 8px}.credentials-heading h3,.fleet-header h2{font-size:.66rem}.credentials-heading>span,.fleet-header>span{color:#85888e;font-size:.51rem}.credential-list{display:grid}.credential-row{min-height:52px;display:grid;grid-template-columns:22px minmax(0,1fr) auto 30px 30px;align-items:center;gap:8px;border-top:1px solid #202226;color:#aeb1b6}.credential-row.disabled{opacity:.48}.credential-row>div{display:grid;gap:3px}.credential-row strong{color:#e9eaec;font-size:.57rem}.credential-row small{color:#74777d;font-size:.49rem}.credential-state{display:flex;align-items:center;gap:6px;color:#9a9da2;font-size:.51rem}.credential-state i,.fleet-header>span i,.worker-state i{width:6px;height:6px;border-radius:50%;background:#4bc28a}.credential-row.disabled .credential-state i,.fleet-header>span.offline i,.worker-state.offline i{background:#d56f6d}.first-token{width:100%;min-height:58px;display:flex;align-items:center;justify-content:center;gap:10px;border:0;border-top:1px solid #202226;background:transparent;color:#989ba1}.first-token span{display:grid;gap:3px;text-align:left}.first-token strong{color:#d9dbde;font-size:.57rem}.first-token small{font-size:.49rem}.fleet-header>span{display:flex;align-items:center;gap:6px}.fleet-list article{min-height:66px;display:grid;grid-template-columns:34px minmax(170px,1.1fr) minmax(150px,.75fr) 120px 120px 30px;align-items:center;gap:12px;padding:0 20px;border-top:1px solid #202226}.worker-avatar{width:32px;height:32px}.worker-name,.worker-latency{display:grid;gap:3px}.worker-name strong,.worker-latency strong{overflow:hidden;font-size:.59rem;text-overflow:ellipsis;white-space:nowrap}.worker-name small,.worker-latency small,.worker-load small{color:#74777d;font-size:.49rem}.worker-load{display:grid;gap:6px}.worker-load>span{height:4px;overflow:hidden;border-radius:2px;background:#292c31}.worker-load>span i{display:block;height:100%;background:#55c58e}.worker-state{display:flex;align-items:center;gap:6px;color:#96999e;font-size:.51rem}.worker-state.stale i{background:#d4a255}.delete-worker:disabled{opacity:.3}.fleet-empty{min-height:160px;display:grid;place-items:center;align-content:center;gap:7px;color:#74777d}.fleet-empty strong{color:#aeb1b6;font-size:.64rem}.fleet-empty span{font-size:.52rem}
@media(max-width:980px){.fleet-list article{grid-template-columns:34px minmax(0,1fr) 130px 100px auto 30px}}
@media(max-width:700px){.engine-header{align-items:flex-start}.engine-stats{grid-template-columns:repeat(2,1fr);gap:14px}.engine-stats>div:nth-child(odd){padding-left:0}.fleet-list article{grid-template-columns:34px minmax(0,1fr) auto}.worker-load,.worker-latency{grid-column:2}.worker-state{grid-column:3;grid-row:1}.delete-worker{grid-column:3}.credential-row{grid-template-columns:22px minmax(0,1fr) 30px 30px}.credential-state{display:none}}
