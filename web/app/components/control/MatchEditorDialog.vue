...
<template>
  ...
  >
    <form class="match-editor" @submit.prevent="save">
      ...
      <label
        ><span>預定時間</span>
        <div><CalendarClock :size="15" /><input v-model="scheduledAt" type="datetime-local" /></div
      ></label>
      <label
        ><span>狀態</span
        ><select v-model="status">
          <option value="PLANNED">待開始</option>
    ...
    <template #footer
      ...
      ><UiButton :disabled="pending || !title.trim()" @click="save">{{
        pending ? '儲存中…' : '儲存變更'
      }}</UiButton></template
    >
  </UiAnimatedModal>
...
.match-editor label > div input {
  min-height: 0;
  padding: 0;
  border: 0;
  background: transparent;
}
.match-editor input:focus,
.match-editor select:focus,
.match-editor label > div:focus-within {
  border-color: #a1a1aa;
}
.form-error {
  margin: 0;
