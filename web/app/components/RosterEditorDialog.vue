...
const activeTeam = computed(
  () => props.match.teams.find(team => team.id === activeTeamId.value) ?? props.match.teams[0],
)
const rows = computed(() => (activeTeam.value ? (rowsByTeam[activeTeam.value.id] ?? []) : []))
const dirtyTeamCount = computed(() => dirtyTeamIds.size)
const saveLabel = computed(() =>
  pending.value ? '儲存中…' : dirtyTeamCount.value > 1 ? '儲存兩隊名單' : '儲存名單',
)
...
function importRows(
  teamId: string,
  players: Array<{ jerseyNumber: string; name: string; position: RosterEditInput['position'] }>,
) {
...
function handlePaste(event: ClipboardEvent) {
  ...
  importStatus.value = result.value.teams
    .map(
      team =>
        `${props.match.teams.find(item => item.id === team.teamId)?.shortName ?? team.teamName} ${team.players.length} 位`,
    )
    .join('、')
...
async function save() {
  ...
  try {
    for (const team of teams) {
      currentTeam = team
      const updated = await domain.updateMatchRoster({
  ...
  <UiAnimatedModal :open="open" title="球員名單" :description="match.title" @close="emit('close')">
    <div class="roster-dialog" @paste.capture="handlePaste">
      <div class="roster-import-bar">
        <span
          ><Sparkles :size="15" /><strong>AI 名單匯入</strong
          ><small>貼上相容 JSON 會填入兩隊草稿</small></span
        >
        <UiTooltip
          side="bottom"
          content="複製包含本場隊名、識別碼與固定 JSON 格式的調查指示。交給 LLM 查詢後，把回傳 JSON 直接貼回此視窗即可匯入兩隊草稿。"
        >
          <UiButton
            variant="secondary"
            size="sm"
            aria-label="複製球員名單調查 Prompt"
            @click="copyResearchPrompt"
            ><ClipboardCopy :size="14" />複製查詢 Prompt</UiButton
          >
        </UiTooltip>
      </div>
      <nav aria-label="選擇隊伍">
      ...
      <UiScrollArea class="roster-table">
        <div class="roster-table__inner">
          <div class="roster-table__head">
            <span>背號</span><span>球員</span><span>位置</span><span />
          </div>
          <div v-for="(row, index) in rows" :key="row.rowKey" class="roster-row">
            <input
              v-model="row.jerseyNumber"
              :aria-label="`第 ${index + 1} 位球員背號`"
              maxlength="12"
              placeholder="00"
              @input="markActiveTeamDirty"
            />
            <input
              v-model="row.name"
              :aria-label="`第 ${index + 1} 位球員姓名`"
              maxlength="120"
              placeholder="球員姓名"
              @input="markActiveTeamDirty"
            />
            <UiSelect
              ...
              :options="ROSTER_POSITION_SELECT_OPTIONS"
              :aria-label="`第 ${index + 1} 位球員位置`"
              @update:model-value="markActiveTeamDirty"
            />
            <UiButton
              variant="ghost"
              size="icon-sm"
              :aria-label="`移除 ${row.name || `第 ${index + 1} 位球員`}`"
              @click="removePlayer(index)"
              ><Trash2 :size="15"
            /></UiButton>
          </div>
          <UiButton class="add" variant="ghost" size="sm" @click="addPlayer"
            ><Plus :size="15" />新增球員</UiButton
          >
        </div>
      </UiScrollArea>
    </div>
    <template #footer>
      ...
      ><span v-else-if="importStatus" class="imported" role="status"
        >已匯入 {{ importStatus }}，尚未儲存</span
      ><span v-else-if="saved" class="saved">已儲存</span><span class="footer-spacer" />
      <UiButton :disabled="pending || !dirtyTeamCount" @click="save"
        ><Save :size="15" />{{ saveLabel }}</UiButton
      >
    </template>
  </UiAnimatedModal>
</template>
...
.roster-import-bar > span > svg {
  grid-row: 1/3;
  color: #a1a1aa;
}
.roster-import-bar strong {
  font-size: 0.69rem;
}
.roster-import-bar small {
  grid-column: 2;
  color: #a1a1aa;
  font-size: 0.6rem;
}
.roster-import-bar :deep(button) {
  flex: none;
}
.roster-dialog nav {
  display: flex;
...
.roster-dialog nav :deep(button) {
  min-width: 0;
  flex: 1;
  justify-content: flex-start;
  text-align: left;
}
.roster-dialog nav :deep(button.active) {
  background: #27272a;
  color: #fafafa;
}
.roster-dialog nav b {
...
.roster-table {
  height: min(430px, calc(86dvh - 232px));
  margin-top: 10px;
}
.roster-table__inner {
  padding: 0 14px 16px;
...
.roster-row {
  display: grid;
  grid-template-columns: 72px minmax(130px, 1fr) minmax(145px, 0.72fr) 34px;
  align-items: center;
  gap: 7px;
}
.roster-table__head {
...
.roster-row :deep(button) {
  color: #a1a1aa;
}
.roster-row :deep(button:hover) {
  background: #2b1114;
  color: #fca5a5;
}
.add {
...
@media (max-width: 680px) {
  .roster-table__head,
  .roster-row {
    grid-template-columns: 62px minmax(120px, 1fr) 34px;
  }
  .roster-table__head span:nth-child(3) {
    display: none;
  }
  .roster-row :deep(.ui-select__trigger) {
    grid-column: 1/3;
  }
}
@media (max-width: 560px) {
  ...
  .roster-table {
    height: min(400px, calc(86dvh - 240px));
  }
}
</style>
