...
const timeline = {
  ...
  availableRanges: [{ startUs: '1000', endUs: '2000', discontinuity: 0 }, { startUs: '3000', endUs: '4000', discontinuity: 1 }],
}
...
describe('DvrTimelineDock mounted interactions', () => {
  it('clears selection, emits exact target, and blocks gaps', async () => { const w = mount(DvrTimelineDock, { props: { timeline, playhead: null } }); const lane = w.find('.buffer-status'); Object.defineProperty(lane.element, 'getBoundingClientRect', { value: () => ({ left: 0, width: 100 }) }); await lane.trigger('pointerdown', { button: 0, clientX: 25 }); expect(w.emitted('clearSelection')).toHaveLength(1); expect(w.emitted('seek')?.[0]).toEqual(['1750']); await lane.trigger('pointerdown', { button: 0, clientX: 62 }); expect(w.emitted('clearSelection')).toHaveLength(2); expect(w.emitted('seek')).toHaveLength(1) })
  it('seeks immediately from the upper ruler with the same authoritative target mapping', async () => { const w = mount(DvrTimelineDock, { props: { timeline, playhead: null } }); const ruler = w.find('.ruler-row'); Object.defineProperty(ruler.element, 'getBoundingClientRect', { value: () => ({ left: 0, width: 100 }) }); await ruler.trigger('pointerdown', { button: 0, clientX: 25 }); expect(w.emitted('clearSelection')).toHaveLength(1); expect(w.emitted('seek')?.[0]).toEqual(['1750']) })
  ...
  it('marks a completed live source with END instead of an active LIVE edge', () => {
    const w = mount(DvrTimelineDock, { props: { timeline, playhead: '3999', playbackMode: 'ended_live' } })
