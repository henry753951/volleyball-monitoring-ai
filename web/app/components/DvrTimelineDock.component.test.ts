...
describe('DvrTimelineDock mounted interactions', () => {
  it('emits exact target and blocks gaps', async () => { const w = mount(DvrTimelineDock, { props: { timeline, playhead: null } }); const lane = w.find('[role="slider"]'); Object.defineProperty(lane.element, 'getBoundingClientRect', { value: () => ({ left: 0, width: 100 }) }); await lane.trigger('click', { clientX: 25 }); expect(w.emitted('seek')?.[0]).toEqual(['1250']); await lane.trigger('click', { clientX: 62 }); expect(w.emitted('seek')).toHaveLength(1) })
  it('bounds zoom/pan and reset control remains available', async () => { const w = mount(DvrTimelineDock, { props: { timeline, playhead: null } }); const dock = w.find('.timeline-dock'); await dock.trigger('wheel', { deltaY: -100 }); await dock.element.dispatchEvent(new WheelEvent('wheel', { deltaY: 100, shiftKey: true })); await w.find('button').trigger('click'); expect(w.find('button').text()).toContain('Reset') })
