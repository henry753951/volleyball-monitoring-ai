  ...
  <div class="analysis-panel">
    ...
    <template v-else>
      <header class="review-status">
        ...
        <span class="save-state"><LoaderCircle v-if="saving" class="spin" :size="14" />{{ saving ? '處理中' : dirtyCount ? `${dirtyCount} 項尚未套用` : reviewStatus === 'approved' ? '已審核' : reviewStatus === 'ready' ? '可審核' : '編輯中' }}</span>
      </header>

      <div class="review-workflow" aria-label="分析審核流程">
        <button type="button" :disabled="!dirtyCount || saving" @click="emit('apply')">套用修改</button>
        <button type="button" class="quiet" :disabled="!dirtyCount || saving" @click="emit('discard')"><RotateCcw :size="13" />捨棄</button>
        <button type="button" :disabled="dirtyCount > 0 || saving" @click="emit('recalculate')">重新分析</button>
        <button type="button" class="publish" :disabled="dirtyCount > 0 || reviewStatus !== 'ready' || saving" @click="emit('approve')"><Send :size="13" />審核發布</button>
      </div>
          ...
          <div :key="page" class="analysis-page" :data-page="page">
            ...
            <template v-else>
              ...
              <section v-if="page === 'hits'" class="hit-page">
                <button type="button" class="add-hit" @click="emit('addHit')"><Plus :size="14" />在目前畫格新增擊球點</button>
                ...
                <ol v-else class="hit-list">
                  <li v-for="hit in hits" :key="hit.keyPointId" :class="{ selected: selectedHitId === hit.keyPointId }">
                    <button type="button" class="hit-main" :aria-current="selectedHitId === hit.keyPointId ? 'true' : undefined" @click="emit('selectHit', hit.keyPointId)">
                      ...
                      <span><strong>{{ hit.actorLabel }}</strong><small>{{ hit.anchorSource === 'ai' ? `AI 擊球建議${hit.anchorConfidence === null ? '' : ` ${Math.round(hit.anchorConfidence * 100)}%`}` : hit.anchorSource === 'manual' ? '人工新增擊球點' : '人工 X 碰撞' }}{{ hit.timeAdjusted ? ' · 已微調' : '' }} · Frame {{ hit.frameIndex }} · {{ hit.ballLabel }}</small></span>
                      <em :class="hit.actorSource">{{ hit.actorSource === 'manual' ? '人工' : hit.actorSource === 'none' ? '無人' : '自動' }}</em>
                    </button>
                  </li>
                </ol>
                <div v-if="selectedHit && selectedHit.anchorSource !== 'human'" class="hit-time-editor" aria-label="微調擊球時間">
                  <span>擊球時間 · F{{ selectedHit.frameIndex }}</span>
                  <button type="button" title="往前一格" @click="emit('adjustHitTime', selectedHit.keyPointId, -1)">往前 1 格</button>
                  <button type="button" title="往後一格" @click="emit('adjustHitTime', selectedHit.keyPointId, 1)">+1 格</button>
                  <button v-if="selectedHit.anchorSource === 'ai' && selectedHit.timeAdjusted" type="button" class="hit-time-editor__reset" @click="emit('resetHitTime', selectedHit.keyPointId)">恢復 AI</button>
                </div>
                <button v-if="selectedHit" type="button" class="delete-hit" @click="emit('deleteHit', selectedHit.keyPointId)"><Trash2 :size="13" />刪除此擊球點</button>
                <section v-if="removedHits.length" class="removed-hits" aria-label="已移除的擊球點">
                  <header><strong>已移除</strong><span>{{ removedHits.length }}</span></header>
                  <button v-for="hit in removedHits" :key="hit.keyPointId" type="button" @click="emit('restoreHit', hit.keyPointId)">
                    <RotateCcw :size="12" /><span>{{ hit.label }}</span><small>Frame {{ hit.frameIndex }}</small>
                  </button>
                </section>
                <p v-if="selectedHit" class="dependency-note">{{ selectedHit.anchorSource === 'ai' ? '可逐格微調 AI 建議；修正後會重新建立事件順序與綁定。' : '人工 X 碰撞可在時間線修改；片段開始與結束不是碰撞事件。' }} 播放器上的人工擊球者指派永遠優先。</p>
              </section>
...
.analysis-menu__item small,.analysis-page__header small,.hit-main strong,.hit-main small{overflow:visible;line-height:1.4;text-overflow:clip;white-space:normal;overflow-wrap:anywhere}.hit-time-editor{grid-template-columns:1fr}.hit-time-editor span{padding:2px 0;text-align:left}.hit-time-editor button{width:100%;min-height:30px}
.review-workflow{display:grid;grid-template-columns:1fr 1fr;gap:6px;padding:8px 0;border-bottom:1px solid #2c3238}.review-workflow button,.add-hit,.delete-hit{min-height:32px;display:flex;align-items:center;justify-content:center;gap:5px;border:1px solid #47515a;border-radius:7px;background:#232a30;color:#e5eaee;font-size:.58rem;font-weight:750}.review-workflow button:hover:not(:disabled),.add-hit:hover,.delete-hit:hover{background:#303941}.review-workflow button:disabled{opacity:.38}.review-workflow .publish{border-color:#34735c;background:#214c3e;color:#d8f4e8}.review-workflow .quiet{background:transparent;color:#aeb8c2}.add-hit{width:100%;margin:8px 0;border-style:dashed;background:#171c20}.delete-hit{width:100%;margin:8px 0;border-color:#694044;background:#332326;color:#efc6ca}
.removed-hits{display:grid;margin:10px 0;border-top:1px solid #2c3238}.removed-hits header{min-height:34px;display:flex;align-items:center;justify-content:space-between;color:#9ba5ae;font-size:.58rem}.removed-hits header span{min-width:20px;padding:2px 6px;border-radius:999px;background:#293039;text-align:center}.removed-hits button{min-height:38px;display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:7px;padding:6px;border:0;border-bottom:1px solid #262c32;background:transparent;color:#d8dfe4;text-align:left}.removed-hits button:hover{background:#20272d}.removed-hits button span,.removed-hits button small{overflow-wrap:anywhere}.removed-hits button small{color:#8f99a3;font:500 .53rem "Cascadia Mono",Consolas,monospace}
