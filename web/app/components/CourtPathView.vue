...
<template>
  <div class="court-view">
    ...
    <svg viewBox="-16 -18 132 236" role="img" aria-label="2D 球場路徑">
      ...
      <g v-for="line in lines" :key="`${line.path.id}:${line.lineIndex}`" class="court-path" :class="{ focused: isFocused(line.pathIndex), terminal: line.path.is_terminal_segment }" :style="{ opacity: pathOpacity(line.pathIndex) }" tabindex="0" role="button" :aria-label="`球路 ${line.path.sequence_index + 1}`" @click="emit('seek', line.path.start_frame_index)" @keydown.enter="emit('seek', line.path.start_frame_index)">
