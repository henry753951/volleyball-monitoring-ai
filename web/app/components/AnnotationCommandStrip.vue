...
function reason(action: AnnotationAction) {
  ...
  if (action === 'contact' && (props.state !== 'OPEN' || !props.canMark))
    return props.state !== 'OPEN' ? '尚未開始片段' : '游標尚未確認'
    ...
    <template v-for="command in visibleCommands" :key="command.action">
      ...
      <UiButton
        ...
        :class="[
          ...
          {
            'command-primary': command.action === 'service',
            'command-mark': command.action === 'contact',
            'command-outcome': command.action.startsWith('close_'),
          },
        ]"
...
.command-strip {
  display: grid;
  grid-template-columns: 0.7fr 0.78fr 1px 1fr 1fr 0.9fr 1px 1fr;
  gap: 6px;
  align-items: center;
  padding: 7px 12px;
