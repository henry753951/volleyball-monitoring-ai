...
describe('AnnotationCommandStrip', () => {
  it.each([
    ...
    ['READY+terminal', 'READY', false, true, [false, false, true, true, true, true]],
  ...
  it('changes the Z label from service to end for an open boundary draft', () => { const w = mount(AnnotationCommandStrip, { props: { bindings, state: 'OPEN', canMark: true, lastKeyPoint: true, serviceMode: 'end', availability: { service: { enabled: true, reason: '' } } } }); expect(w.findAll('button')[0]?.text()).toContain('結束'); expect(w.findAll('button')[0]?.attributes('title')).toBe('結束') })
