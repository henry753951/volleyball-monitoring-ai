...
describe('AnnotationCommandStrip', () => {
  ...
  it('changes the Z label from start to end for an open boundary draft', () => { const w = mount(AnnotationCommandStrip, { props: { bindings, state: 'OPEN', canMark: true, lastKeyPoint: true, serviceMode: 'end', availability: { service: { enabled: true, reason: '' } } } }); expect(w.findAll('button')[0]?.text()).toContain('結束片段'); expect(w.findAll('button')[0]?.attributes('title')).toBe('結束片段') })
  ...
  it('renders formatted keys and concise disabled reasons', () => { const w = mount(AnnotationCommandStrip, { props: { bindings, state: 'IDLE', canMark: false, lastKeyPoint: false } }); expect(w.findAll('button').map(button => button.attributes('title'))).toContain('游標尚未確認'); expect(w.findAll('button').map(button => button.attributes('title'))).toContain('尚未開始片段'); const open = mount(AnnotationCommandStrip, { props: { bindings, state: 'OPEN', canMark: false, lastKeyPoint: false } }); expect(open.findAll('button').map(button => button.attributes('title'))).toContain('游標尚未確認'); expect(open.findAll('button').map(button => button.attributes('title'))).toContain('左側得分') })
