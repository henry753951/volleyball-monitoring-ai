...
export interface AnalysisReviewServiceOptions {
  analysisReviewWsUrl: (analysisRunId: string) => string
}

export function createAnalysisReviewService(
  analysisRunId: MaybeRefOrGetter<string | null>,
  options: AnalysisReviewServiceOptions,
) {
  ...
  function connect(currentGeneration: number) {
    ...
    try {
      nextSocket = new WebSocket(options.analysisReviewWsUrl(id))
      socket = nextSocket
    } catch (cause) {
