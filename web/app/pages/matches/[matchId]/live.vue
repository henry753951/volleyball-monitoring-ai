...
const leftTeam = computed(
  () =>
    teamById.value.get(activeSet.value?.side_assignment?.left_team_id ?? '') ??
    match.value?.teams[0] ??
    null,
)
const rightTeam = computed(
  () =>
    teamById.value.get(activeSet.value?.side_assignment?.right_team_id ?? '') ??
    match.value?.teams[1] ??
    null,
)
...
function scoreLabel() {
  return `${leftTeam.value?.shortName || leftTeam.value?.name || '左隊'} ${activeSetScore.value.left} 比 ${activeSetScore.value.right} ${rightTeam.value?.shortName || rightTeam.value?.name || '右隊'}`
}
...
<template>
  <section class="live-board" :aria-label="scoreLabel()">
    ...
    <div v-else-if="coach.error.value && !match" class="live-board__state" role="alert">
      <strong>無法載入場次</strong><span>{{ coach.error.value.message }}</span
      ><button type="button" @click="coach.refresh">重試</button>
    </div>
    ...
    <template v-else>
      ...
      <div class="live-stage">
        <div class="live-video">
          ...
          />
          <div v-else class="live-video__empty">
            <Radio :size="28" /><strong>{{ dvrError ? '直播載入失敗' : '直播尚未連結' }}</strong>
          </div>
          <a
        ...
        <aside class="live-feed" aria-label="最近回合">
          <div class="live-feed__header">
            <strong>最近回合</strong><NuxtLink :to="`/matches/${matchId}/rallies`">全部</NuxtLink>
          </div>
          <UiScrollArea class="live-feed__scroll">
            <div>
              <ol>
                <li v-for="rally in match.rallies" :key="rally.id">
                  <NuxtLink :to="`/matches/${matchId}/replay/${rally.id}`">
                    ...
                    <span class="live-feed__result"
                      ...
                      ><small>{{ coachRallyContactCount(rally) }} 擊球</small></span
                    >
...
.score-ribbon {
  min-height: clamp(78px, 13dvh, 106px);
  border-radius: 0;
  box-shadow: none;
}
.live-stage {
...
.live-feed li a {
  grid-template-columns: minmax(92px, 1fr) minmax(70px, 0.72fr) 8px;
}
.live-feed li b {
  font-size: 0.74rem;
}
...
.live-feed__result strong {
  font-size: 0.72rem;
}
.live-feed__result small {
  font-size: 0.58rem;
}
.live-board__loading,
.live-board__state {
