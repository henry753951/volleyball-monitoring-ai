...
<template>
  <section class="match-browser" aria-labelledby="match-browser-title">
    <div class="match-browser__heading">
      <div><p class="match-browser__eyebrow">VollyAI</p><h1 id="match-browser-title">場次</h1></div>
      <span class="match-browser__count">{{ matchState.matches.value.length }} 場</span>
    </div>
...
<style scoped>
.match-browser{width:min(100%,1080px);margin:0 auto}.match-browser__heading{display:flex;align-items:end;justify-content:space-between;margin:2px 2px 18px}.match-browser__eyebrow{margin:0 0 5px;color:#697687;font-size:.7rem;font-weight:700;letter-spacing:.08em}.match-browser__heading h1{margin:0;font-size:1.7rem;line-height:1.05;letter-spacing:-.035em}.match-browser__count{padding-bottom:3px;color:#6c7785;font-size:.75rem;font-weight:650}.match-list{margin:0;padding:0;overflow:hidden;border:1px solid #dfe5eb;border-radius:16px;background:#fff;box-shadow:0 12px 34px #1822300d;list-style:none}.match-list li+li{border-top:1px solid #e8ebef}.match-row{min-height:104px;display:grid;grid-template-columns:minmax(220px,1fr) minmax(260px,.82fr) 100px;align-items:center;gap:18px;padding:16px 20px;color:inherit;text-decoration:none;transition:background 120ms ease-out}.match-row:hover{background:#f8fafc}.match-row:active{background:#eef4fb}.match-row__meta{min-width:0;display:grid;grid-template-columns:auto 1fr;align-items:center;gap:5px 8px}.match-row__meta strong{overflow:hidden;font-size:.98rem;letter-spacing:-.01em;text-overflow:ellipsis;white-space:nowrap}.match-row__meta small{grid-column:1/-1;color:#687383;font-size:.74rem}.match-status{display:inline-flex;width:max-content;align-items:center;gap:4px;color:#737985;font-size:.67rem;font-weight:700}.match-status--live{color:#16734a}.match-score{display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:12px;font-variant-numeric:tabular-nums}.match-score div{display:flex;align-items:baseline;justify-content:flex-end;gap:9px}.match-score div:last-child{justify-content:flex-start}.match-score span{max-width:8rem;overflow:hidden;color:#5f6672;font-size:.77rem;font-weight:650;text-overflow:ellipsis;white-space:nowrap}.match-score b{font-size:2rem;line-height:1;letter-spacing:-.035em}.match-score i{color:#a3a9b2;font-style:normal}.match-row__set{display:flex;align-items:center;justify-content:flex-end;gap:5px;color:#777e89;font-size:.74rem}.match-row--skeleton{height:104px;background:#edf1f4;animation:pulse 1.1s ease-in-out infinite alternate}.state-panel{min-height:108px;display:flex;align-items:center;justify-content:center;gap:12px;padding:20px;border:1px solid #dfe5eb;border-radius:16px;background:#fff;color:#626975}.state-panel--error{justify-content:flex-start;color:#a3262e}.state-panel p{margin:2px 0 0;font-size:.78rem}.state-panel button{margin-left:auto;min-height:44px;padding:0 14px;border:0;border-radius:10px;background:#e9eef5;color:#17202b;font-weight:700}@keyframes pulse{to{opacity:.55}}@media(max-width:760px){.match-row{grid-template-columns:minmax(0,1fr) auto;padding:14px}.match-score{grid-row:2;grid-column:1/-1}.match-row__set{grid-row:1/3;grid-column:2}.match-row__meta{padding-right:24px}}@media(prefers-reduced-motion:reduce){.match-row,.match-row--skeleton{transition:none;animation:none}}
</style>
