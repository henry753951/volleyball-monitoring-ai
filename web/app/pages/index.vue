...
<template>
  <section class="match-browser" aria-labelledby="match-browser-title">
    ...
    <div v-if="authUnavailable" class="state-panel state-panel--error" role="alert">
      <CircleAlert :size="20" />
      <div>
        <strong>連線失敗</strong>
        <p>
      ...
      <button type="button" @click="viewerState.refresh">重試</button>
    </div>
    <div
      v-else-if="authRequired || (viewerState.checked.value && !viewerState.viewer.value)"
      class="state-panel"
      role="status"
    >
      請先登入以查看場次。
    ...
    <div
      ...
      aria-busy="true"
    >
      <div v-for="n in 3" :key="n" class="match-row match-row--skeleton" />
    </div>
    <div v-else-if="matchState.error.value" class="state-panel state-panel--error" role="alert">
      <CircleAlert :size="20" />
      <div>
        <strong>場次載入失敗</strong>
        <p>{{ matchState.error.value.message }}</p>
      </div>
      <button type="button" @click="matchState.refresh">重試</button>
    </div>
    <div v-else-if="!matchState.matches.value.length" class="state-panel">
      <p>目前沒有場次。</p>
    </div>
      ...
      <li v-for="match in matchState.matches.value" :key="match.id">
        <NuxtLink :to="`/matches/${match.id}/live`" class="match-row">
          <div class="match-row__meta">
            <span class="match-status" :class="`match-status--${coachMatchStatus(match).kind}`"
              ><Radio v-if="coachMatchStatus(match).kind === 'live'" :size="12" />{{
                coachMatchStatus(match).label
              }}</span
            >
          ...
          <div class="match-score" aria-label="目前比分">
            <div>
              <span>{{
                matchTeams(match).left?.shortName || matchTeams(match).left?.name || '左隊'
              }}</span
            ...
            <div>
              ...
              ><span>{{
                matchTeams(match).right?.shortName || matchTeams(match).right?.name || '右隊'
              }}</span>
            </div>
          </div>
          <div class="match-row__set">
            <span>第 {{ currentSet(match)?.setNumber ?? 1 }} 局</span><ChevronRight :size="20" />
          </div>
        </NuxtLink>
      </li>
    </ol>
