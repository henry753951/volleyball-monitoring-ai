...
const tokenDeleteTarget = shallowRef<DeepReadonly<AiWorkerTokenSnapshot> | null>(null)
...
const filteredMatches = computed(() => {
  ...
  return matchesState.matches.value.filter(match =>
    [match.title, match.venue, ...match.teams.flatMap(team => [team.name, team.shortName])].some(
...
const streams = computed(() =>
  visibleStreamsForMatches(monitor.snapshot.value?.operations.streams ?? [], visibleMatchIds.value),
)
const streamsByMatch = computed(() => Map.groupBy(streams.value, stream => stream.matchId))
const jobsByMatch = computed(() => Map.groupBy(activeAiWork.value, job => job.matchId))
...
const objectStorage = computed(() => monitor.snapshot.value?.operations.objectStorage ?? null)
const matchMediaById = computed(
  () =>
    new Map(
      (monitor.snapshot.value?.operations.matchMedia ?? []).map(item => [item.matchId, item]),
    ),
)
    ...
    [
      {
        name: '控制介面',
        detail: '管理工作站',
        value: monitor.error.value ? '無法連線' : monitor.snapshot.value ? '已連線' : '連線中',
        tone: monitor.error.value ? 'danger' : monitor.snapshot.value ? 'good' : 'warning',
        icon: Braces,
      },
      {
        name: '核心 API',
        detail: `程序運行 ${formatSeconds(monitor.snapshot.value?.operations.process.uptimeSeconds ?? 0)}`,
        value: monitor.snapshot.value?.readiness.status === 'ready' ? '服務正常' : '服務降級',
        tone: monitor.snapshot.value?.readiness.status === 'ready' ? 'good' : 'danger',
        icon: Server,
      },
      {
        name: 'PostgreSQL',
...
async function saveMatch(input: Parameters<typeof core.updateMatch>[0]) {
  ...
  try {
    ...
    editError.value = error instanceof Error ? error : new Error('場次更新失敗')
  } finally {
...
async function confirmDelete() {
  ...
  try {
    ...
    deleteError.value = error instanceof Error ? error : new Error('場次刪除失敗')
  } finally {
...
async function createToken() {
  ...
  try {
    ...
    tokenError.value = error instanceof Error ? error.message : 'Token 建立失敗'
  } finally {
...
async function rotateToken(token: DeepReadonly<AiWorkerTokenSnapshot>) {
  ...
  try {
    ...
    tokenError.value = error instanceof Error ? error.message : 'Token 更新失敗'
  } finally {
    tokenPending.value = false
  }
}
async function toggleToken(token: DeepReadonly<AiWorkerTokenSnapshot>) {
  try {
    await setAiWorkerTokenEnabled(runtimeConfig.public.restBasePath, token.id, !token.enabled)
    await monitor.refresh()
...
async function confirmTokenDelete() {
  ...
  try {
    ...
    await monitor.refresh()
    toast.success(`已刪除「${token.name}」；使用中的本機 Worker 將在下一次心跳退出`)
    if (revokedWorkerRefreshTimer) clearTimeout(revokedWorkerRefreshTimer)
    revokedWorkerRefreshTimer = setTimeout(() => {
      void monitor.refresh()
    }, 11_000)
  } catch (error) {
    tokenDeleteError.value = error instanceof Error ? error : new Error('Token 刪除失敗')
  } finally {
  ...
  <section class="control-page">
    ...
    <div v-if="monitor.error.value" class="monitor-error" role="alert">
      <AlertTriangle :size="17" /><span
        ><strong>監控資料暫時無法更新</strong>{{ monitor.error.value.message }}</span
    ...
    <div v-if="view === 'overview'" class="view-panel">
      <section class="ops-command">
        <div class="ops-health">
          <i :class="monitor.snapshot.value?.readiness.status === 'ready' ? 'good' : 'danger'" />
          <div>
            <strong>{{
              monitor.snapshot.value?.readiness.status === 'ready' ? '所有核心服務正常' : '服務降級'
            }}</strong
    ...
    <div v-else-if="view === 'matches'" class="view-panel">
      <div class="control-actions">
        ...
        ><button type="button" class="primary-action" @click="createOpen = true">
          <Plus :size="16" />新增場次
        </button>
      </div>
      <div v-if="matchesState.pending.value" class="match-loading" />
      <div v-else-if="!filteredMatches.length" class="empty-state">
        <CircleDot :size="22" /><strong>沒有符合的場次</strong><span>調整搜尋條件或新增場次</span>
      </div>
      ...
      <section class="workspace-section">
        ...
        <div class="system-grid">
          <article v-for="service in systemRows" :key="service.name">
            <span><component :is="service.icon" :size="17" /></span>
            <div>
      ...
      <section class="workspace-section runtime">
        ...
        <dl>
          <div>
            ...
            <dd>
              {{ formatBytes(monitor.snapshot.value?.operations.process.residentBytes ?? 0) }}
            </dd>
          </div>
          <div>
            <dt><Activity :size="15" />JavaScript Heap</dt>
            <dd>
              {{ formatBytes(monitor.snapshot.value?.operations.process.heapUsedBytes ?? 0) }}
            </dd>
          </div>
          <div>
            <dt><Clock3 :size="15" />運行時間</dt>
            <dd>
              {{ formatSeconds(monitor.snapshot.value?.operations.process.uptimeSeconds ?? 0) }}
            </dd>
          </div>
          <div>
    ...
    <ControlMatchDeleteDialog
      :open="deleteOpen"
      :match="deleteTarget"
      :media="deleteTarget ? (matchMediaById.get(deleteTarget.id) ?? null) : null"
      ...
      <div class="token-dialog">
        <template v-if="revealedToken"
          ...
          <div class="revealed-token">
            ...
            ><button type="button" @click="copy(revealedToken)"><Copy :size="15" />複製</button>
          </div>
          <p><ShieldCheck :size="14" />伺服器只保存 SHA-256 hash。</p>
          <button type="button" class="dialog-primary" @click="tokenDialogOpen = false">
            完成
          </button></template
        >
        <template v-else-if="rotatingToken"
          ><div class="rotating">
            <RotateCw :size="20" :class="{ spinning: tokenPending }" /><strong>{{
              tokenPending ? '正在產生新 Token…' : tokenError || '準備輪替'
            }}</strong
        ...
        <template v-else
          ...
          />
          <p v-if="tokenError" class="dialog-error">
            {{ tokenError }}
          </p>
          <button
            type="button"
            ...
            @click="createToken"
          >
            <KeyRound :size="15" />{{ tokenPending ? '建立中…' : '建立 Token' }}
          </button></template
        >
      </div>
    </UiAnimatedModal>
