...
const views = [
  { id: 'overview', label: '運行總覽', icon: Gauge, to: '/control' },
  {
    ...
    label: '場次管理',
    icon: CalendarDays,
    to: { path: '/control', query: { view: 'matches' } },
  },
  {
    id: 'systems',
    label: '系統狀態',
    icon: Activity,
    to: { path: '/control', query: { view: 'systems' } },
  },
  {
    id: 'workers',
    label: 'AI Workers',
    icon: KeyRound,
    to: { path: '/control', query: { view: 'workers' } },
  },
...
.control-nav-item__arrow {
  opacity: 0;
}
.control-nav-item.active .control-nav-item__arrow {
  opacity: 0.72;
}
.control-nav-item:focus-visible,
.control-rail__footer a:focus-visible,
.control-brand:focus-visible {
  outline: 2px solid #f1f3f5;
  outline-offset: 2px;
}
.control-rail__footer {
  display: grid;
  gap: 5px;
  margin-top: auto;
  padding-top: 12px;
...
.control-rail__footer a:hover {
  background: #18191d;
  color: #fff;
}
.control-rail__footer a svg:last-child {
  margin-left: auto;
}
.control-main {
  min-width: 0;
