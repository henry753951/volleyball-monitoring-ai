...
- Any authorized annotator may explicitly select and edit an `OPEN` or `READY` draft. Active draft selection, cursor, key-point selection, and outbox remain client-local; passive broadcasts must not switch them. Submitted masks and points remain immutable visual history.
