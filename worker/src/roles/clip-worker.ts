...
async function probeCanonicalClip(
  filePath: string,
  ...
  const output = await runCommand(
    ...
    [
      ...
      'error',
      '-show_entries',
      'stream=codec_type,width,height,avg_frame_rate,time_base,start_pts,duration_ts:frame=media_type,pts,pkt_duration,key_frame',
      '-of',
      'json',
      filePath,
    ],
...
async function failClipJob(database: PrismaClient, jobId: string, error: unknown) {
  const current = await database.clipJob.findUnique({
    where: { id: jobId },
    select: { attemptCount: true, maxAttempts: true, submission: { select: { rallyId: true } } },
  })
  if (!current) return
  const terminal = current.attemptCount >= current.maxAttempts
  await database.$transaction([
    database.clipJob.update({
      where: { id: jobId },
      data: {
        status: terminal ? JobStatus.FAILED : JobStatus.QUEUED,
        leasedUntil: null,
        availableAt: new Date(Date.now() + Math.min(60_000, 1_000 * 2 ** current.attemptCount)),
        errorCode: 'CLIP_GENERATION_FAILED',
        errorMessage:
          error instanceof Error ? error.message.slice(0, 500) : 'clip generation failed',
      },
    }),
    database.rally.update({
      where: { id: current.submission.rallyId },
      data: { processingStatus: terminal ? ProcessingStatus.FAILED : ProcessingStatus.CLIP_QUEUED },
    }),
  ])
}
...
export function createClipWorker(
  ...
  async function processNext(signal: AbortSignal): Promise<boolean> {
    ...
    const cancellationMonitor = setInterval(() => {
      ...
      void database.clipJob
        .findUnique({ where: { id: claimed.id }, select: { status: true } })
        .then(current => {
          if (
            !current ||
    ...
    try {
      ...
      const indexedSegments = await Promise.all(
        segments.map(async segment => {
          const document = JSON.parse(
            (await readVerifiedObject(storage.client, segment.sampleIndexAsset!)).toString('utf8'),
          )
      ...
      await runCommand(
        'ffmpeg',
        buildCanonicalClipFfmpegArgs(sourceListPath, outputPath, selection, 'concat'),
        jobSignal,
      )
      const probe = await probeCanonicalClip(
        outputPath,
        { fpsNum: program.fpsNum, fpsDen: program.fpsDen },
        jobSignal,
      )
      const video = parseCanonicalClipProbe(
        probe.payload,
        selection.sourceSamples.length,
        probe.fallback,
      )
      ...
      const clipUpload = await uploadFile(
        ...
        outputPath,
        'video/mp4',
        { 'x-amz-meta-artifact-kind': 'canonical-clip' },
      )
      const aiKeyPoints = mappings.map(mapping => ({
        key_point_id: mapping.submissionKeyPointId,
        sequence_index: mapping.sequenceIndex,
        marker_kind: mapping.markerKind,
        is_terminal: mapping.isTerminal,
        clip_pts: mapping.clipPts.toString(),
        clip_time_us: mapping.clipTimeUs.toString(),
        clip_frame_index: mapping.clipFrameIndex.toString(),
      }))
      const aiBoundaries = boundaryMappings.map(mapping => ({
        kind: mapping.kind,
        clip_pts: mapping.clipPts.toString(),
        clip_time_us: mapping.clipTimeUs.toString(),
        clip_frame_index: mapping.clipFrameIndex.toString(),
      ...
      const manifest = {
        schema_version: boundaryMappings.length ? '2.0.0' : '1.1.0',
        clip_job_id: job.id,
        submission_id: job.submissionId,
        requested_start_capture_us: job.requestedStartCaptureUs.toString(),
        requested_end_capture_us: job.requestedEndCaptureUs.toString(),
        actual_start_capture_us: actualStart.toString(),
        actual_end_capture_us: actualEnd.toString(),
        source_time_base: {
          num: selection.sourceTimeBase.num.toString(),
          den: selection.sourceTimeBase.den.toString(),
        },
        video: {
          width: video.width,
          height: video.height,
          fps: video.fps,
          time_base: video.timeBase,
          total_frames: video.totalFrames.toString(),
          duration_us: video.durationUs.toString(),
          has_audio: video.hasAudio,
        },
        frame_map: frameMap,
        boundaries: boundaryMappings.map(mapping => ({
          kind: mapping.kind,
          capture_epoch_id: mapping.captureEpochId,
          source_pts: mapping.sourcePts.toString(),
          capture_time_us: mapping.captureTimeUs.toString(),
          capture_frame_index: mapping.captureFrameIndex.toString(),
          clip_pts: mapping.clipPts.toString(),
          clip_time_us: mapping.clipTimeUs.toString(),
          clip_frame_index: mapping.clipFrameIndex.toString(),
        })),
        key_points: mappings.map(mapping => ({
          key_point_id: mapping.submissionKeyPointId,
          sequence_index: mapping.sequenceIndex,
          marker_kind: mapping.markerKind,
          is_terminal: mapping.isTerminal,
      ...
      const manifestUpload = await uploadFile(
        ...
        'application/json',
        {
          'x-amz-meta-artifact-kind': 'timing-manifest',
          'x-amz-meta-internal-schema-version': manifest.schema_version,
        },
      )

      await database.$transaction(async tx => {
        await tx.$queryRaw`SELECT id FROM "ClipJob" WHERE id = ${job.id}::uuid FOR UPDATE`
        const current = await tx.clipJob.findUnique({
          where: { id: job.id },
          select: { status: true },
        })
        if (!current || current.status !== JobStatus.RUNNING)
          throw new Error('PROCESSING_CANCELLED')
        const clipAsset = await tx.mediaAsset.create({
          data: {
            kind: MediaAssetKind.CANONICAL_CLIP,
        ...
        const timingAsset = await tx.mediaAsset.create({
          data: {
            kind: MediaAssetKind.TIMING_MANIFEST,
            bucket: storage.rallyBucket,
            objectKey: manifestKey,
            contentType: 'application/json',
            byteLength: manifestUpload.byteLength,
            sha256: manifestUpload.sha256,
            internalSchemaVersion: manifest.schema_version,
            state: 'READY',
            readyAt: new Date(),
          },
        })
        ...
        await tx.clipJob.update({
          ...
          data: {
            ...
            actualStartCaptureUs: actualStart,
            actualEndCaptureUs: actualEnd,
            clipAssetId: clipAsset.id,
            timingManifestAssetId: timingAsset.id,
            leasedUntil: null,
        ...
        if (aiBoundaries.length !== 2)
          throw new Error('AnalysisData Job 3.0 requires immutable start and end boundaries')
        const jobSchemaVersion = AI_JOB_SCHEMA_VERSION
        const basePayload = {
          schema_version: jobSchemaVersion,
          ai_job_id: aiJobId,
          ...
          clip: {
            ...
            sha256: clipUpload.sha256,
            byte_length: clipUpload.byteLength.toString(),
        ...
        await tx.aiJob.create({
          data: {
            ...
            clipJobId: job.id,
            idempotencyKey: `volleyball-analysis-engine:${job.submissionId}:${job.id}`,
            requestPayload: basePayload,
            requestPayloadHash: sha256Hex(stableJson(basePayload)),
            jobSchemaVersion,
      ...
      if (
        ...
        !(error instanceof Error && error.message === 'PROCESSING_CANCELLED')
      ) {
